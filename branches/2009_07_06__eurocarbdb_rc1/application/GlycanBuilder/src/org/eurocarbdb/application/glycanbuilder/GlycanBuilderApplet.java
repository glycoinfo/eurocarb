...
/**
   Implement an applet in which a {@link GlycanCanvas} is
   inserted. Provides menus and toolbars for creating and editing a
   set of glycan structures.
...
public class GlycanBuilderApplet extends JApplet implements ActionListener, MouseListener {   
    ...
    */
    ...
    theCanvas = new GlycanCanvas(null,theWorkspace);
    ...
    for(Iterator<String> s=CoreDictionary.getSuperclasses().iterator(); s.hasNext(); ) {
        String superclass = s.next();

        JMenu class_menu = new JMenu(superclass);        
        for( Iterator<CoreType> i = CoreDictionary.getCores(superclass).iterator(); i.hasNext(); ) {
        CoreType t = i.next();
    ...
    */
    ...
    try {
        JDialog dlg = new JDialog((JFrame)null, "About GlycanBuilder", true);        
    ...
    /*
    public void addJSListener(String call_back) {
    if( call_back!=null )
        js_listeners.add(call_back);
    }

    public void removeJSListener(String call_back) {
    if( call_back!=null )
        js_listeners.remove(call_back);
    }

    public void fireJSListeners() {
    if( js_listeners.size()>0 ) {
        JSObject win = JSObject.getWindow(this);      
        for( String l : js_listeners ) 
        win.call(l, null);            
    }
    }
    */
    ...
    /**
       Return the structures encoded into a string in the internal
       format
     */
    ...
    /**
       Return the flag that specify if the linkage information should
       be displayed
    ...
    /**
       Return the flag that specify if the mass options should be
