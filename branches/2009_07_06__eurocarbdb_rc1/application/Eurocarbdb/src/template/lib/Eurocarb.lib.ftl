...
<#if scan?exists>
  <#include "/template/action/ms/show_scan/scan_detail_brief.ftl"/>
</#if>
</#macro>
...
-->
<#macro sugar_image id scale=1 output="png"><img id="glycanSequence_${id?c}" class="sugar_image" src="get_sugar_image.action?download=false&amp;scale=${scale}&amp;outputType=${output}&amp;tolerateUnknown=1&amp;glycanSequenceId=${id?c}"/></#macro>
...
-->
<#macro sugar_image_for_seq seq format="gws" output="png" scale=1 map...><img class="sugar_image" src="get_sugar_image.action?download=true&amp;scale=${scale}&amp;outputType=${output}&amp;inputType=${format}&amp;tolerateUnknown=1&amp;sequences=${seq}" <#if map?exists><#list map?keys as k>${k}="${map[k]}" </#list></#if>/></#macro>
