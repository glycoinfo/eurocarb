...
public class AnnotationThread extends Thread implements GeneratorListener {
    ...
    return (structures!=null && peaks!=null) ?(structures.size() * peaks.size()) :0;
    }
    ...
    if( mode==FRAGMENTER ) {
        ...
        if( structure!=null ) {
            ...
            FragmentCollection frag_coll = fragmenter.computeAllFragments(structure);
            ...
            if( ann_opt.DERIVE_FROM_PARENT ) {
                ...
                ion_clouds = IonCloudUtils.getPossibleIonClouds(der_ann_opt);
            }
            
            // annotate fragments
            match_coll = match(peaks,frag_coll,ion_clouds,der_ann_opt);
            annotated_peaks.addPeakAnnotations(structure,match_coll,false);
            }
        }     
        else
            progress += peaks.size();
        ...
        match_coll = match(peaks,frag_doc.getFragments(i),ion_clouds,ann_opt);
        annotated_peaks.addPeakAnnotations(frag_doc.getStructure(i),match_coll,false);
        }
    }
    ...
    static public Collection<FragmentEntry> computeChargesAndExchanges(Glycan structure, Collection<FragmentEntry> fragments, AnnotationOptions ann_opt) {
    // compute possible charges
    if( ann_opt.DERIVE_FROM_PARENT ) 
    ...
    static public Collection<FragmentEntry> computeChargesAndExchanges(Collection<FragmentEntry> fragments, Collection<IonCloud> ion_clouds, AnnotationOptions ann_opt) {
    ...
    if( ann_opt.COMPUTE_EXCHANGES ) {        
        ...
        for( IonCloud cloud : ion_clouds ) {
            ...
            for( Peak p : peaks.getPeaks() ) {
                ...
                match_coll.addPeakAnnotation(p,fe.and(cloud,nex));
            }
            }                
            if( interrupted() )
            return false;
    ...
    static public boolean match(Peak p, FragmentEntry fe, AnnotationOptions ann_opt) {
    double fmz = fe.mz_ratio;
    double mz_ratio = p.getMZ();
    return match(fmz,mz_ratio,ann_opt);
    }

    static public boolean match(Peak p, FragmentEntry fe, IonCloud ions, AnnotationOptions ann_opt) {
    double fmz = ions.computeMZ(fe.mass);
    double mz_ratio = p.getMZ();
    return match(fmz,mz_ratio,ann_opt);
    }

    static public boolean match(Peak p, FragmentEntry fe, IonCloud ions, IonCloud neutral_exchange, AnnotationOptions ann_opt) {
