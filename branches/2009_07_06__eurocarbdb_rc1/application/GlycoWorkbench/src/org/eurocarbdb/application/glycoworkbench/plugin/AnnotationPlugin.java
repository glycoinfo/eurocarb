...
public class AnnotationPlugin implements Plugin, ActionListener {
    ...
    theAnnotationToolsPane.add("Calibration", thePeakAnnotationCalibrationPanel);
    }
    ...
    actions.add(null);
    actions.add(new GlycanAction("findFragmentsCurrent",FileUtils.getIcon(""),"Find all fragments of the current structure with a given m/z value",KeyEvent.VK_C,"",this));
    actions.add(new GlycanAction("findFragmentsSelected",FileUtils.getIcon("findfragments"),"Find all fragments of the selected structures with a given m/z value",KeyEvent.VK_S,"",this));
    actions.add(new GlycanAction("findFragmentsAll",FileUtils.getIcon(""),"Find all fragments of all the structures with a given m/z value",KeyEvent.VK_A,"",this));
    actions.add(null);
    actions.add(new GlycanAction("matchFragmentsCurrent",FileUtils.getIcon(""),"Annotate peaks with fragments from current structure",KeyEvent.VK_U,"",this));
    actions.add(new GlycanAction("matchFragmentsSelected",FileUtils.getIcon("matchfragments"),"Annotate peaks with fragments from selected structures",KeyEvent.VK_E,"",this));
    actions.add(new GlycanAction("matchFragmentsAll",FileUtils.getIcon(""),"Annotate peaks with fragments from all structures",KeyEvent.VK_L,"",this));
    actions.add(null);
    actions.add(new GlycanAction("placeAntennae",FileUtils.getIcon(""),"Place uncertain antennae in current structure using the peak list",KeyEvent.VK_P,"",this));
    ...
    theManager = manager;
    if( theManager!=null ) 
        theManager.addMsMsPeakAction(new GlycanAction("findFragmentsSelected",FileUtils.getIcon("findfragments"),"Find all fragments of the selected structures matching the peaks",KeyEvent.VK_S,"",this));
    }
    ...
    if( view.equals("Stats") )
        theAnnotationToolsPane.setSelectedComponent(thePeakAnnotationStatsPanel);
    else if( view.equals("Details") )
        theAnnotationToolsPane.setSelectedComponent(thePeakAnnotationDetailsPanel);
    else if( view.equals("Summary") )
        theAnnotationToolsPane.setSelectedComponent(thePeakAnnotationSummaryPanel);
    else if( view.equals("Calibration") )
        theAnnotationToolsPane.setSelectedComponent(thePeakAnnotationCalibrationPanel);
    ...
    if( action.equals("findFragmentsCurrent") ) {
        ...
        if( matchAllFragments(ask,theApplication.getCanvas().getCurrentStructure(),peaks) ) {
    ...
    if( action.equals("findFragmentsSelected") ) {
        annotate = false;
        theApplication.getCanvas().enforceSelection();    
        if( matchAllFragments(ask,theApplication.getCanvas().getSelectedStructures(),peaks) ) {
        theManager.show("Search",(theApplication.getCanvas().getSelectedStructures().size()>1) ?"Summary" :"Details");
        return true;
        }
        return false;
    }
    if( action.equals("findFragmentsAll") ) {
        annotate = false;
        if( matchAllFragments(ask,theWorkspace.getStructures().getStructures(),peaks) ) {
        theManager.show("Search",(theWorkspace.getStructures().size()>1) ?"Summary" :"Details");
        return true;
        }
        return false;
    }    
    ...
    if( action.equals("placeAntennae") ) {
        annotate = true;
        theApplication.getCanvas().enforceSelection();    
        if( placeAntennae(ask,theApplication.getCanvas().getCurrentStructure(),peaks) ) {
        theManager.show("Annotation","Stats");
        return true;
        }
        return false;
    ...
    if( structure!=null )
        return matchAllFragments(ask_options,Collections.singleton(structure),peaks);
    return false;
    }
    ...
    // retrieve peak list
    if( peaks.size()==0 ) {
        JOptionPane.showMessageDialog(theApplication, "You must load a peak list first", "Error", JOptionPane.ERROR_MESSAGE);
        return false;
    }
    ...
    AnnotationOptions ann_opt = theWorkspace.getAnnotationOptions();
    if( ask_options ) {
        AnnotationOptionsDialog dlg = new AnnotationOptionsDialog(theApplication,frag_opt,ann_opt,true,true);
        dlg.setVisible(true);     
    ...
    // check for linkages
    if( (frag.getComputeAFragments() || frag.getComputeXFragments()) && ask_options ) {
    ...
    if( current.getNoAntennae()>2 ) {
        JOptionPane.showMessageDialog(theApplication, "This features is still experimental and the maximum number of antennae that can be placed is set to two.", "Warning", JOptionPane.WARNING_MESSAGE);
        return false;
    }
    ...
    // start activity
    theThread = new AnnotationThread(_peaks,_structures,_fragmenter,_ann_opt);
    theThread.start();
    ...
    activityMonitor = new Timer(200, new ActionListener() {  
        public void actionPerformed (ActionEvent event) {  
            ...
            if( progress==theThread.getTarget() || progressDialog.isCanceled ()) {  
