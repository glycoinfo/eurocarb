...
public class Casper extends EurocarbAction
{
    ...
    pSimExp=Pattern.compile("(^sim)|(^exp)");
    pAssign=Pattern.compile("((^1H)|(^13C)|(^CH)) chemical");
    pStr=Pattern.compile("(.*?)\\(error\\sest");
    ...
    try
        ...
        while((data=input.readLine())!=null)
            ...
            if(data.equals("list start"))
                ...
                while((data=input.readLine())!=null && 
                    ...
                    if(data.startsWith("sim"))
                        ...
                        /*                        if(!this.getSequenceGWS().equals(""))
                            {
                            this.appendResults(generateGraphics(input));
                            }
                        else
                            {
                            this.appendResults(fixStructureOutput(structure));
                            }*/
                ...
                this.appendResults("<tr><td colspan=\"2\">");
                this.appendResults("<br /><a onclick=\"toggleShow();\">Show/hide score details</a>");
                this.appendResults("</td></tr>");
                }
            if(data!=null)
                {
            ...
            if(m.lookingAt())
                ...
                data=input.readLine();
                data=input.readLine();
                System.out.println("Generating results table");
                this.appendResults("<table width=\"100%\">");
                m=pStr.matcher(data);
                while(m.lookingAt())
                    {
                    match=m.group(1);
                    this.printResidue(input, match);
                    if((data=input.readLine())!=null)
                        {
                        m=pStr.matcher(data);
                        }
        ...
        if(!list)
            ...
            if(pic.exists())
                {
                this.appendResults("<br /><a href=\"/eurocarb/ww/nmr/casper_" + this.getMode() + "_picture_stream.action?pictureFileName=cSpectra.png\"><img src=\"/eurocarb/ww/nmr/casper_" + this.getMode() + "_picture_stream.action?pictureFileName=cSpectra.png\" width=\"35%\" height=\"35%\" /></a>\n");
                }
            else
                {
                System.out.println("No picture of C spectrum.");
                }
            pic=new File(outDir + File.separator + "chSpectra.png");
            if(pic.exists())
                {
                this.appendResults("<a href=\"/eurocarb/ww/nmr/casper_" + this.getMode() + "_picture_stream.action?pictureFileName=chSpectra.png\"><img src=\"/eurocarb/ww/nmr/casper_" + this.getMode() + "_picture_stream.action?pictureFileName=chSpectra.png\" width=\"35%\" height=\"35%\" /></a><br />\n");
                }
            else
                {
            ...
            if(pic.exists())
                ...
                {
                System.out.println("No picture of H spectrum.");
                }
            }
        }
    catch(IOException e)
    ...
    public void printResidue(BufferedReader input, String str) throws IOException
    {
    String data;
    
    /* First print the structure */
    str=this.fixStructureOutput(str);
    this.appendResults("<tr><td rowspan=3 style=\"vertical-align: middle\">" + str + "</td>");
    ...
    public String generateGraphics(BufferedReader input) throws IOException
    {
    ...
    /*    outDir=new File(File.separator + "tmp" + File.separator + "casper");*/
    ...
    try
        ...
        /*        if(outDir.exists())
            ...
            if(!outDir.isDirectory())
                {
                throw new IOException("Cannot create casper directory");
                }
            }
        else
            {
