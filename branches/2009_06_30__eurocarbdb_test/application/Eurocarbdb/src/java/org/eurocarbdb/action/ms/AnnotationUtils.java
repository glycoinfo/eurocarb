...
*   for more details.
*
*   Last commit: $Rev: 1274 $ by $Author: hirenj $ on $Date:: 2009-06-26 #$  
*/
...
public class AnnotationUtils {
    ...
    protected static final Logger log = Logger.getLogger( AnnotationUtils.class.getName() );
    
    static public AnnotatedPeakList parseAnnotations(File file) throws Exception 
    {
    ...
    static public AnnotatedPeakList restrictToSequence(AnnotatedPeakList annotations, GlycanSequence glycan_sequence) {
        ...
        for( int i=0; i<annotations.getNoStructures(); i++ ) {
            if( glycan_sequence.getSequenceCtCondensed().equals(annotations.getStructure(i).withNoReducingEndModification().toGlycoCTCondensed()) ) 
    ...
    static public AnnotatedPeakList annotate(GlycanSequence glycan_sequence, Scan scan, MassOptions mass_opt, FragmentOptions frag_opt, AnnotationOptions ann_opt) {
    ...
    static private AnnotatedPeakList annotate(GlycanDocument document, PeakList peaklist, MassOptions mass_opt, FragmentOptions frag_opt, AnnotationOptions ann_opt) {
        // create fragmenter
    ...
    {       
        ...
        for( int i=0; i<annotated_peaklist.getNoStructures(); i++ ) {
            ...
            for( org.eurocarbdb.application.glycoworkbench.Annotation a : annotated_peaklist.getAnnotations(l,i) ) {
                ...
                PeakAnnotated pa_obj = new PeakAnnotated(pan_obj,structure_id,a.getFragmentEntry().getStructure(),"",0.,a.getMZ(),contributor_quality);
                ...
                for( Residue cleav : a.getFragmentEntry().getFragment().getCleavages() ) {
                    ...
                    f_obj = new Fragmentation(pa_obj,crt.getRingFragmentType(),"","",-1,crt.getFirstPos(),crt.getLastPos());
                    }
                    else
    ...
    {    
        ...
        int nav_p = 0;
        int nav_sa = 1;
        int peak_id = getPeakId(selected_annotations[0]);
        int ann_id = getAnnotationId(selected_annotations[0]);
        boolean exit_p = false;
        for( Iterator<PeakAnnotation> i=annotation.getPeakAnnotationsOrdered().iterator(); i.hasNext() && !exit_p; ) 
        {
            PeakAnnotation pan = i.next();
    
            if( peak_id==nav_p ) 
            {
                int nav_a = 0;
                boolean exit_a = false;
                for( Iterator<PeakAnnotated> l=pan.getPeakAnnotateds().iterator(); l.hasNext() && !exit_a; ) 
                {
                    PeakAnnotated pa = l.next();
                    ...
                    if( ann_id==nav_a ) 
                    {
                        ...
                        if( nav_sa<selected_annotations.length ) 
                        {
                            // update ids
                            peak_id = getPeakId(selected_annotations[nav_sa]);
                            ann_id = getAnnotationId(selected_annotations[nav_sa]);
                            exit_a = (peak_id!=nav_p); // end of selections for this peak
                        }
                        else 
                        {
                            // end of selections
