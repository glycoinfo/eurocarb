...
public class CreateAnnotations extends EurocarbAction implements RequiresLogin, EditingAction
{
    ...
    if( submitAction.equals("Select structure") ) {
    ...
    // run annotation
    if( submitAction.equals("Annotate") ) {
        if( glycan_sequence==null ) {
        this.addActionError( "No structure selected");
        return "error";
        }
        log.debug("Annotating the peak list for a scan with a glycan sequence");
    ...
    if( submitAction.equals("Delete selected") ) {
        if( selected_annotations!=null ) 
        AnnotationUtils.deleteAnnotations(annotation,selected_annotations);        
        return "edit";
    }
    
    if( submitAction.equals("Finish") ||  submitAction.equals("Store annotations") ) {
