...
*   for more details.
*
*   Last commit: $Rev: 1343 $ by $Author: hirenj $ on $Date:: 2009-06-30 #$  
*/
...
public class CreateAcquisition extends AbstractMsAction implements RequiresLogin, EditingAction
{
    ...
    {
        glycanSequenceContext = getObjectFromParams(GlycanSequenceContext.class, params, "glycanSequenceContext.glycanSequenceContextId");
        if (glycanSequenceContext == null)
        {
            this.addActionError("Must supply a valid GlycanSequenceContext ID if you wish to create");
            if (params.get("glycanSequenceContext.glycanSequenceContextId") == null) {
                this.addFieldError("glycanSequenceContext.glycanSequenceContextId", "No glycan sequence context identifier provided");
            } else {
                this.addFieldError("glycanSequenceContext.glycanSequenceContextId", "Bad glycan sequence context identifier provided");
            }
        } else {
            log.debug("Retrieved a glycan sequence context with id of "+glycanSequenceContext.getGlycanSequenceContextId());            
        }
    ...
    {       
        ...
        {    
            ...
            if( acquisitionFile!=null && "true".equals(Eurocarb.getProperty("ecdb.tranche.enabled")) ) {
                String hashcode = TrancheUtility.uploadFile(acquisitionFile).toString();
