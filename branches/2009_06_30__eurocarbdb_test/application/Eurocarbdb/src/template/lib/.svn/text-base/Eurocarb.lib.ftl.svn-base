...
<#-- Import javascript library handling macro -->
...
<#-- just prints the contents of ${context_menu} -->
...
<#--
Adds a sugar notation widget box thingy to the context panel.
-->
...
<script type="text/javascript">
document.getElementById('notation_${sugarImageNotation}').className = 'selected';
</script>
</#if>
</@context_box>
</#macro>
...
Args:
    ...
    - *    
        ...
        into the link, eg: title="href title" onlick="..."
...
action:<#attempt>${action.class.name}<#recover></#attempt>
...
<a href="show_${ object.type }.action?${ object.identifierClass.simpleName?uncap_first }Id=${ object.id?c }"<#if (map?exists && map?size > 0 )><#list map?keys as key > ${key}="${map[key]}"</#list></#if>>${text}</a></#macro>
...
<#--
macro: use_js_lib
usage: <@use_js_lib name=[Library name] /> 
author: hirenj

  Use a macro to include ECDB libraries. All dependencies for the libraries
...
<#--
macro: contributor
usage: <@contributor c=[Contributor object] /> 
usage: <@contributor c=[Contributor object] text="[some custom text here]" /> 
author: mjh

renders contributor detail text with hyperlink
-->
<#macro contributor c text=c.contributorName ><@detail_link object=c text=text /></#macro>


<#--
macro: taxonomy
usage: <@taxonomy t=[Taxonomy object] />
usage: <@taxonomy t=[Taxonomy object] text="[some custom text here]" />
author: mjh

display taxonomy detail text with hyperlinks
-->
...
<#--
macro: tissue
usage: <@tissue t=[TissueTaxonomy object] />
author: mjh
...
<#--
macro: disease
usage: <@disease d=[Disease object] />
author: ac
...
<#--
macro: perturbation
usage: <@perturbation p=[Perturbation object] />
author: ac
...
usage: <@biological_context bc=[BiologicalContext object] />
usage: <@biological_context bc=[BiologicalContext object] separator=[String] />
author: mjh,ac
...
<#--
macro: sugar_image
usage: <@sugar_image id=[GlycanSequence id] scale=[scale factor, default=1] output=[image type, default='png']/>
author: mjh
...
-->
<#macro sugar_image id scale=1 output="png"><img class="sugar_image" src="get_sugar_image.action?download=false&scale=${scale}&outputType=${output}&tolerateUnknown=1&glycanSequenceId=${id?c}"/></#macro>

<#--
macro: sugar_image_for_seq
usage: <@sugar_image seq=[a glycan sequence string] format=[seq format, default='gws'] scale=[scale factor, default=1] output=[image type, default='png']/>
author: mjh
...
-->
<#macro sugar_image_for_seq seq format="gws" output="png" scale=1><img class="sugar_image" src="get_sugar_image.action?download=true&scale=${scale}&outputType=${output}&inputType=${format}&tolerateUnknown=1&sequences=${seq}"/></#macro>

<#--
macro: linked_sugar_image
usage: <@linked_sugar_image id=[GlycanSequence id] scale=[scale factor]/>
author: mjh
...
'scale' argument should be between 0-1.0 inclusive.
-->
<#macro linked_sugar_image id seq="" scale=1><a href="show_glycan.action?glycanSequenceId=${id?c}" title="Sequence ID ${id?c} - click for full detail" class="sugar_image"><#if (seq?exists && seq?length > 0)><@sugar_image_for_seq seq=seq scale=scale /><#else><@sugar_image id=id scale=scale/></#if></a></#macro>


<#--
macro: experiment
usage: <@experiment e=[Experiment] />
author: mjh
...
usage: <@experiment_step object=[ExperimentStep] />
author: mjh
...
<#--
macro: reference
usage: <@reference ref=[Reference object] />
author: ac
...
<#elseif (ref.referenceType?exists && ref.referenceType=="database")>
      ${ref.externalReferenceName} entry <a href="${ref.url}">${ref.externalReferenceId}</a>
...
usage: <@reference_brief object=[Reference] />
author: mjh
...
${object.externalReferenceName} entry <a href="${object.url}">${object.externalReferenceId}</a>
...
<#--
macro: addGu
usage: <@addGu ref=[Reference object] />
author: mc
...
usage: <@page_navigator action_name=[string] />
author: ac, mjh

Display the links to change page in a pageable action
-->
...
<#macro xml object detail=true><#if detail>${object.toXml(true)}<#else/>${object.toXml(false)}</#if></#macro>
