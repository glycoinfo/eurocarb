...
/* Sorts 2D ChShifts and ChLrShifts on the distance from origo */
		       ...
		       unsigned int exp_ptr)
{
  ...
  */
  if(altdiff<diff*4 || altdiff<cutoff*4)
    {
  ...
  if(diff<altdiff*4 || diff<cutoff*4 || !temp_usedexp)
    {
	  ...
	  if(usedsim[sim_ptr]==-1)
	    {
		  ...
		  else if(ass_hi>ass_lo*8 && ass_hi>cutoff*8)
		    {
		      sim_ptr=sCnt;
		    }
		      ...
		      if(usedexp[new_exp_ptr]!=-1)
			{
			  ...
			  if(mode==SE_PURGE_CH)
			      ...
			      if(combDiff>=0)
				{
				 ...
				 in HH or ChLr add the combinated difference
				 from above */
			  ...
			  /* If the difference is getting too large don't continue to look */
			  else if(ass_hi>ass_med*8 && ass_hi>cutoff*8)
			    new_exp_ptr=eCnt;
      ...
      if(exp_ptr>=0)
	{
	  if(mode==SE_PURGE_C || mode==SE_PURGE_H)
	  ...
	  else
	    {
	      ...
	      if(mode==SE_PURGE_CHLR)
		{
		  if(mismatches[sim_ptr]>0)
		    {
		      abs_err+=cutoff*SP_MISMATCH_PENALTY;
		    }
		}
	    }
  ...
  if(mode==SE_PURGE_C||mode==SE_PURGE_CH)
      ...
      for(;i<sim->nCAssignments && fabs(sim->CAssignments[1][i]-sCShift[sim_ptr])<0.0001;i++)
	{
	  for(j=0;j<cnt;j++)
		...
		{
		  if(mode==SE_PURGE_CH||mode==SE_PURGE_CHLR)
		    {
...
/* Non-recursive function for matching very few shifts. It is used for calculating the combinated differences
   of Hh and ChLr to assist in assigning Ch. */
