  ...
  for(i=0;i<n;i++)
    {
      ...
      if(numDim==1)
	{
	  peak=CC_FindPeak(peakList, firstdim[i]+firstDimCorrection, 0, numDim, 0.00001);
	}
      else
	{
	  peak=CC_FindPeak(peakList, firstdim[i]+firstDimCorrection,
			   seconddim[i]+secondDimCorrection, numDim, 0.00001);
	}
...
Nmr_Peak CC_FindPeak(Nmr_PeakList peakList, float firstdim, float seconddim, int numDim, float tolerance)
{
  ...
  ApiFloat value;
  ApiInteger apiIntFirstDim, apiIntSecondDim;
  float peakDimValue;
  Nmr_Peak peak=0;
  ...
  for(j=0;j<numPeaks;j++)
    {
      ...
      peakDimValue=ApiFloat_Get(value);
      ApiObject_Free(value);
      if(fabs(peakDimValue-firstdim)<tolerance)
	{
	  ...
	  if(numDim>=2)
	    {
	      ...
	      peakDimValue=ApiFloat_Get(value);
	      ApiObject_Free(value);
	      if(fabs(peakDimValue-seconddim)<tolerance)
		{
		  j=numPeaks;
		}
	      else
...
Mols_MolSystem CC_MakeMoleculeFromBU_Struct(Impl_MemopsRoot *ccpnProject, struct BU_Struct *inp, char *name)
{
  ...
  if(strlen(inp->CcpnChainCode)>0)
      ...
      if(chain && !ApiObject_IsNone(chain))
	{
	  ...
	  if(molecule && !ApiObject_IsNone(molecule))
	    {
	      /*	      printf("Name %s\n", ApiString_Get(Mole_Molecule_GetName(molecule)));*/
  ...
  for(unit=(struct BU_Unit *)inp->Units.Head.Succ; unit->Node.Succ!=NULL;
      ...
      if(linked==0)
	{
	 ...
	 unit->Subst[unit->Residue->Shifts.Type->Anomeric]!=NULL)
	{
      ...
      if(!Mole_Molecule_CheckAllValid(molecule,ApiBoolean_False()))
	{
      ...
      if(!residue)
	{
	  printf("Cannot create residue. Is the reducing end free? Line %d.\n", __LINE__);
	  ApiObject_Free(molecule);
  ...
  if(!analProj || ApiObject_IsNone(analProj))
    {
      ...
      ApiMap_SetString(objectParameters, "name", ApiString_Get(string));
      ApiMap_SetItem(objectParameters, "nmrProject", *nmrProject);
      analProj=Impl_MemopsRoot_NewAnalysisProject(*ccpnProject, objectParameters);
      ApiObject_Free(objectParameters);
      ApiObject_Free(string);
      if(!analProj || ApiObject_IsNone(analProj))
	{
  ...
  while((residue=ApiSetIterator_Next(residueIterator)))
      ...
      else
	{
	  ...
	  ApiObject_Free(seqId);
	  {
	    ...
	    callModuleFunction("ccpnmr.analysis.core.MoleculeBasic", "getResidueMapping", args);
	  }
	  ApiObject_Free(objectParameters);
	  ApiObject_Free(resonanceGroup);
	}
...
int CC_AssignPeakToAtoms(Nmr_NmrProject *nmrProj, ApiSet atoms, float firstDim, float secondDim, int assignDim, const char mode)
{
  ...
  ApiSet peaks, shiftPeakDims, peakPeakDims, chemAtoms, experiments, resonanceSetsInAtomSet;
  ...
  int i, j, numShifts, numPeaks, wantedNumPeakDims, numChemAtoms;
  ...
  if(!atomSet||ApiObject_IsNone(atomSet))
      ...
      if(numAtoms>1)
	{
      ...
      ApiMap_SetString(parameters,"name",name);
      atomSet=Nmr_NmrProject_NewAtomSet(*nmrProj, parameters);
      ApiObject_Free(parameters);
      if(!atomSet || ApiObject_IsNone(atomSet))
	{
	  ApiObject_Free(residue);
	  ...
	  projectValid=0;
	  Error(PA_ERR_FATAL,"Could not create AtomSet");
	}
    }
  ApiObject_Free(atoms);
  ...
  switch (mode)
    ...
    case SE_PURGE_HH:
      ...
      break;
    case SE_PURGE_CHLR:
      experiments=CC_GetExpOfType(nmrProj, "H_[C].Jmultibond");
      break;
    default:
  ...
  peaks=CC_FindPeaks(peakListCollection, firstDim, secondDim, wantedNumPeakDims, 0.002);
  shifts=CC_FindShiftsFromPeaks(nmrProj, peaks, assignDim);
  ...
  if(numShifts>0)
      ...
      if(unassignedShiftNr<999 && targetResonance)
	{
	  ...
	  /*	  mergeResonances(targetResonance, resonanceInShift);*/

	  {
	    ApiObject args[] = { resonanceInShift, targetResonance, NULL };
	    callModuleFunction("ccpnmr.analysis.core.AssignmentBasic", "mergeResonances", args);
	  }
  ...
  for(j=0;j<numPeakDims;j++)
    {
      ...
      peakDimContrib=callModuleFunction("ccpnmr.analysis.core.AssignmentBasic", "assignPeakDim", args);
      /*      ApiObject_Free(peakDim);*/
      ApiObject_Free(peakDimContrib);
    }
  ApiObject_Free(atomSetCollection);
...

  {
    ...
    callModuleFunction("ccpnmr.analysis.core.AssignmentBasic", "initResonance", args);
  }
			    ...
			    float *newShifts, int newCnt, unsigned int mode)
{
  ...
  /*  if(newShifts==str->ChCShift || newShifts==str->ChHShift ||
     newShifts==str->ChLrCShift || newShifts==str->ChLrHShift ||
     newShifts==str->HhLoShift || newShifts==str->HhHiShift)
    {
  ...
  *//* If there is more than one entry with the same shift check
  ...
  /*  if(currentShifts==str->ChCShift || currentShifts==str->ChHShift ||
     currentShifts==str->ChLrCShift || currentShifts==str->ChLrHShift ||
     currentShifts==str->HhLoShift || currentShifts==str->HhHiShift)
    {
...
void CC_MergeShiftsInResonance(Nmr_NmrProject *nmrProj, Nmr_Resonance resonance)
{
...

  {
    ...
    shiftLists=callModuleFunction("ccpnmr.analysis.core.AssignmentBasic", "getShiftLists", args);
  }
  numShiftLists=ApiSet_Len(shiftLists);
		  ...
		  if(shiftList==secShiftList)
		    {
		      ...
		      shift=callModuleFunction("ccpnmr.analysis.core.MergeObjects", "mergeObjects", args);
		      args[0]=shift;
		      args[1]=NULL;
		      callModuleFunction("ccpnmr.analysis.core.AssignmentBasic", "averageShiftValue", args);
		    }
