...
/**
   ...
   structure. The identity of the possible ions is defined in {@link
   MassOptions}
...
public class IonCloud {    
    ...
    /**
       Compute the mass/charge value of a structure with a certain
       mass given the charges in this object
       @param mass the mass of the glycan molecule
     */
    ...
    /**
       Compute the original mass of a structure with a certain
       mass/charge value given the charges in this object
       @param mz the mass/charge value of the glycan molecule with the
       associated charges
     */
    ...
    /**
       Return <code>true</code> if the object represent a set of
       charges that could be found associated to a glycan structure
     */
    public boolean isRealistic() {
    for( Map.Entry<String,Integer> c : ions.entrySet() ) 
        if( !c.getKey().equals(MassOptions.ION_H) && c.getValue().intValue()<0 )
        return false;
    return true;
    }
    ...
    /**
       Return <code>true</code> if the number of charges is
       undetermined.
     */
    ...
    /**
       Return a copy of this object to which the content of a second
       object is added
     */
    ...
    /**
       Return a copy of this object to which a new charge has been
       added
     */
    ...
    /**
       Return a copy of this object to which a certain quantity of a
       new charge has been added
     */
    ...
    /**
       ...
       @param skip_undetermined <code>true</code> if charges with
       undetermined quantity should be ignored
     */
    ...
    /**
       Merge the information about a specific charge in a second
       object to this one
     */
    ...
    /**
       Return a map containing the identities and quantities of the
       charges in this object
     */
    ...
    /**
       Convert this object to a {@link Molecule} object containing the
       same information
     */
    ...
    /**
       Create a new object from its string representation. The string
       must contain either 0 or a mathematical formula specifying the
       identity and quantity of the charges (Example: 2Na-2H)
       @throws Exception if the string is in the wrong format
       @see #initFromString
     */
    ...
    for( int i=0; i<str_buffer.length; ) {
        ...
        for( ; i<str_buffer.length && Character.isDigit(str_buffer[i]); i++ )
            count.append(str_buffer[i]);
        }
        else 
        count.append('1');
        ...
        for( ; i<str_buffer.length && Character.isLetter(str_buffer[i]); i++ )
