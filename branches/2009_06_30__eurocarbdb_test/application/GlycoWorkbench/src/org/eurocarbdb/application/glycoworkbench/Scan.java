...
public class Scan implements BaseDocument.DocumentChangeListener {            
    ...
    if( doc==theStructures || doc==theFragments || doc== theSpectra ||
        doc==thePeakList || doc==theAnnotatedPeakList || doc==theNotes ||
        theAnnotationReports.contains(doc) )
        return this;
    ...
    if( e.getSource()==theSpectra ) 
        is_msms = (theSpectra.getNoScans()>0) ?(theSpectra.getScanDataAt(0).getMSLevel()>1) :false;    
    }
    ...
    static public Scan fromXML(GlycanWorkspace ws, Node scan_node) throws Exception {
    ...
    Vector<Node> ard_nodes = XMLUtils.findAllChildren(scan_node, "AnnotationReportDocument");
    ...
    protected SAXUtils.ObjectTreeHandler getHandler(String namespaceURI, String localName, String qName) {
        ...
        return new GlycanDocument.SAXHandler(new GlycanDocument(theWorkspace),false);
        ...
        return new SpectraDocument.SAXHandler(new SpectraDocument(),false);
    ...
    protected void initContent(String namespaceURI, String localName, String qName, Attributes atts) throws SAXException{ 
        ...
        ret.precursor_mz = doubleAttribute(atts,"precursor_mz",ret.precursor_mz);
    ...
    protected Object finalizeContent(String namespaceURI, String localName, String qName) throws SAXException{
        ...
        ret.theStructures = (GlycanDocument)getSubObject(GlycanDocument.SAXHandler.getNodeElementName(),ret.theStructures);
        ret.theFragments = (FragmentDocument)getSubObject(FragmentDocument.SAXHandler.getNodeElementName(),ret.theFragments);
        ret.theSpectra = (SpectraDocument)getSubObject(SpectraDocument.SAXHandler.getNodeElementName(),ret.theSpectra);
        ret.thePeakList = (PeakList)getSubObject(PeakList.SAXHandler.getNodeElementName(),ret.thePeakList);
        ret.theAnnotatedPeakList = (AnnotatedPeakList)getSubObject(AnnotatedPeakList.SAXHandler.getNodeElementName(),ret.theAnnotatedPeakList);
        ret.theNotes = (NotesDocument)getSubObject(NotesDocument.SAXHandler.getNodeElementName(),ret.theNotes);

        for( Object o : getSubObjects(AnnotationReportDocument.SAXHandler.getNodeElementName()) )
