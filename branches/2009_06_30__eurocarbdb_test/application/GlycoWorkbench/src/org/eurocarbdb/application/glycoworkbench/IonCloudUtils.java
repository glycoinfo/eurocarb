...
public class IonCloudUtils {
    ...
    static public Vector<IonCloud> getPossibleIonClouds(AnnotationOptions ann_opt) {
    ...
    if( avail_ions.size()>0 ) {
        ...
        enumerateCombinations(ret,new IonCloud(),avail_ions,avail_ions.getIons(),0,i,ann_opt.NEGATIVE_MODE);    
    }
    return ret;
    }

    static public Vector<IonCloud> getPossibleNeutralExchanges(int max_no_ex, AnnotationOptions ann_opt) {
    Vector<IonCloud> ret = new Vector<IonCloud>();
    ...
    if( avail_ions.size()>0 ) {
        ...
        enumerateCombinations(ret,new IonCloud().and(MassOptions.ION_H,-i),avail_ions,avail_ions.getIons(),0,i,false);
    }
    ...
    static public void enumerateCombinations(Vector<IonCloud> buffer, IonCloud combination, IonCloud limits, Vector<String> collection, int ind, int remaining, boolean negative) {
    ...
    if( ind==collection.size()-1 ) {
        // add all
        if( remaining<=limits.get(ion) ) 
        buffer.add(combination.and(ion,negative ?-remaining :remaining));
        return;
    }
    */
    ...
    for( int i=0; i<=max; i++ )
        enumerateCombinations(buffer,combination.and(ion,negative ?-i :i),limits,collection,ind+1,remaining-i,negative);        
    }
