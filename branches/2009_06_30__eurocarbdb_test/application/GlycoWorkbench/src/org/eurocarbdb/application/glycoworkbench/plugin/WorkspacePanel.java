...
public class WorkspacePanel extends JPanel implements TreeModel, ActionListener, TreeSelectionListener, BaseDocument.DocumentChangeListener, GlycanWorkspace.Listener, MouseListener {    
    ...
    public Component getTreeCellRendererComponent(JTree tree, Object value, boolean sel, boolean expanded, boolean leaf, int row, boolean hasFocus) {
        ...
        if( doc.wasSaved() ) 
            text += " - " + FileHistory.getAbbreviatedName(doc.getFileName());
        if( doc.hasChanged() )
            text += "*";
        setText(text);
        ...
        String text = (scan.getName()!=null && scan.getName().length()>0) ?scan.getName() :"Scan";
        if( scan.getPrecursorMZ()!=null )
            text += " [precursor m/z= "  +  new DecimalFormat("0.0000").format(scan.getPrecursorMZ().doubleValue()) + " Da]";
        setText(text);
    ...
    // file
    theActionManager.add("new",FileUtils.getIcon("new"),"Clear selected document",KeyEvent.VK_N, "",this);
    theActionManager.add("open",FileUtils.getIcon("open"),"Open selected document",KeyEvent.VK_O, "",this);
    theActionManager.add("save",FileUtils.getIcon("save"),"Save selected document",KeyEvent.VK_S, "",this);
    theActionManager.add("saveas",FileUtils.getIcon("saveas"),"Save selected document as...",KeyEvent.VK_A, "",this);

    theActionManager.add("newall",FileUtils.getIcon("newall"),"Clear the workspace",-1, "",this);
    theActionManager.add("openall",FileUtils.getIcon("openall"),"Open a workspace",-1, "",this);
    theActionManager.add("saveall",FileUtils.getIcon("saveall"),"Save the workspace",-1, "",this);
    theActionManager.add("saveallas",FileUtils.getIcon("saveallas"),"Save the workspace as...",-1, "",this);

    // edit
    //theActionManager.add("undo",FileUtils.getIcon("undo"),"Undo",KeyEvent.VK_U, "",this);
    //theActionManager.add("redo",FileUtils.getIcon("redo"),"Redo", KeyEvent.VK_R, "",this);

    theActionManager.add("add",FileUtils.getIcon("add"),"Add scan",KeyEvent.VK_D, "",this);
    theActionManager.add("delete",FileUtils.getIcon("delete"),"Delete selected document",KeyEvent.VK_E, "",this);
    theActionManager.add("select",FileUtils.getIcon("select"),"Activate current selection",KeyEvent.VK_L, "",this);

    theActionManager.add("properties",FileUtils.getIcon("properties"),"Change properties of selected scan",KeyEvent.VK_P, "",this);
    }

    private void updateActions() {
    
    theActionManager.get("new").setEnabled(hasSelectedDocument() && !hasSelectedAnnotationReport());
    theActionManager.get("open").setEnabled(hasSelectedDocument());
    theActionManager.get("save").setEnabled(hasSelectedDocument() && !hasSelectedSpectra());
    theActionManager.get("saveas").setEnabled(hasSelectedDocument() && !hasSelectedSpectra());
    ...
    theActionManager.get("delete").setEnabled(hasSelectedScan() || hasSelectedAnnotationReport());    
    ...
    theTree.setSelectionPath(theTree.getClosestPathForLocation(e.getX(),e.getY()));
    ...
    if( sel!=null ) {
        ...
        return (Scan)node;
        if( (node instanceof GlycanDocument) || (node instanceof FragmentDocument) || (node instanceof SpectraDocument) ||
        (node instanceof PeakList) || (node instanceof AnnotatedPeakList) 
        || (node instanceof NotesDocument) || (node instanceof AnnotationReportDocument) ) 
    ...
    /*
    if( theWorkspace.hasChanged() && !theWorkspace.isEmpty() ) {
        int ret = JOptionPane.showConfirmDialog(this,"Save changes to peak list?", null, JOptionPane.YES_NO_CANCEL_OPTION, JOptionPane.QUESTION_MESSAGE);
        if( ret == JOptionPane.CANCEL_OPTION ) 
        return false;        
        if( ret == JOptionPane.YES_OPTION ) {
        if( !theApplication.onSaveAs(theWorkspace) )
            return false;
        }
    }    
    return true;
    */
    ...
    /*public void onUndo()  {
    try {
        theWorkspace.getUndoManager().undo();
    }
    catch(Exception e) {
        LogUtils.report(e);
    }    
    ...
    public void onRedo()  {    
    try {
        theWorkspace.getUndoManager().redo();
    }
    catch(Exception e) {
        LogUtils.report(e);
    }    
    ...
    if( hasSelectedScan() ) {
        ...
        if( s!=null && JOptionPane.showConfirmDialog(this,"Are you sure to remove the selected scan, its children, and all the data associated?")==JOptionPane.YES_OPTION ) 
        theWorkspace.removeScan(s.getParent(),s);    
    }
    else {
        AnnotationReportDocument ard = getSelectedAnnotationReport();
        if( ard!=null && JOptionPane.showConfirmDialog(this,"Are you sure to remove the selected annotation report?")==JOptionPane.YES_OPTION ) 
    ...
    try {    
        ...
        select(getSelectedObjectOrParentScan());
        ((ReportingPlugin)theApplication.getPluginManager().get("Reporting")).showAnnotationsReport((AnnotationReportDocument)sel,false);
        }
    }
    catch(Exception ex) {
        LogUtils.report(ex);
    ...
    Scan s = getSelectedScan();
    if( s!=null ) {
        /*String m_z = JOptionPane.showInputDialog(this, "Insert m/z value of the precursor ion", s.getPrecursorMZ()); 
        if( m_z!=null ) 
        theWorkspace.setPrecursorMZ(s,Double.valueOf(m_z));
        */
        new ScanPropertiesDialog(theApplication,s,theWorkspace).setVisible(true);
    }    
    }
    ...
    */    
    ...
    TreePath par_path = (parent==null) ?getTreePath(theWorkspace) :getTreePath(parent);
    for( TreeModelListener tml : tm_listeners ) 
        tml.treeNodesInserted(new TreeModelEvent(this,par_path,new int[] {index},new Object[] {child}));
    }

    public void fireTreeNodesRemoved(Object parent, Object child, int index) {
    TreePath par_path = (parent==null) ?getTreePath(theWorkspace) :getTreePath(parent);
    for( TreeModelListener tml : tm_listeners ) 
        tml.treeNodesRemoved(new TreeModelEvent(this,par_path,new int[] {index},new Object[] {child}));
    }

    public void fireTreeStructureChanged(Object node) {
    TreePath path = (node==null) ?getTreePath(theWorkspace) :getTreePath(node);
    for( TreeModelListener tml : tm_listeners ) 
