...
/**
   Stores a complete annotated list of peaks labeled from an MS or
   MS/MS spectrum. Each peak can have multiple annotations. The
   annotations can come from multiple structures, or from the same
   structure. A list of structures from where the annotations have
   been originated, as well as a list of labeled peaks are
   mantained. The annotations can be intact or fragment glycan
   molecules. In case of an MS spectrum there is only one (empty)
   structure from which the annotations are derived. The source
   structure can it-self be an intact molecule, in case of an MS/MS
   spectrum, or a fragment molecule, in case of an MSn spectrum.
...
public class AnnotatedPeakList extends BaseDocument implements SAXUtils.SAXWriter {    
    ...
    /**
       Interface that must be implemented by all objects that want to
       register to listen for changes in the annotations
     */
    public interface AnnotationChangeListener {
    
    /**
       Called when the number or identity of the source structures
       are changed
     */
    public void structuresChanged(AnnotationChangeEvent e);
    
    /**
       Called when the number or identity of the annotations are
       changed
     */
    ...
    /**
       Contains information about the event resulting from a change in
       the annotations
     */
    ...
    /**
       Constructor.
       @param _source the object that originated the event
     */
    ...
    */
    ...
    return FileUtils.getIcon("annpeaksdoc");
    }
    ...
    filters.add(new ExtensionFileFilter("msa", "Cartoonist annotated peaklist file"));
    filters.add(new ExtensionFileFilter("gwa", "GlycoWorkbench annotated peaklist file"));
    filters.add(new ExtensionFileFilter(new String[]{"gwa", "msa"}, "All annotated peaklist files"));

    return filters;
    }

    public javax.swing.filechooser.FileFilter getAllFileFormats() {
    return new ExtensionFileFilter(new String[]{"gwa", "msa"}, "Annotated peaklist files");
    }
    ...
    // update peak annotation singles
    peak_annotations_single.insertElementAt(new PeakAnnotationCollection(),s_ind);
    ...
    /**
       Return the index of the specified structure in the list of
       structures from which the annotations are originated, or -1 if
       the structure cannot be found
     */
    ...
    /**
       Return the index of the peak object in the list of labeled
       peaks that have been annotated
    ...
    /**
       Return the index of peak with the specified mass/charge value
       in the list of labeled peaks that have been annotated
       @see PeakAnnotationMultiple
     */
    ...
    /**
       Add a collection of annotations deriving from a specified
       structure. If the structure is not existing it is first added
       to the list
       @param structure the molecule from which the annotations have
       been derived
       @param pac the annotations collection
       @param merge <code>true</code> if the annotations should be
    ...
    /**
       Add a single annotation deriving from a specified structure. If
       the structure is not existing it is first added to the list
       @param structure the molecule from which the annotations have
       been derived
       @param pa the annotation
       @param merge <code>true</code> if the annotation should be merged
    ...
    /**
       Add a collection of annotations deriving from a specified
       structure. The originating structure is added at the specified
       position in the list
       @param structure the molecule from which the annotations have
       been derived
       @param pac the annotations collection
       @param s_ind the index of the structure in the list
       @return <code>true</code> if the operation was successfull
    ...
    int ret = peak_annotations_single.elementAt(s_ind).removePeakAnnotation(pa,true);
    if( ret==-1 )
        return -1;
    ...
    /**
       ...
       @param s_ind the index of the originating structure
       @param pa the annotation to remove
       @return the index of the removed annotation or <code>-1</code>
    ...
    */
    ...
    /**
       ...
       @param s_inds the index of the originating structure
    */
    ...
    /**
       Clear all annotations for a specified peak. Leave the peak in
       the list
     */
    ...
    /**
       Clear all annotations for set of peaks. Leave the peaks in
       the list
     */
    ...
    /**
       Update the annotations for all the peaks in a range of
       mass/charge values.
       @param s_ind the index of the structure
       @param start_mz the minimum mass/charge value of the peaks to
       which the annotations correspond
       @param end_mz the maximum mass/charge value of the peaks to
       which the annotations correspond
       @param toupdate the new annotations
       @param merge <code>true</code> if the old annotations should be kept
     */
    ...
    boolean removed = false;
    if( !merge ) {
        Vector<PeakAnnotation> peak_annotations = new Vector<PeakAnnotation>(pac.getPeakAnnotations()); // avoid concurrent modifications
        for( PeakAnnotation pa : peak_annotations ) {
        if( start_mz==end_mz || pa.getPeak().getMZ()>=start_mz && pa.getPeak().getMZ()<=end_mz ) {
    ...
    for(int i=0; i<peak_annotations_multiple.size(); i++ ) {
        ...
        if( assertive ) {
            // insert here
            PeakAnnotationMultiple toadd = new PeakAnnotationMultiple(p,structures.size());        
    ...
    /**
       Return a new annotated peak list containing only the
       annotations with the best coverage
       @param max_num the number of annotations to return
       @see #getCoverage
     */
    ...
    AnnotatedPeakList ret = new AnnotatedPeakList();
     for( int i=0; i<max_num && i<coverages.size(); i++) {
        ...
        ret.addPeakAnnotationsPVT(structures.elementAt(ind),peak_annotations_single.elementAt(ind),false);
    }       
    ...
    /**
       Return a new annotated peak list containing only the
       annotations originating from the specified structures
       @param s_indexes the indexes of the structures
     */
    ...
    for( int i=0; i<s_indexes.length; i++ ) {
        ...
        PeakAnnotationCollection pac = peak_annotations_single.elementAt(s_indexes[i]);
    ...
    /**
       ...
       @param s_ind the index of the structure
       @param pac_indexes the indexes of the annotations
    ...
    /**
       Return a new annotated peak list containing only the
       annotations for the specified peaks
       @param pam_indexes the indexes of the peaks
       @see PeakAnnotationMultiple
     */
    ...
    for( int i=0; i<structures.size(); i++ ) {
        ...
        PeakAnnotationMultiple pam = peak_annotations_multiple.elementAt(pam_indexes[l]);
    ...
    /**
       Copy the content of another annotated peak list on the current
       one
     */
    ...
    if( src!=null ) {
        ...
        addPeakAnnotationsPVT(src.structures.elementAt(i),src.peak_annotations_single.elementAt(i),false);
    ...
    /**
       Merge the content of another annotated peak list with the
       current one
     */
    ...
    if( src!=null ) {     
        // copy peak annotations
        for( int i=0; i<src.structures.size(); i++ )
        addPeakAnnotationsPVT(src.structures.elementAt(i),src.peak_annotations_single.elementAt(i),true);
    ...
    max_intensity = 0.;
    for( PeakAnnotationMultiple pam : peak_annotations_multiple )
        max_intensity = Math.max(max_intensity,pam.getPeak().getIntensity());
    }
    ...
    /**
       Return the list of labeled peaks as a 2xN table of mass/charge
    ...
    /**
       ...
       @param p_ind the index of the peak
     */
    ...
    /**
       Return the intensity of a specific peak normalize by the
       maximum intensity in the list of labeled peaks
       @param p_ind the index of the peak
     */
    ...
    /**
       Return the index of the peak whose mass/charge values is
       nearest to the specified value, or -1 if the annotated peak
       list is empty
     */
    ...
    /**
       Return <code>true</code> if the peak at the specified position
       is annotated
     */
    ...
    /**
       Return all annotations for all peaks and from all structures.
       Each PeakAnnotationMultiple correspond to one peak
    ...
    return peak_annotations_multiple.elementAt(indexOf(p));
    }
    ...
    /**
       Return all annotations for a specified peak and from a specific
       structure
     */
    public Vector<Annotation> getAnnotations(Peak p, int s_ind) {
    return peak_annotations_multiple.elementAt(indexOf(p)).getAnnotations(s_ind);
    }
    ...
    /**
       Return the structure of all annotations for a specified peak
       and from a specific structure
     */
    public Vector<Glycan> getFragments(int p_ind, int s_ind) {
    Vector<Glycan> ret = new Vector<Glycan>();
    for( Annotation a : peak_annotations_multiple.elementAt(p_ind).getAnnotations(s_ind) ) 
    ...
    /**
       Return all annotations for all peaks and from all structures
       Each {@link PeakAnnotationCollection} correspond to one
       structure
     */
    ...
    /**
       Return all annotations for all peaks and from a specific
       structure.
     */
    ...
    /**
       Return all annotations for all peaks and from a specific
       structure. Return <code>null</code> if the structure is not
       found
     */
    ...
    /**
       Return the sum of the intensities of the annotated peaks as a
    ...
    /**
       Return the RMSD between the mass/charge values of the peaks and
       the mass/charge values of their annotations
       @param s_ind the index of the structure
     */
    ...
    /**
       Return the RMSD between the mass/charge values of the peaks and
       the mass/charge values of their annotations in PPM
       @param s_ind the index of the structure
     */
    ...
    /**
       Return the number of peaks with intensity greater than a
       certain value
       @param min_rel_int the minimum value of intensity
     */
    ...
    /**
       Return the number of peaks with annotations derived from a
       specific structure
       @param s_ind the index of the structure
    */
    ...
    /**
       Return the number of peaks with annotations derived from a
       specific structure and intensity greater than a certain value
       @param s_ind the index of the structure
       @param min_rel_int the minimum value of intensity
    */
    ...
    /**
       Return the range of accuracies for all the annotations from the
       specific structure
     */
    ...
    /**
       Return the minimum accuracy for all the annotations from the
       specific structure
     */
    ...
    /**
       Return the maximum accuracy for all the annotations from the
       specific structure
     */
    ...
    /**
       Return a 2xN table of mass/charge and accuracy values for all
       annotated peaks and a specific structures. These values can be
       use to check the calibration of the spectra
     */
    ...
    /**
       Return a 2xN table of mass/charge and accuracy values for all
       annotated peaks and a specific structures. Only the most
       accurate annotation for each peak is used. These values can be
       use to check the calibration of the spectra
     */
    ...
    /**
       Return a 2xN table of mass/charge and accuracy values in PPM
       for all annotated peaks and a specific structures. Only the
       most accurate annotation for each peak is used. These values
       can be use to check the calibration of the spectra
     */
    ...
    /**
       Notify the document of a change in the intensity for a specific
       peak
     */    
    ...
    /**
       Notify the document of a change in the intensity for a list of
       peaks
     */    
    ...
    /**
       Register a listener to receive events when the annotations are
       changed
     */
    ...
    /**
       De-register one of the listeners that was receiving events when
    ...
    /**
       Notify all listeners of a change in the number or identity of
       the structures
     */
    ...
    Element apl_node = toXML(document);
    if( apl_node == null )
        return "";

    document.appendChild(apl_node);    
    ...
    */
    ...
    // read annotations
    Vector<Node> ann_nodes = XMLUtils.findAllChildren(apl_node, "Annotations");
    for( Node ann_node : ann_nodes) {
        ...
        Node pac_node = XMLUtils.assertChild(ann_node,"PeakAnnotationCollection");
    ...
    */
    ...
    for( int i=0; i<structures.size(); i++ ) {
        ...
        Element pac_node = peak_annotations_single.elementAt(i).toXML(document);
        if( pac_node==null )
        continue;
    ...
    /**
       Default SAX handler to read a representation of all annotations
       of a single structure from an XML stream.
     */    
    ...
    protected Object finalizeContent(String namespaceURI, String localName, String qName) throws SAXException{
        Glycan g = (Glycan)getSubObject(Glycan.SAXHandler.getNodeElementName(),true);
        PeakAnnotationCollection pac = (PeakAnnotationCollection)getSubObject(PeakAnnotationCollection.SAXHandler.getNodeElementName(),true);
    ...
    protected Object finalizeContent(String namespaceURI, String localName, String qName) throws SAXException{
        ...
        // read data
        for( Object o : getSubObjects(AnnotationsSAXHandler.getNodeElementName()) ) {
        Pair<Glycan,PeakAnnotationCollection> p = (Pair<Glycan,PeakAnnotationCollection>)o;
        theDocument.addPeakAnnotationsPVT(p.getFirst(),p.getSecond(),true);
        }
    ...
    while( (line = br.readLine())!=null ) {
        ...
        MassOptions mopt = new MassOptions(MassOptions.PERMETHYLATED,"freeEnd"); //!!! o-glycans        
    ...
    for( int i=0; i<tokens.length; i++ ) {
        // parse count;
        int c=0;
        for(; c<tokens[i].length() && Character.isDigit(tokens[i].charAt(c)); c++ );
