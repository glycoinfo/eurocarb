...
*   for more details.
*
*   Last commit: $Rev: 1272 $ by $Author: glycoslave $ on $Date:: 2009-06-26 #$  
*/
...
*
*   Utility methods to support parsing carbohydrates in GlycoCT 
*   sequence format.
*
*   @see ResidueToken
*   @see LinkageToken
...
public abstract class GlycoctParserAdaptor extends ParserAdaptor
{
    ...
    /!*  enum Modification  *!//*!**************************************
    *
    *   Enumeration of valid modifications to bast-type monosaccharides
    *   for the GlycoCT format.
    *!/
    ...
    */
    ...
    throws SequenceFormatException
    {
        ...
        //  residue token *must* be a MonosacResidueToken
        ...
        {
            SequenceFormatException sfex = createSyntaxException( term1, ex.getMessage() );
            sfex.initCause( ex ); 
            throw sfex;
        }
    }
    ...
    */  
    ...
    private final void inlineRepeats( Graph<LinkageToken,ResidueToken> ast )
    {
        ...
        for ( RepeatResidueToken repeat_residue : repeats )
        {
            ...
            //  there should only be 1 incoming edge & 1 outgoing edge into
            //  a virtual repeat residue
            assert v.countAttachedEdges() == 2;
            
            Edge<LinkageToken,ResidueToken> incoming_edge, outgoing_edge;
            List<Edge<LinkageToken,ResidueToken>> elist = new ArrayList( v.getAttachedEdges());
        
            if ( elist.get(0).getParent() == v )
            {
                //  first element is the outgoing edge
                outgoing_edge = elist.get(0);
                incoming_edge = elist.get(1);
            }
            else
            {
                //  first element is the incoming edge
                incoming_edge = elist.get(0);
                outgoing_edge = elist.get(1);
            }

            assert incoming_edge.getChild() == outgoing_edge.getParent();
