...
/**
   Utility class containing functions to facilitate the access to
   files, both generic and application specific.
...
public class FileUtils {
    ...
    /**
       Convert the escape codes in a URI-like string into the original
       unicode character. Return the result.
     */    
    ...
    /**
       Load an icon from the application Jar file.
       @param id the identifier of the icon (without the extension)
     */
    static public ImageIcon getIcon(String id) {
    String path = "/icons/" + id + ".gif";
    URL imgURL = FileUtils.class.getResource(path);
    if (imgURL != null) 
        return new ImageIcon(imgURL);    
    return new ImageIcon(new BufferedImage(GraphicOptions.ICON_SIZE_SMALL,GraphicOptions.ICON_SIZE_SMALL,BufferedImage.TYPE_INT_ARGB));
    }   

    /**
       Load an icon from the Jar file containing the specified class.
       @param id the identifier of the icon (without the extension)
     */
    static public ImageIcon getIcon(Class owner, String id) {
    String path = "/icons/" + id + ".gif";
    URL imgURL = owner.getResource(path);
    if (imgURL != null) 
        return new ImageIcon(imgURL);    
    return new ImageIcon(new BufferedImage(GraphicOptions.ICON_SIZE_SMALL,GraphicOptions.ICON_SIZE_SMALL,BufferedImage.TYPE_INT_ARGB));
    }   
    ...
    /**
       Load an icon from the application Jar file.
       @param id the identifier of the icon (without the extension)
       @param ext the icon file extension
     */
    static public ImageIcon getIcon(String id, String ext) {
    ...
    /**
       ...
       @param id the identifier of the cursor (without the extension)
     */
    ...
    /**
       Make sure that a path to a file contain the specified
       extension. If not the extension is added and the resulting path
       is returned. If a different extension is present, it is first
       removed before adding the new one.
       @param filename the original path
    ...
    /**
       Open a stream to a desired resource irregarding if it's a
       regular file or if it's contained in a Jar file.
       @param owner the class requesting the resource, used to find
       the correspding Jar file
       @param filename the identifier of the desired resource
       @throws Exception if the resource is not found
     */
    static public InputStream open(Class owner, String filename) throws Exception {
    ...
    /**
       ...
       the correspding Jar file
       @param filename the identifier of the desired resource
       @param in_jar <code>true</code> if the resource should be
       looked in the jar file only, <code>false</code> if it should be
       looked in the file system only
       @throws Exception if the resource is not found
     */
    static public InputStream open(Class owner, String filename, boolean in_jar) throws Exception {
