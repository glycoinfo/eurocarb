...
int SP_LoadExp()
{
  ...
  float shift1, shift2;
  int i, j, k, follows, addnew, nDims, cnt;
  unsigned int mode;
      ...
      for(i=0;i<cnt;i++)
	{
	  switch(mode)
	    ...
	    case SP_ID_2D_CH:
	      ...
	      break;
	    case SP_ID_2D_HH:
	      spectrum->HhHiShiftAtom[i].unit=0;
	      spectrum->HhHiShiftAtom[i].atom[0]=-1;
	      spectrum->HhHiShiftAtom[i].atom[1]=-1;
	      spectrum->HhLoShiftAtom[i].unit=0;
	      spectrum->HhLoShiftAtom[i].atom[0]=-1;
	      spectrum->HhLoShiftAtom[i].atom[1]=-1;
	      break;
	      
	    }
	}
      /* If there are chemical shifts set in the experimental structure add the assignments to the list of
	 shifts */
      for(unit=(struct BU_Unit *)spectrum->Units.Head.Succ; unit->Node.Succ!=NULL;
	  unit=(struct BU_Unit *)unit->Node.Succ)
	{
	  for(j=0; j<unit->Shifts.Type->HeavyCnt; j++)
	    {
	      for(k=0; k<unit->Shifts.Type->Atom[j].HCnt; k++)
		{
		  if(unit->Shifts.C[j]>0 || unit->Shifts.H[j][k]>0)
		    {
		      shiftToAtom.unit=unit;
		      shiftToAtom.atom[0]=j;
		      shiftToAtom.atom[1]=k;
		      SP_AssignOtherShiftsToAtom(spectrum, unit->Shifts.C[j], unit->Shifts.H[j][k], &shiftToAtom, 0, 0);
		    }
		}
	      if(unit->Shifts.C[j]>0 && unit->Shifts.Type->Atom[j].HCnt==0)
		{
		  shiftToAtom.unit=unit;
		  shiftToAtom.atom[0]=j;
		  shiftToAtom.atom[1]=-1;
		  SP_AssignOtherShiftsToAtom(spectrum, unit->Shifts.C[j], unit->Shifts.H[j][k], &shiftToAtom, 0, 0);
		}
	    }
	}
  ...
  while (sim && exp)
    {
      ...
      /* After all chemical shifts are loaded they need to be assigned to the right atoms. The reason why this is
	 done here and not in CC_LoadAssignments is that the shifts first need to be sorted to find them properly. */
      for(unit=(struct BU_Unit *)exp->Units.Head.Succ; unit->Node.Succ!=NULL;
	  unit=(struct BU_Unit *)unit->Node.Succ)
	{
	  for(j=0; j<unit->Shifts.Type->HeavyCnt; j++)
	    {
		  ...
		  if(unit->Shifts.C[j]>0 || unit->Shifts.H[j][k]>0)
		    {
		      ...
		      shiftToAtom.atom[0]=j;
		      shiftToAtom.atom[1]=k;
		      SP_AssignOtherShiftsToAtom(exp, unit->Shifts.C[j], unit->Shifts.H[j][k], &shiftToAtom, 0, 0);
		    }
		}
	      if(unit->Shifts.Type->Atom[j].HCnt==0)
		{
		  shiftToAtom.unit=unit;
		  shiftToAtom.atom[0]=j;
		  shiftToAtom.atom[1]=-1;
...
int SP_MigrateSimulatedAssignments()
{
  ...
  while (sim && exp)
	    ...
	    sUnit=(struct BU_Unit *)sUnit->Node.Succ)
	{
      ...
      SP_CalcChSpectra(exp);
      SP_CalcHhSpectra(exp);
      SP_Sort2DSpectra(exp);
...
void SP_CalcChSpectra(struct BU_Struct *spectrum)
{
  struct BU_Unit *unit;
  struct TY_Type *type;
...
void SP_CalcHhSpectra(struct BU_Struct *spectrum)
{
  struct BU_Unit *unit;
  struct TY_Type *type;
  unsigned int i,k,l,m;
		 ...
		 const unsigned int mode)
{
  ...
  switch(mode)
      ...
      else
	{
      ...
      return(sqrt(SP_C_SCALING*dx*dx+dy*dy));
    case SE_PURGE_HH:
      if(spec2)
	{
...
/* Correct spectrum for errors in reference. */
/* When used with multiflag it will try to correct simulated spectra instead. This makes more sense
   when e.g. curating already assigned projects. */
/* TODO: Correct simulated spectra the default in CASPER. */
/* SYNTAX: Correct <str> <c-off> [<h-off>] */
int SP_Correct()
{
  struct BU_Struct *str;
  struct BU_Unit *unit;
  ApiString name;
  char strName[NODE_NAME_LENGTH], prefix[NODE_NAME_LENGTH];
  float dc, dh;
  ...
  if(PA_Result.String[0]=='/')
	...
	{
	  /* When using multicorrect correct the simulated spectra instead of the experimental spectra.
	     Makes more sense when e.g. curating an already assigned project. */
	  strcpy(prefix, "sim");
	}
      /* Find the first structure that starts with "sim" - removed */
      /*      else
	{
      */
  ...
  if(multiFlag)
    {
      sprintf(strName, "%s%d", prefix, j++);
    }
  else
    {
      strcpy(strName, prefix);
    }
  str=(struct BU_Struct *)FindNode((struct Node *)&StructList, strName);
  if (str==NULL)
    {
      Error(PA_ERR_FAIL,"No such spectrum");
    }
  ...
  while(str)
    {
      for (i=0; i<str->CCnt; i++)
	str->CShift[i]-=dc;
      for (i=0; i<str->HCnt; i++)
	str->HShift[i]-=dh;
      for (i=0; i<str->ChCnt; i++)
	{
	  str->ChCShift[i]-=dc;
	  str->ChHShift[i]-=dh;
	}
      for (i=0; i<str->HhCnt; i++)
	{
	  str->HhLoShift[i]-=dh;
	  str->HhHiShift[i]-=dh;
	}
...
int SP_CalculateCorrections(struct BU_Struct *sim, struct BU_Struct *exp)
{
  ...
  for(eUnit=(struct BU_Unit *)exp->Units.Head.Succ,
      ...
      for(i=0;i<eUnit->Shifts.Type->HeavyCnt;i++)
	{
	  if(eUnit->Shifts.C[i]>0.1)
	    {
	      cError+=sUnit->Shifts.C[i]-eUnit->Shifts.C[i];
	      cCnt++;
	    }
	  for(j=0; j<2; j++)
	    {
		...
		{
		  if(sUnit->Shifts.Type->Atom[i].HCnt==2)
		    {
		      if(fabs(eUnit->Shifts.H[i][j]-sUnit->Shifts.H[i][j])>
			 fabs(eUnit->Shifts.H[i][j]-sUnit->Shifts.H[i][(j+1)%2]))
			{
			  hError+=sUnit->Shifts.H[i][(j+1)%2]-eUnit->Shifts.H[i][j];			  
			  hCnt++;
			  continue;
			}
		    }
		  hError+=sUnit->Shifts.H[i][j]-eUnit->Shifts.H[i][j];
		  hCnt++;
		}
	    }
	}
  ...
  if(cCnt>0)
    {
      sim->CSysErr=cError/cCnt;
    }
  if(hCnt>0)
    {
      sim->HSysErr=hError/hCnt;
    }
...
{
  int *usedsim=0, *usedexp=0, **globalSimAss=0, **globalExpAss=0;
  float cutoff;
  float *sys_err=0, abs_err=0, diff=0;
  float rms_err=0;
  unsigned int i, eCnt, sCnt;
  int localNUnassigned=0;
  struct VA_Variable *noMulti;

  usedexp=0;
  usedsim=0;

  noMulti=(struct VA_Variable *)FindNode(&(VariableList.Head), "nomultiassign");
  ...
  globalSimAss = (int **) malloc(sCnt*sizeof(int *));
  globalSimAss[0] = (int *) malloc(sCnt * 2 * sizeof(int));
  ...
  /* Allocate a continuous 2D array (sCnt rows and 2 columns) */
  globalExpAss = (int **) malloc(eCnt*sizeof(int *));
  globalExpAss[0] = (int *) malloc(eCnt * 2 * sizeof(int));
  ...
  switch(mode)
    {
    case SE_PURGE_C:
      /*      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 140, 200, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 90, 140, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 25, 90, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 10, 25, mode);*/
      /*SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 0, 200, mode);*/

      SP_AssExpSimfind(exp, sim, mode, usedsim, 0, usedexp, 0, globalSimAss, globalExpAss);
      break;
    case SE_PURGE_H:
      /*      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 4.5, 7, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 2.5, 4.5, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 0, 2.5, mode);*/
      /*      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 0, 7, mode);*/
    ...
    case SE_PURGE_CH:
      ...
      /*      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 90, 110, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 25, 90, mode);
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 10, 25, mode);*/
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 0, 110, mode);
      break;
    case SE_PURGE_HH:
      SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 0, 7, mode);
    }
  ...
  SP_CalcErrs(exp, sim, eCnt, usedexp, &diff, &abs_err, &rms_err, sys_err, mode);
      ...
      if(noMulti == NULL || noMulti->Value.Value.Float<=0)
	{
	  SP_MultiExpAssignments(exp, sim, usedsim, &diff, &abs_err, &rms_err, &localNUnassigned, mode);
	}
...
/* REPLACED BY  SP_AssExpSimRegion() for 2D*/
...
float SP_AssExpSimfind(const struct BU_Struct *exp, struct BU_Struct *sim,
		       ...
		       unsigned int exp_ptr, int **globalSimAss, int **globalExpAss)
{
  int j;
  unsigned int eCnt, sCnt, temp_exp_ptr;
  int exp_rem, sim_rem, simSkipCounter;
  float diff, altDiff, tempDiff;
  ...
  while(exp_ptr<eCnt && usedexp[exp_ptr]!=-1)
    {
      exp_ptr++;
    }
  while(sim_ptr<sCnt && usedsim[sim_ptr]!=-1)
    {
      sim_ptr++;
    }
  ...
  if(exp_rem==sim_rem)
      ...
      for(;exp_ptr<eCnt;exp_ptr++,sim_ptr++)
	{
	  ...
	  if(assignFlag && SP_AssignMode!=SP_AM_CUTOFF)
	    {
		 ...
		 (mode==SE_PURGE_H && !(matched&4) && matched&8))
		{
		  ...
		  temp_exp_ptr=SP_FindExpAssignedToSim(exp, sim, sim_ptr, mode);
		  if(temp_exp_ptr>0 && usedexp[temp_exp_ptr]==-1)
		      ...
		      for(;j--;)
			{
		      ...
		      diff=SP_AssExpSimfind(exp,sim,mode,usedsim,sim_ptr+1-simSkipCounter,usedexp,
					    exp_ptr+1, globalSimAss, globalExpAss)+diff;
		      ...
		      altDiff=SP_AssExpSimfind(exp,sim,mode,temp_usedsim,
					       ...
					       exp_ptr, globalSimAss, globalExpAss)+altDiff;
		      if(altDiff<diff)
			{
			  /* Reverse loop */
			  for(j=sCnt-1;j>eCnt;j--)
  ...
  temp_usedexp=0;
  temp_usedsim=0;
  if(globalExpAss[exp_ptr][0]>(int)sim_ptr)
    {
      usedsim[globalExpAss[exp_ptr][0]]=exp_ptr;
      usedexp[exp_ptr]=globalExpAss[exp_ptr][0];
      diff=SP_AssExpSimfind(exp,sim,mode,usedsim,globalExpAss[exp_ptr][0]+1,usedexp,
			    exp_ptr+1, globalSimAss, globalExpAss) + fabs(eShift[exp_ptr] - sShift[globalExpAss[exp_ptr][0]]);
      return(diff);
    }

  if(globalExpAss[exp_ptr][0]==sim_ptr && globalExpAss[exp_ptr][1]==sim_ptr+1)
    {
      tempDiff=99999;
    }
  else if(globalExpAss[exp_ptr][0]==sim_ptr+1 && globalExpAss[exp_ptr][1]==sim_ptr)
    {
      diff=99999;
    }
  else
    {
      diff=fabs(eShift[exp_ptr] - sShift[sim_ptr]);
      tempDiff=fabs(eShift[exp_ptr] - sShift[sim_ptr+1]);
    }
  ...
  */
  if(tempDiff<=diff/* && tempDiff<cutoff*10*/)
    {
  ...
  if(temp_usedexp && temp_usedsim && globalExpAss[exp_ptr][0]<=(int)sim_ptr &&
     (globalSimAss[sim_ptr+1][0]==-1 || globalSimAss[sim_ptr+1][0]==exp_ptr))
    {
      altDiff=SP_AssExpSimfind(exp,sim,mode,temp_usedsim,sim_ptr+1,
			       temp_usedexp,exp_ptr, globalSimAss, globalExpAss);
    }
  else
    {
      altDiff=99999;
  ...
  if(diff<tempDiff*10 || diff<cutoff*10 || !temp_usedexp)
    {
  ...
  if(altDiff<diff)
      ...
      for(;j<sCnt;j++)
	{
      ...
      if(globalExpAss[exp_ptr][0]<=(int)sim_ptr)
	{
	  globalExpAss[exp_ptr][0]=sim_ptr+1;
	  globalSimAss[sim_ptr+1][0]=exp_ptr;
	}
    }
  /*  else if(globalExpAss[exp_ptr][0]==-1 || globalExpAss[exp_ptr][0]>sim_ptr)
    {
...
/* This function finds shifts in the specified region (it is possible to assign one region at a time) and then
   called SP_AssExpSimRegionFind, which matches the chemical shifts, using a modified version of the Gale-Shapley
   algorithm. */
void SP_AssExpSimRegion(struct BU_Struct *exp, struct BU_Struct *sim,
			int *usedexp, int *usedsim, float min, float max,
			const unsigned int mode)
{
  int i, eCnt, sCnt;
  int firstExp=-1, lastExp=-1, firstSim=-1, lastSim=-1;
  const float *eCShift, *eHShift, *sCShift, *sHShift;
  ...
  switch(mode)
    ...
    case SE_PURGE_HH:
      ...
      sCShift=sim->HhHiShift;
      sHShift=sim->HhLoShift;
      break;
  ...
  /* Find the first and last signal in the experimental list of chemical shifts. */
  for(i=0; i<eCnt && lastExp==-1; ++i)
    {
      if(mode==SE_PURGE_H)
	{
	  if(firstExp==-1)
	    {
	      if(eHShift[i]<max && eHShift[i]>=min)
		{
		  firstExp=i;
		}
	    }
      ...
      else
	{
	  if(firstExp==-1)
	    {
	      if(eCShift[i]<max && eCShift[i]>=min)
		{
		  firstExp=i;
		}
	    }
  ...
  /* Find the first and last signal in the simulated list of chemical shifts. */
  for(i=0; i<sCnt && lastSim==-1; ++i)
    {
      if(mode==SE_PURGE_H)
	{
	  if(firstSim==-1)
	    {
	      if(sHShift[i]<max && sHShift[i]>=min)
		{
		  firstSim=i;
		}
	    }
      ...
      else
	{
	  if(firstSim==-1)
	    {
	      if(sCShift[i]<max && sCShift[i]>=min)
		{
		  firstSim=i;
		}
	    }
  ...
  if(firstExp != -1 && lastExp != -1 && firstSim != -1 && lastSim != -1)
    {
      SP_AssExpSimRegionFind(exp, sim, usedexp, usedsim, firstExp, lastExp, firstSim, lastSim, mode);
    }
}

/* This function finds the best match of signals of a specific region. The algorithm is based on the Gale-Shapley
   algorithm to solve the Stable marriage problem, but with some small modifications to increase the chances of
   finding a "global" best match */
float SP_AssExpSimRegionFind(struct BU_Struct *exp, struct BU_Struct *sim,
			     int *usedexp, int *usedsim,
			     int firstExp, int lastExp,
			     int firstSim, int lastSim, const unsigned int mode)
{
  int i, j, exp_ptr, sim_ptr, eCnt, sCnt, matched, nSimRem, **expPref, **simPref, *expPrefCnt, *simPrefCnt, temp_ptr, *prefPos, prefListLen=10;
  const float *eCShift, *eHShift, *sCShift, *sHShift;
  float diff=0, loDiff, altDiff, combDiff, cutoff, tot_err=0, **expPrefScore, **simPrefScore;
  int *temp_usedsim, *temp_usedexp, *orig_usedsim, *orig_usedexp;

  eCnt=lastExp-firstExp+1;
  sCnt=lastSim-firstSim+1;

  if(eCnt<10 && sCnt<10)
    {
      prefListLen=Max(eCnt,sCnt);
    }
  else if(eCnt>35 || sCnt>35)
    {
      prefListLen=25;
    }
  else if(eCnt>25 || sCnt>25)
    {
      prefListLen=15;
    }
  ...
  /* Allocate memory for preference lists and preference scores */
  prefPos=malloc(eCnt*sizeof(int));
  expPrefCnt=malloc(eCnt*sizeof(int));
  simPrefCnt=malloc(sCnt*sizeof(int));
  expPref=(int **)malloc(eCnt*sizeof(int *));
  simPref=(int **)malloc(sCnt*sizeof(int *));
  expPrefScore=(float **)malloc(eCnt*sizeof(float *));
  simPrefScore=(float **)malloc(sCnt*sizeof(float *));

  for(i=0;i<eCnt;++i)
    {
      prefPos[i]=0;
      expPrefCnt[i]=0;
      expPref[i]=(int *)malloc(prefListLen*sizeof(int));
      expPrefScore[i]=(float *)malloc(prefListLen*sizeof(float));
    }
  for(i=0;i<sCnt;++i)
    {
      simPrefCnt[i]=0;
      simPref[i]=(int *)malloc(prefListLen*sizeof(int));
      simPrefScore[i]=(float *)malloc(prefListLen*sizeof(float));
    }

  /* Generate preference lists by iterating through all combinations of experimental and simulated shifts. */
  for(exp_ptr=firstExp; exp_ptr<=lastExp; ++exp_ptr)
    {
      if(usedexp[exp_ptr]!=-1)
	{
	  continue;
	}
      for(sim_ptr=firstSim;sim_ptr<=lastSim; ++sim_ptr)
	{
	  if(mode!=SE_PURGE_C && mode!=SE_PURGE_H)
	  ...
	  else if (mode==SE_PURGE_C)
	    {
	      diff=fabs(eCShift[exp_ptr]-sCShift[sim_ptr]);
	      matched=SP_MatchAssignment(exp,exp_ptr,sim,sim_ptr,mode);
	      if(!(matched&1) && matched&2)
		{
		  diff+=cutoff*SP_MISMATCH_PENALTY;
	    ...
	    {
	      diff=fabs(eHShift[exp_ptr]-sHShift[sim_ptr]);	      
	      matched=SP_MatchAssignment(exp,exp_ptr,sim,sim_ptr,mode);
	      if(!(matched&4) && matched&8)
		{
		  diff+=cutoff*SP_MISMATCH_PENALTY;
	  ...
	  /* Both experimental and simulated preferences can be added at the same time */
	  SP_InsertInPrefList(expPref[exp_ptr-firstExp], expPrefScore[exp_ptr-firstExp], &expPrefCnt[exp_ptr-firstExp], prefListLen, sim_ptr, diff);
	  SP_InsertInPrefList(simPref[sim_ptr-firstSim], simPrefScore[sim_ptr-firstSim], &simPrefCnt[sim_ptr-firstSim], prefListLen, exp_ptr, diff);
	}
    }

  /* Loop through all experimental positions */
  for(exp_ptr=0; exp_ptr<eCnt; ++exp_ptr)
    {
      if(usedexp[exp_ptr+firstExp]!=-1)
	{
	  continue;
	}
      matched=0;
      i=prefPos[exp_ptr];
      while(matched==0&&i<expPrefCnt[exp_ptr])
	{
	  if(mode==SE_PURGE_C || mode==SE_PURGE_H)
	    {
	      if(i<expPrefCnt[exp_ptr]-1 && expPref[exp_ptr][i]>expPref[exp_ptr][i+1])
		{
		  if(usedsim[prefPos[i+1]]==-1 || usedsim[expPref[exp_ptr][i+1]]>exp_ptr)
		    {
		      printf("Exp: %d skipping %d to use %d.\n", exp_ptr, expPref[exp_ptr][i], expPref[exp_ptr][i+1]);
		      ++i;
		      continue;
		    }
		  else
		    {
		      printf("Exp: %d would like %d, which is already matched with %d.\n", exp_ptr, expPref[exp_ptr][i], usedsim[expPref[exp_ptr][i]]);
		    }
		}
	    }
	  sim_ptr=expPref[exp_ptr][i];
	  /* If this simulated shift has not been matched with an experimental shift make a match. This
	     match is not permanent. If a better choice comes up later it will be replaced. */
	  if(usedsim[sim_ptr]==-1)
	    {
	      usedsim[sim_ptr]=firstExp+exp_ptr;
	      usedexp[firstExp+exp_ptr]=expPref[exp_ptr][i];
	      matched=1;
	      break;
	    }
	  else if((mode==SE_PURGE_C || mode==SE_PURGE_H) && usedsim[sim_ptr]<exp_ptr)
	    {
	      ++i;
	      continue;
	    }
	    ...
	    {
	      /* The simulated shift has already been matched */
	      
	      temp_ptr=usedsim[sim_ptr]-firstExp;

	      /* If a new match was found */
	      if(matched)
		{
		  temp_ptr=usedsim[sim_ptr];
		  usedsim[sim_ptr]=firstExp+exp_ptr;
		  usedexp[firstExp+exp_ptr]=sim_ptr;
		  usedexp[temp_ptr]=-1;
		  prefPos[exp_ptr]=i;
		  prefPos[temp_ptr-firstExp]+=1;
		  exp_ptr=Min(exp_ptr,temp_ptr-firstExp-1);
		  break;
		}

	      for(j=0;j<simPrefCnt[sim_ptr-firstSim];++j)
		{
		  /* If the simulated shift prefers the match that has been made before: */
		  if(simPref[sim_ptr-firstSim][j]==usedsim[sim_ptr])
		    {
		      /* If this new match would still give a better score, i.e. if the next match
			 of the experimental shift is worse than the next match of the simulated shift. This
			 is different from the Gale-Shapley algorithm to solve the stable marriage problem. */
		      if(expPrefScore[exp_ptr][i]+expPrefScore[temp_ptr][prefPos[temp_ptr]+1]<
			 expPrefScore[exp_ptr][i+1]+expPrefScore[temp_ptr][prefPos[temp_ptr]])
			{
			  matched=1;
			}
		      break;
		    }
		  /* This match is better than the match that has been made before */
		  if(simPref[sim_ptr-firstSim][j]==firstExp+exp_ptr)
		    {
		      if(expPrefCnt[temp_ptr]>0)
			{
			  /* If this new match would give a better score, i.e. if the next match
			     of the experimental shift is worse than the next match of the simulated shift. This
			     is different from the Gale-Shapley algorithm to solve the stable marriage problem. */
			  if(expPrefScore[exp_ptr][i]+expPrefScore[temp_ptr][prefPos[temp_ptr]+1]<
			     expPrefScore[exp_ptr][i+1]+expPrefScore[temp_ptr][prefPos[temp_ptr]])
...
void SP_InsertInPrefList(int *pos, float *scores, int *n, int listLen, int thisPos, float thisScore)
{
  int i, j, step;
  ...
  /* Find the position */
  while(i<*n && i>=1 && (thisScore<scores[i-1] || thisScore>scores[i]))
    {
      while(i>=1 && thisScore<scores[i-1])
	{
	  step=(step+1)/2;
	  i-=step;
	}
      while(i<*n && thisScore>scores[i])
	{
	  step=(step+1)/2;
	  i+=step;
	}
...
{
  int i, j, k, firstsim=9999, firstexp=9999, eCnt, sCnt, nSimTwins=0, nExpTwins=0, found, minPtr;
  float diff, minDiff;
  ...
  for(i=0; i<sCnt-1; i++)
    {
      if(usedsim[i]==-1 && sim->ChCShift[i]<80 && sim->ChCShift[i]>30)
	{
  ...
  /* If there were more sim than exp twins try to use the same exp more than once */
  if(nSimTwins>nExpTwins && sCnt>eCnt)
    {
      for(i=firstsim; i<sCnt-1; i++)
	{
	  if(usedsim[i]==-1)
	    {
	      minDiff=9999;
	      ...
	      for(k=0; k<eCnt; k++)
		{
		  if(usedexp[k]==-1)
		    {
		      diff=SP_2DDelta(sim,i,exp,k,SE_PURGE_CH)+SP_2DDelta(sim,j,exp,k,SE_PURGE_CH);
		      if(diff<SP_CH_CUTOFF*2 && diff<minDiff)
			{
			  minDiff=diff;
			  minPtr=k;
			}
...
{
  int i, j, k, l, ptr=-1, lptr=-1, secondPtr=-1, secondLptr=-1, eCnt, sCnt, multi;
  int *temp_usedsim, *temp_usedexp;
  ...
  for(sim_ptr=0; sim_ptr<sCnt-1; sim_ptr++)
      ...
      if(SP_2DDelta(sim,sim_ptr,sim,sim_ptr+1,SE_PURGE_CH)<0.0001)
	{
	  ...
	  if(usedsim[sim_ptr]==-1 && usedsim[sim_ptr+1]==-1)
	    {
	      temp_ptr=-1;
	      for(exp_ptr=0;exp_ptr<eCnt-1;++exp_ptr)
		{
		  if(usedexp[exp_ptr]==-1)
		    {
		      /* If the next experimental shift is unassigned very close to this shift skip this assignment. Otherwise the other
			 experimental shift might not have a good match anymore. */
		      if(fabs(exp->ChCShift[exp_ptr]-exp->ChCShift[exp_ptr+1])<0.01 && usedexp[exp_ptr+1]==-1)
			{
			  exp_ptr++;
			  continue;
			}
...
/* REPLACED BY  SP_AssExpSimRegion() */
			 ...
			 unsigned int firstexp, int **globalSimAss, int **globalExpAss)
{
  ...
  for (exp_ptr=firstexp; exp_ptr<eCnt; ++exp_ptr)
      ...
      for(;sim_ptr<sCnt;++sim_ptr)
	{
      ...
      /* If this is an alpha/beta mixture there are more experimental
	 than simulated shifts. Look for the experimental shift closest to
	 the simulated shift instead. */
      if(sim->anomericInvertedStructure)
	{
	  if(ass_lo>cutoff)
	    {
	      if(exp_ptr==firstexp)
	  ...
	  ass_lo=99999;
	  for(temp_exp_ptr=exp_ptr+1; temp_exp_ptr<eCnt; ++temp_exp_ptr)
	    {
	      if(usedexp[temp_exp_ptr]!=-1)
		{
		  continue;
		}
	      ass_hi=SP_2DDelta(exp,temp_exp_ptr,sim,sim_ptr,mode);
	  ...
	  if(ass_lo<temp_ass_lo)
	    {
	      usedsim[sim_ptr]=new_exp_ptr;
	      usedexp[new_exp_ptr]=sim_ptr;
	      continue;
	    }
	}
      ...
      if(alt_temp_ptr!=-1)
	{
	  if(ass_second_lo > ass_lo + cutoff*5)
	    {
	      alt_temp_ptr=sim_ptr;
	      flag=2;
	    }
	  /* Check if these two alternatives have already been checked in an earlier iteration */
	  if(globalExpAss[exp_ptr][0]!=-1 && sim_ptr != alt_temp_ptr)
	    {
	      if(globalExpAss[exp_ptr][0]==sim_ptr && globalExpAss[exp_ptr][1]==alt_temp_ptr)
		{
	      ...
	      else if (globalExpAss[exp_ptr][0]==alt_temp_ptr && globalExpAss[exp_ptr][1]==sim_ptr)
		{
      ...
      if(alt_temp_ptr==sim_ptr)
	{
      ...
      temp_usedexp[exp_ptr]=alt_temp_ptr;
      temp_usedsim[sim_ptr]=-1;
      temp_usedsim[alt_temp_ptr]=exp_ptr;
	 ...
	 
	{
	  ...
	  globalExpAss[exp_ptr][0]=alt_temp_ptr;
      ...
      else
	{
	  ...
	  globalExpAss[exp_ptr][0]=sim_ptr;
	  globalExpAss[exp_ptr][1]=alt_temp_ptr;
	}
    }
			...
			struct SP_assignments *nr_assign)
{
  ...
  int *usedsim, *usedexp, localNUnassigned=0, *mismatches, i, stepSize;
  ...
  /*  mismatches=malloc (sCnt*sizeof(int));*/
  ...
  if(SP_AssignMode!=SP_AM_CUTOFF)
    {
      if(mode==SE_PURGE_CH)
	{
	  /* Assign anomeric region (13C 90-110 ppm) */
	  SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 90, 110, mode);
	  /* Assign low shift region */
	  SP_AssExpSimRegion(exp, sim, usedexp, usedsim, 10, 25, mode);
      ...
      for(i=0;i<sCnt;++i)
	{
	  exp_ptr=SP_FindExpAssignedToSim(exp, sim, i, mode);
	  if(exp_ptr>=0)
	    {
	      usedexp[exp_ptr]=i;
	      usedsim[i]=exp_ptr;
  ...
  for(exp_ptr=0; exp_ptr<eCnt; ++exp_ptr)
      ...
      if(usedexp[exp_ptr]==-1)
	{
	  ...
	  for(sim_ptr=0; sim_ptr<sCnt; ++sim_ptr)
	    {
		  ...
		  switch(mode)
		    ...
		    case SE_PURGE_H:
		      diff=fabs(eShift[exp_ptr] - sShift[sim_ptr]);
		      /* Stop if the error is increasing - since both lists are sorted there will not be better matches. */
		      if(diff>minDiff)
			{
			  fail=1;
			}
		      break;
		    case SE_PURGE_CH:
		      /* These limits below give a significant speed-up, but they must be verified to work properly.
			 SP_CH_CUTOFF*15 seems to give reliable results so this should be fine. */
		      /* Check if the experimental shift is very far from the simulated shift */
		      if(fabs(exp->ChCShift[exp_ptr]-sim->ChCShift[sim_ptr])>SP_CH_CUTOFF*20)
			{
			  /* If the simulated shift is a lot lower break the loop */
			  if(minDiff<9999 && sim->ChCShift[sim_ptr]-exp->ChCShift[exp_ptr]<-SP_CH_CUTOFF*20)
			    {
			      fail=1;
			      break;
			    }
		  ...
		  /* Simulated shifts can be used more than once for quick assignments to avoid extreme mismatches. */
		  if(usedsim[sim_ptr]!=-1)
		    {
		      diff+=2*cutoff;
		    }
	  ...
	  /*	  if(minDiff<cutoff*10)*/
	  if(minDiff<99999)
	    {
	      usedsim[temp_ptr]=exp_ptr;
	      usedexp[exp_ptr]=temp_ptr;
	    }
	  /*	  mismatches[temp_ptr]=mismatchFlag;*/
	}
      else
	{
	  sim_ptr=usedexp[exp_ptr];
	}
  ...
  /* Print the assignment table showing the errors. */
  if(print==1)
    {
      SP_PrintAssignmentTable(exp, sim, usedsim, usedexp, &abs_err, sys_err, &rms_err, nr_assign, localNUnassigned, mode);
    }
...
{
  int exp_ptr, sim_ptr;
  for(exp_ptr=0;exp_ptr<cnt;++exp_ptr)
    {
      if(usedexp[exp_ptr]==-1)
	{
	  continue;
...
void SP_MultiExpAssignments(struct BU_Struct *exp, struct BU_Struct *sim, int *usedsim, float *err, float *abs_err, float *rms_err, int *nUnassigned, const unsigned int mode)
{
  int i, mismatchFlag=0, exp_ptr, sim_ptr, temp_ptr, matched, sCnt, eCnt;
      ...
      if(usedsim[i]==-1)
	{
	  ...
	  for(exp_ptr=0; exp_ptr<eCnt; ++exp_ptr)
		...
		case SE_PURGE_H:
		  ...
		  break;
		case SE_PURGE_CH:
		  /* Check if the experimental shift is far from the simulated shift */
		  if(ass_lo<9999 && fabs(exp->ChCShift[exp_ptr]-sim->ChCShift[sim_ptr])>SP_CH_CUTOFF*7.5)
		    {
		      /* If it is a lot lower break the loop */
		      if(exp->ChCShift[exp_ptr]-sim->ChCShift[sim_ptr]<-SP_CH_CUTOFF*10)
			{
			  break;
			}
		      continue;
...
void SP_PrintAssignmentTable(struct BU_Struct *exp, struct BU_Struct *sim, int *usedsim, int *usedexp, float *abs_err, float *sys_err, float *rms_err, struct SP_assignments *nr_assign, int nUnassigned, const unsigned int mode)
{
  ...
  for(sim_ptr=0; sim_ptr<sCnt; sim_ptr++)
    {
      exp_ptr=usedsim[sim_ptr];
      if(exp_ptr==-1 || usedexp[exp_ptr]!=sim_ptr)
	{
	  fail=1;
	}
      else
...
int SP_Assign()
{
  ...
  float error, rms_err, abs_err, totalExpectedError;
  struct SP_assignments *nr_assign;
  ...
  Nmr_Shift shift;
  char simName[NODE_NAME_LENGTH], expName[NODE_NAME_LENGTH], simPrefix[NODE_NAME_LENGTH],
    expPrefix[NODE_NAME_LENGTH], shiftListNameString[NODE_NAME_LENGTH+16];
  ...
  exp=(struct BU_Struct *)FindNode((struct Node *)&StructList, expName);
  /* If the experimental spectrum does not exist still try to assign the simulated spectrum and then return */
  if (exp==NULL)
    {
      if(projectValid)
	{
	  ...
	  if(projectValid)
	    {
	      CC_CreateResonanceGroups(&Global_CcpnProject, 
				       &Global_NmrProject, expMolSystem, sim->CcpnChainCode);
	      if(projectValid)
		{
  ...
  if(!sim->anomericInvertedStructure &&
     (exp->CCnt>sim->CCnt || exp->HCnt>sim->HCnt ||
      exp->ChCnt>sim->ChCnt || exp->HhCnt>sim->HhCnt ))
    {
  ...
  while (sim && exp)
      ...
      if (exp->ChCnt>0)
	{
	  ...
	  if(projectValid)
	    {
	      CC_SetPeakListError(&Global_CcpnProject, &Global_NmrProject, SP_ID_2D_CH, error, sim->ChRms);
	    }
	};
      ...
      if (exp->HhCnt>0 && (onlyBasic == NULL || onlyBasic->Value.Value.Float<=0))
	{
	  ...
	  if(projectValid)
	    {
	      CC_SetPeakListError(&Global_CcpnProject, &Global_NmrProject, SP_ID_2D_HH, error, -1);
	    }
	};
      ...
      if (exp->HCnt>0)
	{
	  ...
	  if(projectValid)
	    {
	      CC_SetPeakListError(&Global_CcpnProject, &Global_NmrProject, SP_ID_PROTON, error, sim->HRms);
	    }
	}
      ...
      if (exp->CCnt>0)
	{
	  ...
	  if(projectValid)
	    {
	      CC_SetPeakListError(&Global_CcpnProject, &Global_NmrProject, SP_ID_CARBON, error, sim->CRms);
	    }
	}

      

      totalExpectedError=0;
      ...
      for(i=0;i<size;++i)
	{
	  ...
	  totalExpectedError+=eunit->Error;
	  ...
	  if(projectValid)
	    {
	      ...
	      CC_SetResidueError(&Global_CcpnProject, &Global_NmrProject, 0, residue, eunit->Error);
	      CC_SetResidueError(&Global_CcpnProject, &Global_NmrProject, SP_ID_CARBON, residue, eunit->CError);
	      CC_SetResidueError(&Global_CcpnProject, &Global_NmrProject, SP_ID_PROTON, residue, eunit->HError);
	    }
	}
      sim->TotFit=SP_C_WEIGHT*sim->CFit+SP_H_WEIGHT*sim->HFit+SP_CH_WEIGHT*sim->ChFit+
	SP_HH_WEIGHT*sim->HhFit;
      ...
      if(projectValid)
	{
	  CC_SetStructureError(&Global_CcpnProject, &Global_NmrProject, &expMolSystem, sim->CcpnChainCode, 0, totalExpectedError);
	  CC_SetStructureError(&Global_CcpnProject, &Global_NmrProject, &expMolSystem, sim->CcpnChainCode, SP_ID_CARBON, sim->CFit);
	  CC_SetStructureError(&Global_CcpnProject, &Global_NmrProject, &expMolSystem, sim->CcpnChainCode, SP_ID_CARBON, sim->HFit);
	}
      ...
      if(projectValid)
	{
      ...
      if(projectValid)
	{
	  CC_SetAllShiftErrors(&Global_CcpnProject, &Global_NmrProject, sim, exp);
	}
      if(!multiFlag)
	{
	  break;
	     ...
	     struct SP_assignments *nr_assign)
{
  ...
  int j, match=1, anomerPos, unitnr, substPos;
  float error;
  char unitName[NODE_NAME_LENGTH];
  struct VA_Variable *onlyCurate;

  onlyCurate=(struct VA_Variable *)FindNode(&(VariableList.Head), "onlycurate");
  ...
  if(match==0)
	     ...
	     unitnr<BU_MAX_UNITS)
	{
      ...
      if(projectValid && (onlyCurate==NULL||onlyCurate->Value.Value.Float<=0))
	{
	  ...
	  if(residue && !ApiObject_IsNone(residue) && projectValid)
	    {
	      ...
	      if(strcasecmp("subst", eunit->Residue->Shifts.Type->CTsuperclass)==0)
		{
	      ...
	      else
		{
	      ...
	      atoms=CC_FindAtomsNamed(residue,
				      unit->Residue->Shifts.Type->Atom[j].Label,"C", substPos);
	     ...
	     struct SP_assignments *nr_assign)
{
  ...
  ApiSet atoms;
  struct BU_Unit *unit=0, *eunit=0;
  int j, k, match=1, anomerPos, unitnr, substPos;
  float error;
  char atomname[3];
  char unitName[NODE_NAME_LENGTH];
  struct VA_Variable *onlyCurate;
	  ...
	  if(residue && !ApiObject_IsNone(residue) && projectValid)
	      ...
	      else
		{
		 ...
		 to show which hydrogen this is. */
	      atoms=CC_FindAtomsNamed(residue,
				      unit->Residue->Shifts.Type->Atom[j].Label,atomname, substPos);
	      ...
	      struct SP_assignments *nr_assign)
{
  ...
  ApiSet atoms;
  struct BU_Unit *unit=0, *eunit=0;
  int j, k, l, m, match=1, anomerPos, unitnr, substPos;
  float error;
  ...
  /* no good assignment */
  if (e_off<0 || (error>SP_HH_CUTOFF && printfail==1))
    {
      ...
      if(projectValid && (onlyCurate==NULL||onlyCurate->Value.Value.Float<=0))
	{
	  ...
	  if(residue && !ApiObject_IsNone(residue) && projectValid)
	      ...
	      else
		{
	      ...
	      /* Find the correct atom or atoms in the CCPN project. */
	      atoms=CC_FindAtomsNamed(residue,
				      unit->Residue->Shifts.Type->Atom[j].Label, atomname1, substPos);
	      if(!atoms||ApiSet_Len(atoms)<1)
		{
		  if(atoms && !ApiObject_IsNone(atoms))
		    {
	      ...
	      atoms=CC_FindAtomsNamed(residue,
				      unit->Residue->Shifts.Type->Atom[l].Label, atomname2, substPos);
...
int SP_PopulateAssignments(struct BU_Struct *exp, struct BU_Struct *sim, int *usedexp, int *usedsim, const unsigned int mode)
{
  ...
  if(mode==SE_PURGE_C)
      ...
      for(i=0; i<exp->HCnt; i++)
	{
	  ...
	  for(j=0; j<sim->HCnt; j++)
		...
		{
		  if(sim->HShiftAtom[j].unit->Shifts.Type->Atom[sim->HShiftAtom[j].atom[0]].HCnt==2)
		    {
		      if(fabs(exp->HShift[i]-sim->HShiftAtom[j].unit->Shifts.H[sim->HShiftAtom[j].atom[0]][sim->HShiftAtom[j].atom[1]])>
			 fabs(exp->HShift[i]-sim->HShiftAtom[j].unit->Shifts.H[sim->HShiftAtom[j].atom[0]][(sim->HShiftAtom[j].atom[1]+1)%2]))
			{
			  continue;
			}
		    }
      ...
      for(i=0; i<exp->ChCnt; i++)
	{
	  ...
	  for(j=0; j<sim->ChCnt; j++)
		...
		{
		  if(sim->ChShiftAtom[j].unit->Shifts.Type->Atom[sim->ChShiftAtom[j].atom[0]].HCnt==2)
		    {
		      if(fabs(exp->ChHShift[i]-sim->ChShiftAtom[j].unit->Shifts.H[sim->ChShiftAtom[j].atom[0]][sim->ChShiftAtom[j].atom[1]])>
			 fabs(exp->ChHShift[i]-sim->ChShiftAtom[j].unit->Shifts.H[sim->ChShiftAtom[j].atom[0]][(sim->ChShiftAtom[j].atom[1]+1)%2]))
			{
			  continue;
			}
		    }
  ...
  /* Do not handle TOCSY shifts yet. */
  /*  else if(mode==SE_PURGE_HH)
      ...
      for(i=0; i<exp->HhCnt; i++)
	{
	  ...
	  for(j=0; j<sim->HhCnt; j++)
		...
		{
		  if(sim->HhLoShiftAtom[j].unit->Shifts.Type->Atom[sim->HhLoShiftAtom[j].atom[0]].HCnt==1 &&
		     sim->HhHiShiftAtom[j].unit->Shifts.Type->Atom[sim->HhHiShiftAtom[j].atom[0]].HCnt==1)
		    {
		      usedexp[i]=j;
		      usedsim[j]=i;
		      j=sim->HhCnt;
			 ...
			 struct BU_Struct *sim, const unsigned int mode)
{
  ...
  if(mode==SE_PURGE_C||mode==SE_PURGE_CH)
    {
      if(QuickFind(sim->CAssignments[0], 0, sim->nCAssignments-1, eCShift)==-1)
	{
	  return 0;
	}
      else
  ...
  if(mode==SE_PURGE_H||mode==SE_PURGE_HH||mode==SE_PURGE_CH)
    {
      if(QuickFind(sim->HAssignments[0], 0, sim->nHAssignments-1, eHShift)==-1)
	{
	  found=0;
	}
      else
...
int SP_FindExpAssignedToSim(const struct BU_Struct *exp,
		  ...
		  if(mode==SE_PURGE_HH)
		      ...
		      for(k=0;k<sim->nHAssignments;k++)
			{
			  /* Just to cause confusion CShift is used to
			     point at HhHiShift */
			  if(fabs(sim->HAssignments[1][k]-sCShift[sim_ptr])<0.0001)
			    {
...
int SP_Gnuplot()
{
  ...
  if(!plot)
    {
      Error(PA_ERR_FAIL, "Cannot find gnuplot");
    }
  if(strlen(outputfile)>0)
    {
      if(strlen(outputfile)>4 && strcasecmp(outputfile+strlen(outputfile)-4,".eps")==0)
	{
	  gnuplot_cmd(plot, "set terminal post eps color enh solid");
	}
      else
	{
	  gnuplot_cmd(plot, "set terminal png enh");
	}
...
int SP_Gnuplot1D(gnuplot_ctrl *plot, struct BU_Struct *sim, struct BU_Struct *exp, unsigned int mode)
{
  ...
  switch(mode)
    {
    case SP_ID_PROTON:
      gnuplot_set_xlabel(plot, "^{1}H chemical shift /ppm");
      break;
    case SP_ID_CARBON:
      gnuplot_set_xlabel(plot, "^{13}C chemical shift /ppm");
      break;
    }
...
int SP_Gnuplot2D(gnuplot_ctrl *plot, struct BU_Struct *sim, struct BU_Struct *exp, unsigned int mode)
{
  ...
  switch(mode)
    {
    case SP_ID_2D_HH:
      gnuplot_set_ylabel(plot, "^{1}H chemical shift /ppm");
    case SP_ID_2D_CH:
      
      gnuplot_set_ylabel(plot, "^{13}C chemical shift /ppm");
      break;
    }
  gnuplot_plot_xy(plot, dsXShift, dsYShift, sCnt, sim->Node.Name);
  if(exp)
