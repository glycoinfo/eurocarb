...
/* This function loads the CCPN xml project file specified by *projectFileName into memory at
   the address specified by ccpnProject.
   The function returns 1 if successful and 0 if the project could not be opened. */
int CC_LoadProject(Impl_MemopsRoot *ccpnProject, char *projectFileName)
{

  *ccpnProject = loadProject(projectFileName, NULL);
...
/* This function saves the ccpn project to the specified projectFileName.
   ...
   project are saved. */
int CC_SaveProject(Impl_MemopsRoot *ccpnProject, char *projectFileName)
{
  ApiString path=ApiString_New(projectFileName);
  ApiObject args[] = { *ccpnProject, path, NULL };
...
/* This function loads the current or first Nmr project of the CCPN project in memory.
   *nmrProject is a pointer to the Nmr project address.
   The function returns 1 is successful and 0 if the Nmr project could not be opened. */
int CC_GetNmrProject(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject)
{
  ApiSet projects=0;
  ApiString name;
  int numProjects;

  if(!*nmrProject||ApiObject_IsNone(*nmrProject))
    {
      *nmrProject = Impl_MemopsRoot_GetCurrentNmrProject(*ccpnProject);
    }
  if(!*nmrProject||ApiObject_IsNone(*nmrProject))
    {
      projects = Impl_MemopsRoot_GetNmrProjects(*ccpnProject);

      if(projects)
	{
	  numProjects=ApiSet_Len(projects);
	}
      else
	{
	  numProjects=0;
	 ...
	 TODO: Should there be a way to select which nmrProject to use? */
      if(projects && numProjects>0)
	{
	  *nmrProject=ApiSet_Get(projects,0);
	}
      /* If there were no nmrProjects generate a new one */
      else
	{
	  *nmrProject = Nmr_NmrProject_Init_reqd(*ccpnProject,"exp");
 
	  if(nmrProject)
	    {
	      name=Impl_MemopsRoot_GetName(*ccpnProject);
	      if(!Anal_AnalysisProject_Init_reqd(*ccpnProject,
						 ApiString_Get(name),
						 *nmrProject))
		{
...
/* This function points *nmrProject at the Nmr project of a specific name - specified by
...
int CC_GetNmrProject_Named(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, char *wanted_name)
{
  ApiString name;
  ApiMap parameters;
  Anal_AnalysisProject analysisProject;

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "name", wanted_name);

  *nmrProject=Impl_MemopsRoot_FindFirstNmrProject(*ccpnProject,parameters);
  ...
  *nmrProject = Nmr_NmrProject_Init_reqd(*ccpnProject,wanted_name);
  if(!*nmrProject || ApiObject_IsNone(*nmrProject))
    {
      projectValid=0;
      return 0;
  ...
  analysisProject=Impl_MemopsRoot_FindFirstAnalysisProject_keyval1(*ccpnProject, "name", name);
  if(!analysisProject || ApiObject_IsNone(analysisProject))
    {
      analysisProject=Anal_AnalysisProject_Init_reqd(*ccpnProject,
						  ApiString_Get(name),
						  *nmrProject);
	if(!analysisProject || ApiObject_IsNone(analysisProject))
	  {
  ...
  ApiObject_Free(analysisProject);
  ApiObject_Free(name);
  projectValid=1;
  return 1;
}

/* This function finds experiments corresponding to a specific type. *nmrProject is the Nmr
...
ApiSet CC_GetExpOfType(Nmr_NmrProject *nmrProject, char *type)
{
  int j, numExps;
  ...
  exps = Nmr_NmrProject_GetExperiments(*nmrProject);
  if(exps)
    {
...
/* This function gets the peak positions from *experiment and populates the firstDim[] and
   ...
   went wrong. */
int CC_GetPeaks(ApiSet experiments, float firstDim[], float secondDim[])
{
  ApiSet peaks, experimentPeaks, peakDims, peakDimContribs, /*atomSets, atoms,*/ resonances, shifts;
  ApiSetIterator experimentIterator, peakIterator, peakDimContribIterator;
  ApiInteger numDim;
  ApiFloat value;
  Nmr_Experiment experiment;
  ...
  /*  Nmr_AtomSet atomSet;
      Mols_Atom atom;*/
  int i, j, l, cnt=0, numDims, numPeaks=0, /*numAtoms,*/ numPeakDimContribs,
    numMultiResonances, skipflag, nFirstDimValues, nSecondDimValues;
  float *dim, firstDimValues[16], secondDimValues[16];
  ...
  while((experiment=ApiSetIterator_Next(experimentIterator)))
    {
      ...
      if (CC_GetProcSpectrumOfDim(&experiment, ApiInteger_Get(numDim), &spectrum))
	{
	  ...
	  experimentPeaks = Nmr_PeakList_GetPeaks(peakList);
	  peakIterator=ApiSet_Iterator(experimentPeaks);
	  while((peak=ApiSetIterator_Next(peakIterator)))
	    {
	      ApiSet_Add(peaks, peak);
	      ApiObject_Free(peak);
	    }
  ...
  ApiObject_Free(experimentIterator);
  ...
  while((peak=ApiSetIterator_Next(peakIterator)))
      ...
      if(peakDims)
	{
			  ...
			  if(ApiSet_Len(shifts)>0)
			    {
			      ...
			      if(j==0)
				{
				  firstDimValues[nFirstDimValues++]=ApiFloat_Get(value);
				}
			      if(j==1)
				{
				  /* Do not add diagonal peaks in TOCSY experiments. */
				  if(fabs(ApiFloat_Get(value)-firstDimValues[nFirstDimValues-1])<0.00001)
				    {
				      nFirstDimValues--;
				    }
				  else
				    {
				      secondDimValues[nSecondDimValues++]=ApiFloat_Get(value);
				    }
				}
	  ...
	  /* Add the values to the float arrays */
	  for(i=0; i<nFirstDimValues; i++)
	    {
	      if(nSecondDimValues>0)
		{
		  for(j=0; j<nSecondDimValues; j++)
		    {
		      /* The first dimension always has higher shifts than the second dimension (13C resp 1H or 1H high
			 resp 1H low). */
		      if(firstDimValues[i]>=secondDimValues[j])
			{
			  firstDim[cnt]=firstDimValues[i];
			  secondDim[cnt++]=secondDimValues[j];
			}
		      else
			{
			  firstDim[cnt]=secondDimValues[j];
			  secondDim[cnt++]=firstDimValues[i];
			}
		    }
		}
	      else if(numDims==1)
...
/* This function gets the shift (only the first shift) of each resonance of a certain isotopeCode
   in *nmrProject. The chemical shifts are put in the float array *values.
   The function returns the number of chemical shifts that have been retrieved. */
int CC_GetShifts(Nmr_NmrProject *nmrProject, char *isotope, float *values)
{
  ...
  resonances=Nmr_NmrProject_FindAllResonances(*nmrProject, parameters);
  if(resonances)
    {
...
/* This function adds peaks in the spectrum with positions corresponding to the input arrays (one or two arrays provided).
   ...
   fails it returns 0. */
int CC_PutPeaks(Nmr_NmrProject *nmrProject, Nmr_DataSource *spectrum, float firstDim[], float secondDim[],
		int n, int type, int addnew, struct BU_Struct *BU_Struct)
{
  ...
  ApiSet peakLists, dataDimRefs, peakContribs, shiftLists;
  ...
  if(!peakList||ApiObject_IsNone(peakList))
    {
      parameters=ApiMap_New();
      ApiMap_SetString(parameters, "details", "Added in CASPER");
  ...
  shiftLists=Nmr_NmrProject_FindAllMeasurementLists(*nmrProject,apiShiftListClass);
  ...
  if(numShiftLists<=0)
    {
      shiftList=Nmr_NmrProject_NewShiftList_reqd(*nmrProject);
    }
  else
    {
 ...
 /* Set some initial variables depending on experiment type */
  ...
  for(i=0;i<n;i++)
      ...
      if(!peak||ApiObject_IsNone(peak)||!Nmr_Peak_CheckAllValid(peak, ApiBoolean_False()))
	{
	  ...
	  if(type!=SP_ID_PROTON && type!=SP_ID_CARBON)
	    {
      ...
      if(firstDim && numDim>=1)
	{
	  ...
	  if(peakDim && !ApiObject_IsNone(peakDim))
	      ...
	      /*	      if(!Nmr_Peak_CheckAllValid(peak, ApiBoolean_False()))
		{
	      ...
	      CC_SetPeakValues(nmrProject, shiftList, &peak, peakDim, peakContrib, dataDimRef, dataDim,
			       resonanceParameters[0],firstDim[i]+firstDimCorrection, addnew);
      ...
      if(secondDim && numDim>=2)
	{
	  ...
	  if(peakDim && !ApiObject_IsNone(peakDim))
	      ...
	      /*	      if(!Nmr_Peak_CheckAllValid(peak, ApiBoolean_False()))
		{
	      ...
	      CC_SetPeakValues(nmrProject, shiftList, &peak, peakDim, peakContrib, dataDimRef, dataDim,
			       resonanceParameters[1],secondDim[i]+secondDimCorrection, addnew);
	      /*	      printf("PeakDim: Dim %d. Value %f.\n", ApiInteger_Get(Nmr_PeakDim_GetDim(peakDim)), ApiFloat_Get(Nmr_PeakDim_GetValue(peakDim)));*/
	      ApiObject_Free(dataDimRefs);
		     ...
		     ApiInteger *apiIntFirstDim, ApiInteger *apiIntSecondDim)
{
  ApiSet peaks, shifts;
  ApiMap parameters;
  ApiSetIterator peakIterator, shiftIterator;
  ApiList isotopeCodes;
  ApiString isotopeCode;
  ApiFloat value, shiftValue;
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "className", "Shift");
  ...
  while((peak=ApiSetIterator_Next(peakIterator)))
      ...
      if(firstPeakDim)
	{
	  ...
	  /* See if any of the shifts match the wanted value */
	  shifts=Nmr_PeakDim_FindAllMeasurements(firstPeakDim, parameters);
	  shiftIterator=ApiSet_Iterator(shifts);
	  foundShift=0;
	  while((shift=ApiSetIterator_Next(shiftIterator)))
	    {
	      shiftValue=Nmr_Shift_GetValue(shift);
	      if(fabs(ApiFloat_Get(shiftValue)-firstDim)<tolerance)
		{
		  foundShift=1;
		}
	  ...
	  ApiObject_Free(shifts);
	  if(foundShift || fabs(firstPeakDimValue-firstDim)<tolerance)
	    {
		  ...
		  if(secondPeakDim)
		    {
		      ...
		      shifts=Nmr_PeakDim_FindAllMeasurements(secondPeakDim, parameters);
		      shiftIterator=ApiSet_Iterator(shifts);
		      foundShift=0;
		      while((shift=ApiSetIterator_Next(shiftIterator)))
			{
			  shiftValue=Nmr_Shift_GetValue(shift);
			  if(fabs(ApiFloat_Get(shiftValue)-secondDim)<tolerance)
			    {
			      foundShift=1;
			    }
			  ApiObject_Free(shiftValue);
		      ...
		      ApiObject_Free(shiftIterator);
		      ApiObject_Free(shifts);
		      if(foundShift || fabs(secondPeakDimValue-secondDim)<tolerance)
			{
			  break;
			}
		      else
...
ApiSet CC_FindPeaks(ApiSet peakLists, float firstDim, float secondDim, int numDim, float tolerance,
		    ApiInteger *apiIntFirstDim, ApiInteger *apiIntSecondDim)
{
  ApiSet peaks, foundPeaks, shifts;
  ApiMap parameters;
  ApiSetIterator peakIterator, peakListIterator, shiftIterator;
  ApiList isotopeCodes;
  ApiString isotopeCode;
  ApiFloat value, shiftValue;
  float firstPeakDimValue, secondPeakDimValue;
...
int CC_LoadSpectra(Nmr_NmrProject *nmrProject, unsigned int mode,
		   struct BU_Struct *spectrum)
{
  ...
  if(mode==0||mode==SP_ID_CARBON)
    {
      exps = CC_GetExpOfType(nmrProject,"C");
      if(exps)
	{
	  numExps=ApiSet_Len(exps);
	  if(numExps>0)
	    {
	      spectrum->CCnt=CC_GetPeaks(exps, spectrum->CShift, 0);
	    }
	}
	 ...
	 This means that chemical shifts can be loaded from projects that do not contain any spectra */
      ...
      /*      if(spectrum->CCnt==0)
	{
	  spectrum->CCnt=CC_GetShifts(nmrProject, "13C", spectrum->CShift);
	  nSpectra++;
	  }*/
    }
  ...
  if(mode==0||mode==SP_ID_PROTON)
    {
      exps = CC_GetExpOfType(nmrProject,"H");
      if(exps)
	{
	  numExps=ApiSet_Len(exps);
	  if(numExps>0)
	    {
	      spectrum->HCnt=CC_GetPeaks(exps, spectrum->HShift, 0);
	    }
	}
	 ...
	 This means that chemical shifts can be loaded from projects that do not contain any spectra */
      ...
      /*      if(spectrum->HCnt==0)
	{
	  spectrum->HCnt=CC_GetShifts(nmrProject, "1H", spectrum->HShift);
	  nSpectra++;
	  }*/
    }
  ...
  if(mode==0||mode==SP_ID_2D_CH)
    {
      exps = CC_GetExpOfType(nmrProject,"H[C]");
      if(exps)
	{
	  numExps=ApiSet_Len(exps);
	  if(numExps>0)
	    {
	      spectrum->ChCnt=CC_GetPeaks(exps, spectrum->ChCShift, spectrum->ChHShift);
	    }
	}
    }
  ...
  if(mode==0||mode==SP_ID_2D_HH)
    {
      exps = CC_GetExpOfType(nmrProject,"H_H.TOCSY");
      if(exps)
	{
	  numExps=ApiSet_Len(exps);
	  if(numExps>0)
	    {
	      spectrum->HhCnt=CC_GetPeaks(exps, spectrum->HhHiShift, spectrum->HhLoShift);
	    }
	}
    }
...
/* This function creates a new spectrum (new Nmr_DataSource) in the specified NmrProject.
   ...
   successful. */
Nmr_DataSource CC_NewSpectrum(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, int type)
{
  ...
  experiment=Nmr_NmrProject_NewExperiment(*nmrProject, parameters);
  ...
  /*  if(!Nmr_NmrProject_CheckAllValid(*nmrProject, ApiBoolean_False()))
  ...
  expPrototypes=Impl_MemopsRoot_FindAllNmrExpPrototypes(*ccpnProject,parameters);
  ...
  if(!typefound)
    {
      ...
      expPrototype=Impl_MemopsRoot_NewNmrExpPrototype(*ccpnProject,parameters);
...
int CC_AllPeaksInSpectra(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject,
			    struct BU_Struct *BU_Struct)
{
  ...
  if(BU_Struct->HCnt>0)
    {
      mode=SP_ID_PROTON;
      experiments=CC_GetExpOfType(nmrProject,"H");
      if(ApiSet_Len(experiments)>0 && projectValid)
	{
	  ...
	  if(!CC_GetProcSpectrumOfDim(&experiment, 1, &ccpn_Spectrum))
	    {
	      ccpn_Spectrum=CC_NewSpectrum(&Global_CcpnProject,nmrProject, mode);
	    }
	  ApiObject_Free(experiment);
	}
      else if (projectValid)
	{
      ...
      if(ccpn_Spectrum)
	{
	  if(!CC_PutPeaks(nmrProject, &ccpn_Spectrum, BU_Struct->HShift,
			  0, BU_Struct->HCnt, mode, 0, BU_Struct))
	    {
  ...
  if(BU_Struct->CCnt>0)
    {
      mode=SP_ID_CARBON;
      experiments=CC_GetExpOfType(nmrProject,"C");
      if(ApiSet_Len(experiments)>0 && projectValid)
	{
	  experiment=ApiSet_Get(experiments,0);
	  if(!CC_GetProcSpectrumOfDim(&experiment, 1, &ccpn_Spectrum))
      ...
      else if (projectValid)
	{
      ...
      if(ccpn_Spectrum)
	{
	  if(!CC_PutPeaks(nmrProject, &ccpn_Spectrum, BU_Struct->CShift,
			  0, BU_Struct->CCnt, mode, 0, BU_Struct))
	    {
	      ApiObject_Free(ccpn_Spectrum);
	      return 0;
	    }
  ...
  if(BU_Struct->ChCnt>0)
    {
      mode=SP_ID_2D_CH;
      experiments=CC_GetExpOfType(nmrProject,"H[C]");
      if(ApiSet_Len(experiments)>0 && projectValid)
	{
	  experiment=ApiSet_Get(experiments,0);
      ...
      else if (projectValid)
	{
      ...
      if(ccpn_Spectrum)
	{
	  if(!CC_PutPeaks(nmrProject, &ccpn_Spectrum, BU_Struct->ChCShift,
			  BU_Struct->ChHShift, BU_Struct->ChCnt, mode, 0, BU_Struct))
	    {
	      ApiObject_Free(ccpn_Spectrum);
	      return 0;
	    }
  ...
  if(BU_Struct->HhCnt>0)
    {
      mode=SP_ID_2D_HH;
      experiments=CC_GetExpOfType(nmrProject,"H_H.TOCSY");
      if(ApiSet_Len(experiments)>0 && projectValid)
	{
	  experiment=ApiSet_Get(experiments,0);
	  if(!CC_GetProcSpectrumOfDim(&experiment, 2, &ccpn_Spectrum))
      ...
      else if (projectValid)
	{
      ...
      if(ccpn_Spectrum)
	{
	  if(!CC_PutPeaks(nmrProject, &ccpn_Spectrum, BU_Struct->HhHiShift,
			  BU_Struct->HhLoShift, BU_Struct->HhCnt, mode, 0, BU_Struct))
	    {
	      ApiObject_Free(ccpn_Spectrum);
	      return 0;
	    }
...
void CC_SetPeakValues(Nmr_NmrProject *nmrProject, Nmr_ShiftList shiftList, Nmr_Peak *peak,
...
{
  ApiMap peakDimContribParameters;
  ApiSet shifts=0, peakContribs, shiftPeakDims;
  ApiFloat apiValue;
  Nmr_Shift shift;
  Nmr_Resonance resonance;
  Nmr_PeakDimContrib peakDimContrib;
  ...
  /*  if(match>=0)
    {
      ...
      resonance=Nmr_NmrProject_FindFirstResonance(*nmrProject,localResonanceParameters);
	...
	{
	  resonance=Nmr_NmrProject_NewResonance(*nmrProject, resonanceParameters);
  ...
  */
  ...
  /*else*/ if(addnew!=1)
    {
      shifts=CC_FindShifts(nmrProject, value);
      if(shifts)
	{
	  numShifts=ApiSet_Len(shifts);
	}
      ...
      for(i=0;i<numShifts;i++)
	{
	  ...
	  if(!ApiSet_Contains(shiftPeakDims, peakDim))
	    {
	      Nmr_Shift_AddPeakDim(shift, peakDim);
	    }
...
int CC_ResiduetoUnit(Impl_MemopsRoot *ccpnProject, struct BU_Struct *BU_Struct, int multiBuildFlag)
{
  ...
  ApiInteger nr;
  ApiString string;
  int i, j, k, cnt=0, allMolSystemsLen, nmods, flag, repeatFlag, nsubsts, substAnomerPos;
  int currResPos, fromResPos, toResPos, numChains, repeatFromPos, repeatToPos;
  int closingPos1, closingPos2;
  char residuename[NODE_NAME_LENGTH], descriptor[32], stereo[4], *basetype, *basetype_tok, *type, *ringAndMods, *mods;
  char *substs, *subst, *mod, name[4], linkCode[16], structBaseName[NODE_NAME_LENGTH], unusedSubsts[64];
  struct TY_CTmodifier modProperties[8];
  struct TY_CTsubstituent substProperties[8];
  struct RE_Residue *res, *substRes;
  struct BU_Unit *BU_Unit, *repeatFromUnit=0, *repeatToUnit=0, *substUnit;
  ...
  for(i=0; i<numChains; i++)
      ...
      while((residue=ApiSetIterator_Next(residueIterator)))
	{
	  ...
	  basetype_tok=strtok(basetype,"-_");
	  type=strtok(NULL,"-");
	  ringAndMods=strtok(NULL,":");
	  substs=strtok(NULL,"");
	  /* Special case for e.g. dgro-dgal having "double name" */
	  while(ringAndMods[0]<'0'||ringAndMods[0]>'9')
	    {
	      j=strlen(type)+1;
	      sprintf(basetype_tok, "%s-%s", basetype_tok, type);
	      type+=j;
	      type=strtok(type,"-");
	      ringAndMods=strtok(NULL,":");
	      /*	      while(ringAndMods[0]!='-' && strlen(ringAndMods)>0)
		{
		  type[j++]=ringAndMods[0];
		  ringAndMods++;
		  }*/
	      /*	      type[j]=0;*/
	      /*	      ringAndMods++;*/
	  ...
	  if(!substs)
	    ...
	    {
	      /* Check which substituents there are. They are not added to the residue here, but those that
		 are not part of the residue as such are listed separately */
	      subst=strtok(substs, ":");
	      while(subst)
		{
		  strcpy(substProperties[nsubsts].Node.Name, subst);
		  strcpy(substProperties[nsubsts].substituent, subst+3);
		  /* See if this substituent exists - otherwise it will not be added as a substituent,
		     but searched for together with the sugar. This happens for e.g. NAc and OMe. */
		  substRes=(struct RE_Residue *)FindNode((struct Node *)&ResidueList, substProperties[nsubsts].substituent);
		  if(!substRes || strcmp("subst", substRes->Shifts.Type->CTsuperclass)!=0)
		    {
		      if(strlen(unusedSubsts)>0)
			{
			  strcat(unusedSubsts, ":");
			}
		      sprintf(unusedSubsts,"%s%s", unusedSubsts, subst);
		    }
		  else
		    {
	  ...
	  for(res=(struct RE_Residue *)ResidueList.Head.Succ;
	      ...
	      if(strcasecmp(residuename,res->CTname)==0)
		{
		  ...
		  if(strcasecmp(type,res->Shifts.Type->CTsuperclass)==0)
		    {
		      if(strlen(ringAndMods)>=3 && ringAndMods[0]!='0' && ringAndMods[2]!='0')
			{
			  closingPos1=atoi(&ringAndMods[0])-1;
			  closingPos2=atoi(&ringAndMods[2])-1;
			  if(!(res->Shifts.Type->Atom[closingPos1].Type & TY_ANOMERIC))
			    {
			      continue;
			    }
			  if(!(res->Shifts.Type->Atom[closingPos2].Type & TY_CLOSING))
			    {
			      continue;
			    }
			}
			...
			{
			  flag=1;
			  if(strcasecmp(res->Shifts.Type->ccpSub, unusedSubsts)!=0)
			    {
			      flag=0;
			    }
			  if(flag==1)
	  ...
	  for(j=0; j<nsubsts; j++)
	    {
	      substRes=(struct RE_Residue *)FindNode((struct Node *)&ResidueList, substProperties[j].substituent);
	      if(!substRes || strcmp("subst", substRes->Shifts.Type->CTsuperclass)!=0)
		{
		  printf("Could not add substituent %s. Line %d.\n", substProperties[nsubsts].substituent, __LINE__);
		  ApiObject_Free(chain);
		  ApiObject_Free(chains);
		  return 0;
		}
	      sprintf(name,"%ss%d", name, substProperties[j].position[0]);
	      substUnit=(struct BU_Unit*)ME_CreateNode(&UnitMethod, &(BU_Struct->Units), name);
	      if (substUnit==NULL) Error(PA_ERR_FATAL,"Out of memory");
	      if(!(BU_Unit->Shifts.Type->Atom[substProperties[j].position[0]-1].Type & TY_FREE_POS))
		{
		  printf("Substitution at a position that cannot be substituted. Line %d.\n", __LINE__);
		  ApiObject_Free(chain);
		  ApiObject_Free(chains);
		  return 0;
		}
	      substUnit->Residue=substRes;
	      substUnit->Shifts=substRes->Shifts;
	      substUnit->Error=res->Error;
	      substUnit->CcpnUnitNr=ApiInteger_Get(nr);
	      substUnit->Position=substProperties[j].position[0]-1;
	      substAnomerPos=BU_ANOMER_POS(substUnit);
	      /* If no anomeric position find first free position instead */
	      if(substAnomerPos<0)
		{
		  for(k=0; k<substUnit->Shifts.Type->HeavyCnt; k++)
		    {
		      if(substUnit->Shifts.Type->Atom[k].Type & TY_FREE_POS)
			{
			  substAnomerPos=k;
			  k=substUnit->Shifts.Type->HeavyCnt;
			}
		    }
		}
	      substUnit->Subst[substAnomerPos]=BU_Unit;
	      BU_Unit->Subst[substProperties[j].position[0]-1]=substUnit;
	    }
	  ApiObject_Free(nr);
	}
	      ...
	      else
		{
		  ...
		  if(linkCode[0]=='O')
		  ...
		  else
		    {
		      ...
		      if(strcmp(BU_Unit->Shifts.Type->CTsuperclass,"non")==0 && currResPos==0)
			{
			  while(currResPos!=BU_Unit->Shifts.Type->Anomeric)
			    {
			      currResPos++;
			    }
			}
...
Mols_MolSystem CC_MakeMoleculeFromBU_Struct(Impl_MemopsRoot *ccpnProject, struct BU_Struct *inp, char *name)
{
  ...
  for(unit=(struct BU_Unit *)inp->Units.Head.Succ; unit->Node.Succ!=NULL;
      unit=(struct BU_Unit *)unit->Node.Succ)
    {
	 ...
	 way. */
      if(strcmp(unit->Residue->Shifts.Type->CTsuperclass, "aglycan")==0)
	{
  ...
  */
  ...
  if(unit->Subst[2]!=NULL || unit->Subst[3]!=NULL)
      ...
      if(strcmp(unit->Residue->Node.Name,"Asn")==0)
	{
      ...
      /* FIXME: Are link:OG and link:OG1 missing in Ser and Thr chemComps now? */
			     ...
			     Mols_MolSystem molSystem, char *chainCode)
{
  ...
  ApiSet chains, residues, resonanceGroups, analProjects;
  ApiString string;
  ApiInteger seqId;
  ApiSetIterator residueIterator;
  Anal_AnalysisProject analProject;
  Mols_Chain chain;
  Mols_Residue residue;
  char ccpCode[128], name[128], molType[64];
  Nmr_ResonanceGroup resonanceGroup;

  analProject=Impl_MemopsRoot_GetCurrentAnalysisProject(*ccpnProject);
  if(!analProject || ApiObject_IsNone(analProject))
    {
      analProject=Nmr_NmrProject_GetAnalysisProject(*nmrProject);
      if(!analProject || ApiObject_IsNone(analProject))
	{
	  analProjects=Impl_MemopsRoot_GetAnalysisProjects(*ccpnProject);
	  if(ApiSet_Len(analProjects)>0)
	    {
	      analProject=ApiSet_Get(analProjects,0);
	      Impl_MemopsRoot_SetCurrentAnalysisProject(*ccpnProject, analProject);
	    }
	  ApiObject_Free(analProjects);
	}
    }
  if(!analProject || ApiObject_IsNone(analProject))
    {
      string=Nmr_NmrProject_GetName(*nmrProject);
      analProject=Anal_AnalysisProject_Init_reqd(*ccpnProject, ApiString_Get(string), *nmrProject);
      ApiObject_Free(string);
      if(!analProject || ApiObject_IsNone(analProject))
	{
  ...
  ApiObject_Free(residueIterator);
...
ApiSet CC_FindAtomsNamed(Mols_Residue residue, char *name, char *type, int substituentPosition)
{
  Mols_Atom atom;
  ApiSet atoms;
  ApiMap searchParameters;
  ApiString string;
  char ccpnAtomName[6], localType[8], localName[6], residueType[64];
  int i, j, ptr;
  ...
  string=Mols_Residue_GetCcpCode(residue);
  strcpy(residueType, ApiString_Get(string));
  ApiObject_Free(string);

  /* Handle protein names in a special way */
  if(strcmp(residueType, "Asn")==0 || strcmp(residueType, "Ser")==0)
    {
      if(strcmp(localName, "1")==0)
	{
	  strcpy(localName, "");
	}
      else if(strcmp(localName, "2")==0)
	{
	  strcpy(localName, "A");
	}
      else if(strcmp(localName, "3")==0)
	{
	  strcpy(localName, "B");
	}
      else if(strcmp(localName, "4")==0)
	{
	  strcpy(localName, "G");
	}
    }
  else if(strcmp(residueType, "Thr")==0)
    {
      if(strcmp(localName, "1")==0)
	{
	  strcpy(localName, "");
  ...
  else if(substituentPosition!=-1)
    {
      sprintf(localName, "%s_%d", localName, substituentPosition+1);
    }
  ...
  if(strcmp(localType,"H1")==0)
  ...
  else if(strcmp(localType,"H2")==0)
    {
      ...
      if(strcmp(residueType, "Asn")==0 || strcmp(residueType, "Ser")==0)
	{
	  sprintf(localName,"%s3", localName);
	}
      else
	{
	  sprintf(localName,"%s2",localName);
  ...
  if(type[0]=='H' && (localName[0]=='M' || (strcmp(residueType, "Thr")==0 && localName[0]=='G')))
    {
      ccpnAtomName[0]='H';
      j=1;
      if(localName[0]=='M')
	{
	  for(i=0;i<strlen(localName);j++,i++)
	    {
	      ccpnAtomName[j]=localName[i];
	      if(localName[i+1]=='_')
		{
		  j++;
		  ptr=j;
  ...
  if(!atoms || ApiSet_Len(atoms)<1)
    {
      Mols_Residue_CheckAllValid(residue, ApiBoolean_False());
      printRaisedException();
      atoms = Mols_Residue_GetAtoms(residue);
      ApiSetIterator atomIterator=ApiSet_Iterator(atoms);
      printf("Atom not found. Available atoms:\n");
      while ((atom=ApiSetIterator_Next(atomIterator)))
	{
	  printf("Atom: %s\n", ApiString_Get(Mols_Atom_GetName(atom)));
	}
      ApiObject_Free(atoms);
      atoms=0;
    }
...
ApiSet CC_FindShifts(Nmr_NmrProject *nmrProject, float value)
{
  ApiSet shiftLists=0;
  ApiMap apiShiftListClass;
  Nmr_ShiftList shiftList=0;
  int numShiftLists, lowDiffNumber;
  float lowDiff;
  ...
  return CC_FindShiftsInShiftList(shiftList, value);
}


ApiSet CC_FindShiftsInShiftList(Nmr_ShiftList shiftList, float value)
{
  ...
  Nmr_Shift shift=0;
  int i, j, numShifts, numPeakDims, lowDiffNumber=-1;
...
/* This function returns all shifts in the specified shiftList that link to the peakDim specified
   by wantedDim in a set of peaks */
ApiSet CC_FindShiftsFromPeaks(Nmr_ShiftList shiftList, ApiSet peaks, int wantedDim)
{
  ApiSet allShifts, matchingShifts, peakDims, shiftPeakDims;
  ApiSetIterator peakIterator, shiftIterator, peakDimIterator;
  ApiInteger dim;
  Nmr_Peak peak;
  Nmr_Shift shift=0;
  Nmr_PeakDim peakDim;
  ...
  while((shift=ApiSetIterator_Next(shiftIterator)))
      ...
      if(ApiObject_IsNone(shift) || Impl_MemopsObject_GetIsDeleted(shift)==ApiBoolean_True())
	{
	 ...
	 present in peakDims */
      shiftPeakDims=Nmr_Shift_FindAllPeakDims_keyval1(shift, "dim", dim);
...
int CC_AssignPeakToAtoms(Nmr_NmrProject *nmrProject, ApiSet atoms, float firstDim, float secondDim, int assignDim, const char mode)
{
  ApiSet shifts, atomSetCollection, resonanceSets;
  ApiSet dataSourceCollection, peakListCollection;
  ApiSet peakLists, peakDims, resonances;
  ApiSet peaks, shiftPeakDims, peakPeakDims, experiments, resonanceSetsInAtomSet;
  ApiSetIterator peakIterator, primShiftIterator, peakDimIterator, resonanceIterator,
  ...
  Nmr_ResonanceSet resonanceSet=0, resonanceSetAlt;
  Nmr_Peak peak;
  ...
  int unassignedShiftNr=999, assignedShiftNr=-1, numPeakPeakDims, numAtomSets;
  int numExperiments, numDataSources, numAtoms, numShiftPeakDims, numPeakDims=-1;
  char name[16];
  ...
  if(ApiSet_Len(resonanceSetsInAtomSet)>0)
	...
	{
	  resonances=Nmr_ResonanceSet_GetResonances(resonanceSet);
	  /*	  if(ApiSet_Len(resonances)==1)
	    {
	      targetResonance=ApiSet_Get(resonances,0);
	      }*/
	  /* Find the first resonance without a name */
	  if(ApiSet_Len(resonances)>0)
	    {
	      resonanceIterator=ApiSet_Iterator(resonances);
	      while((resonance=ApiSetIterator_Next(resonanceIterator)))
		{
		  string=Nmr_Resonance_GetDetails(resonance);
		  if(!string || ApiObject_IsNone(string))
		    {
	      ...
	      ApiObject_Free(resonanceIterator);
	    }
	  else
	    {
	      targetResonance=0;
  ...
  if(numPeaks==0)
    {
      printf("Could not find any peaks. Line %d.\n", __LINE__);
      return 0;
    }
  
  shifts=CC_FindShiftsFromPeaks(shiftList, peaks, ApiInteger_Get(apiIntDim[assignDim-1]));
  ...
  if(numShifts>0)
      ...
      for(i=0;i<numShifts;i++)
	{
	  ...
	  string=Nmr_Resonance_GetDetails(resonanceInShift);
	      ...
	      if(strlen(name)<=2)
		{
		...
		else if(name[strlen(name)-1]=='1' || name[strlen(name)-1]=='2')
		  {
		    ...
		    if(!string || ApiObject_IsNone(string))
		      {
			assignedShiftNr=i;
			targetResonance=resonanceInShift;
		      }
		    ApiObject_Free(string);
		    ApiObject_Free(resonance);
		  }
      ...
      else if(assignedShiftNr>=0)
	{
	  ...
	  {
	    ApiObject args[] = { targetResonance, NULL };
	  ...
	  string=ApiString_New(name);
	  Nmr_Resonance_SetDetails(targetResonance, string);
      ...
      if(unassignedShiftNr<999 && targetResonance && !ApiObject_IsNone(targetResonance))
	{
	  resonanceInShift=Nmr_Shift_GetResonance(shift);
  ...
  if((unassignedShiftNr>=999 && assignedShiftNr<0) || !shift || ApiObject_IsNone(shift))
      ...
      if(numPeakDims<=0 && numPeaks>0)
	{
	  ...
	  while((peak=ApiSetIterator_Next(peakIterator)))
	      ...
	      while((peakPeakDim=ApiSetIterator_Next(peakDimIterator)))
		{
		  ...
		  if(fabs(ApiFloat_Get(value)-*primDim)<ApiFloat_Get(tolerance))
		    {
      ...
      if(shift && !ApiObject_IsNone(shift))
	{
      ...
      if(targetResonance && !ApiObject_IsNone(targetResonance) && resonance && !ApiObject_IsNone(resonance))
	{
	  /*	  printf("Merging resonances\n");*/
	  /*	  mergeResonances(targetResonance, resonance);*/
	  {
	    ApiObject args[] = { resonance, targetResonance, NULL };
	    callModuleFunction("ccpnmr.analysis.util.CasperBasic", "mergeResonances", args);
	  }
  ...
  atomSetCollection=CC_GetAtomSetCollectionOfAtom(nmrProject, atom, residue);
...
ApiSet CC_GetAtomSetCollectionOfAtom(Nmr_NmrProject *nmrProject, Mols_Atom atom, Mols_Residue residue)
{
      ...
      for(j=0;j<numChemAtoms;j++)
	{
	  ...
	  if(atomAlt && !ApiObject_IsNone(atomAlt))
	      ...
	      if(!atomSetAlt || ApiObject_IsNone(atomSetAlt))
		{
		  ...
		  atomSetAlt=Nmr_NmrProject_NewAtomSet(*nmrProject, parameters);
		  ApiObject_Free(parameters);
		  ApiObject_Free(atoms);
		}
...
int CC_AssignSimulatedShifts(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, struct BU_Struct *str, Nmr_ShiftList shiftList)
{
  ...
  for (unit=(struct BU_Unit *)str->Units.Head.Succ;
      ...
      if(!residue || ApiObject_IsNone(residue))
	{
      ...
      if(strcasecmp("subst", unit->Residue->Shifts.Type->CTsuperclass)==0)
	{
	  substPos=unit->Position;
	}
      else
	{
	  substPos=-1;
      ...
      for(i=0; i<unit->Residue->Shifts.Type->HeavyCnt; i++)
	{
	  ...
	  cAtoms=CC_FindAtomsNamed(residue, unit->Residue->Shifts.Type->Atom[i].Label, "C", substPos);
	  if(!cAtoms || ApiSet_Len(cAtoms)<=0)
	    {
	      projectValid=0;
	      printRaisedException();
	      printf("No atoms found. Line %d.\n", __LINE__);
	      return 0;
	    }
	  ...
	  if(unit->Residue->Shifts.Type->Atom[i].HCnt==1)
	    {
	      hAtoms=CC_FindAtomsNamed(residue, unit->Residue->Shifts.Type->Atom[i].Label, "H", substPos);
	      if(!hAtoms || ApiSet_Len(hAtoms)<=0)
		{
		  projectValid=0;
		  printRaisedException();
		  printf("No atoms found. Line %d.\n", __LINE__);
	    ...
	    {
	      hAtoms=CC_FindAtomsNamed(residue, unit->Residue->Shifts.Type->Atom[i].Label, "H1", substPos);
	      if(!hAtoms || ApiSet_Len(hAtoms)<=0)
		{
		  projectValid=0;
		  printRaisedException();
	      ...
	      ApiObject_Free(hAtoms);
	      ApiObject_Free(shift);
	      hAtoms=CC_FindAtomsNamed(residue, unit->Residue->Shifts.Type->Atom[i].Label, "H2", substPos);
	      if(!hAtoms || ApiSet_Len(hAtoms)<=0)
		{
		  projectValid=0;
		  printRaisedException();
...
Nmr_Shift CC_CreateShiftAssignedToAtoms(Nmr_NmrProject *nmrProject, Nmr_ShiftList shiftList, float value, ApiSet atoms)
{
  ...
  ApiString string;
  ApiFloat apiValue;
  ApiSet atomSets, resonances, resonanceSetsInAtomSet, atomSetsInResonanceSet, shiftsInResonance, shiftsInShiftList;
  ApiSetIterator atomIterator, resonanceSetIterator, resonanceIterator, shiftInResonanceIterator;
  ApiMap parameters;
  char name[16];
  int numAtoms;
  ...
  if(ApiSet_Len(resonanceSetsInAtomSet)>0)
    {
      ...
      parameters=ApiMap_New();
      ApiMap_SetString(parameters, "details", name);
      resonance=Nmr_ResonanceSet_FindFirstResonance(resonanceSet, parameters);
      ApiObject_Free(parameters);
      ...
      if(ApiObject_IsNone(resonance))
	{
	  ...
	  if(ApiSet_Len(resonances)==1)
	    {
	      ...
	      /* If there are more atoms in the atomSet there should be more resonances. If this resonance
		 is already assigned to an atom create a new one instead. */
	      atomSetsInResonanceSet=Nmr_ResonanceSet_GetAtomSets(resonanceSet);
	      if(ApiSet_Len(atomSetsInResonanceSet)>1)
		{
		  string=Nmr_Resonance_GetDetails(resonance);
		  if(!ApiObject_IsNone(string))
		    {
		      resonance=0;
		      ApiObject_Free(string);
		    }
		}
	      ApiObject_Free(atomSetsInResonanceSet);
	    }
	  /* If there is more than one resonance find the first resonance without a name */
	  else if(ApiSet_Len(resonances)>1)
	    {
	      resonanceIterator=ApiSet_Iterator(resonances);
	      while((resonance=ApiSetIterator_Next(resonanceIterator)))
		{
      ...
      if(resonance)
	{
	  shiftsInResonance=Nmr_Resonance_FindAllShifts_keyval1(resonance, "parentList", shiftList);
	  shiftsInShiftList=CC_FindShiftsInShiftList(shiftList, value);
	  if(shiftsInResonance && shiftsInShiftList && !ApiObject_IsNone(shiftsInResonance) && !ApiObject_IsNone(shiftsInShiftList))
	    {
	      shiftInResonanceIterator=ApiSet_Iterator(shiftsInResonance);
	      while((shift=ApiSetIterator_Next(shiftInResonanceIterator)))
		{
		  if(ApiSet_Contains(shiftsInShiftList, shift))
		    {
		      ApiObject_Free(shiftsInResonance);
		      ApiObject_Free(shiftsInShiftList);
		      ApiObject_Free(resonance);
		      return shift;
		    }
		}
	    }
	  if(strlen(name)>2 && (name[2]=='1'||name[2]=='2'))
	    {
	      resonances=Nmr_ResonanceSet_GetResonances(resonanceSet);
	      if(ApiSet_Len(resonances)<2)
		{
		  resonance=0;
		}
		ApiObject_Free(resonances);
...
/* Sets the resonanceGroup of the resonance to the first resonanceGroup of
   the residue */
int CC_SetResonanceGroup(Nmr_NmrProject *nmrProject, Nmr_Resonance resonance, Mols_Residue residue)
{
  Nmr_ResonanceGroup resonanceGroup;
  ...
  if(numResonanceGroups>0)
    ...
    {
      parameters=ApiMap_New();
      resonanceGroup=Nmr_NmrProject_NewResonanceGroup(*nmrProject, parameters);
      ApiObject_Free(parameters);
...
int CC_LoadAssignments(Nmr_NmrProject *proj, struct BU_Struct *str)
{
  ...
  Mols_Chain chain;
  struct BU_Unit *unit;
  int i, j, atomSetCnt, atomNr, protonNumber, cAdded;
  char name[16];
      ...
      for(j=0;j<atomSetCnt;j++)
	{
	  ...
	  while((atom=ApiSetIterator_Next(atomIterator)))
	      ...
	      else if(name[0]=='H')
		{
		  ...
		  else if(name[2]=='2' && unit->Shifts.Type->Atom[atomNr].HCnt>1)
		    {
		      protonNumber=0;
		    }
		  else
		...
		case 'C':
		  ...
		  break;
		case 'H':
		  /* If the protonNumber is invalid */
		  while(protonNumber>unit->Shifts.Type->Atom[atomNr].HCnt-1 && protonNumber>0)
		    {
		      protonNumber--;
		    }
		  /* First check that this has not already been assigned (another resonance in the
  ...
  for(unit=(struct BU_Unit *)str->Units.Head.Succ; unit->Node.Succ!=NULL;
      ...
      for(i=0; i<unit->Shifts.Type->HeavyCnt; i++)
	{
	  ...
	  for(j=0; j<unit->Shifts.Type->Atom[i].HCnt; j++)
		...
		{
		  CC_PopulateShiftLists(proj, str, unit->Shifts.C[i], unit->Shifts.H[i][j], cAdded);
		  if(unit->Shifts.C[i]>0)
		    {
		      cAdded=1;
		    }
		}
	    }
	  if(unit->Shifts.Type->Atom[i].HCnt==0)
	    {
	      CC_PopulateShiftLists(proj, str, unit->Shifts.C[i], -1, cAdded);
	    }
	}
    }
...
void CC_PopulateShiftLists(Nmr_NmrProject *nmrProject, struct BU_Struct *str, float CShift, float HShift, int cAdded)
{
  ApiSet exps, shifts;
  Nmr_Experiment experiment;
  Nmr_DataSource spectrum;
  ...
  Nmr_ShiftList shiftList;
  Nmr_Peak peak;
  ApiInteger apiFirstDim, apiSecondDim;
  int numExps;

  apiFirstDim=ApiInteger_New(1);
  apiSecondDim=ApiInteger_New(2);
  ...
  if(!cAdded)
      ...
      if(numExps>0)
	{
	  ...
	  if(CC_GetProcSpectrumOfDim(&experiment, 1, &spectrum))
	    {
	      if(CC_GetFirstPeakList(&spectrum, &peakList))
		{
		  peak=CC_FindPeak(peakList, CShift, -1, 1, 0.00001, &apiFirstDim, &apiSecondDim);
		  if(peak && !ApiObject_IsNone(peak))
		    {
		      str->CShift[str->CCnt]=CShift;
		      str->CShiftAtom[str->CCnt].unit=0;
		      str->CShiftAtom[str->CCnt].atom[0]=-1;
		      str->CShiftAtom[str->CCnt++].atom[1]=-1;
		      ApiObject_Free(peak);
		    }
		  ApiObject_Free(peakList);
		}
	  ...
	  else
	    {
	      ...
	      if(shiftList && !ApiObject_IsNone(shiftList))
		{
		  shifts=CC_FindShiftsInShiftList(shiftList, CShift);
		  if(shifts && !ApiObject_IsNone(shifts))
		    {
		      if(ApiSet_Len(shifts)>0)
			{
  ...
  if(HShift>0)
	  ...
	  if(CC_GetProcSpectrumOfDim(&experiment, 1, &spectrum))
	    {
	      if(CC_GetFirstPeakList(&spectrum, &peakList))
		{
		  peak=CC_FindPeak(peakList, HShift, -1, 1, 0.00001, &apiFirstDim, &apiSecondDim);
		  if(peak && !ApiObject_IsNone(peak))
		    {
		      str->HShift[str->HCnt]=HShift;
		      str->HShiftAtom[str->HCnt].unit=0;
		      str->HShiftAtom[str->HCnt].atom[0]=-1;
		      str->HShiftAtom[str->HCnt++].atom[1]=-1;
		      ApiObject_Free(peak);
		    }
		  ApiObject_Free(peakList);
		}
	  ...
	  else
	    {
	      ...
	      if(shiftList && !ApiObject_IsNone(shiftList))
		{
		  shifts=CC_FindShiftsInShiftList(shiftList, HShift);
		  if(shifts && !ApiObject_IsNone(shifts))
		    {
		      if(ApiSet_Len(shifts)>0)
			{
      ...
      if(exps && !ApiObject_IsNone(exps))
	{
      ...
      exps = CC_GetExpOfType(nmrProject, "H[C]");
      numExps=ApiSet_Len(exps);
      /* For CH require processed spectra with a peak list. Just a shift list is not enough. */
      if(numExps>0)
	{
	  experiment = ApiSet_Get(exps,0);
	  if(CC_GetProcSpectrumOfDim(&experiment, 2, &spectrum))
	    {
	      if(CC_GetFirstPeakList(&spectrum, &peakList))
		{
		  peak=CC_FindPeak(peakList, CShift, HShift, 2, 0.00001, &apiFirstDim, &apiSecondDim);
		  if(peak && !ApiObject_IsNone(peak))
		    {
		      str->ChCShift[str->ChCnt]=CShift;
		      str->ChHShift[str->ChCnt]=HShift;
		      str->ChShiftAtom[str->ChCnt].unit=0;
		      str->ChShiftAtom[str->ChCnt].atom[0]=-1;
		      str->ChShiftAtom[str->ChCnt++].atom[1]=-1;
		      ApiObject_Free(peak);
		    }
		  ApiObject_Free(peakList);
		}
  ...
  ApiObject_Free(apiFirstDim);
  ApiObject_Free(apiSecondDim);
}

/* Merge shifts from the same shiftlist in a resonance */
void CC_MergeShiftsInResonance(Nmr_NmrProject *nmrProject, Nmr_Resonance resonance)
{
  ...
  {
    ApiObject args[] = { *nmrProject, NULL };
...
void CC_MergeShiftWithShiftsInResonance(Nmr_NmrProject *nmrProject, Nmr_Shift shift, Nmr_Resonance resonance)
{
  ApiSet shifts;
  Nmr_Shift secShift;
  Nmr_ShiftList shiftList, secShiftList;
...
int CC_CcpnPdb(Impl_MemopsRoot *ccpnProject, char *str, char *filename)
{
  ...
  molSystem=Impl_MemopsRoot_FindFirstMolSystem_keyval1(*ccpnProject, "name", name);
...
int CC_SetAllShiftErrors(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, struct BU_Struct *sim, struct BU_Struct *exp)
{
  ApiSet shiftLists, experiments, resonances, experimentalShifts, simulatedShifts;
  ApiSetIterator shiftListIterator, resonanceIterator;
  ApiMap parameters;
  ApiString shiftListName;
  ApiFloat apiFloatSimValue, apiFloatExpValue;
  Nmr_ShiftList shiftList, experimentalList=0, simulatedList;
  Nmr_Resonance resonance;
  Nmr_Shift simShift, expShift;
  int numShiftLists, nrExperiments, highestNrExperiments=0;
  char shiftListNameString[128], keyword[64], details[512];
  float totalError=0;
  Vald_ValidationStore store;
  Vald_NmrMeasurementListValidation shiftListValidation;
  Vald_NmrMeasurementValidation shiftValidation;
  
  CC_CreateKeyWordDefinitions(ccpnProject);
  store=CC_GetCasperValidationStore(ccpnProject, nmrProject);

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "className", "ShiftList");
  shiftLists=Nmr_NmrProject_FindAllMeasurementLists(*nmrProject,parameters);
  ApiObject_Free(parameters);

  sprintf(shiftListNameString, "CASPER simulated %s", sim->Node.Name);
  shiftListName=ApiString_New(shiftListNameString);
  simulatedList=Nmr_NmrProject_FindFirstMeasurementList_keyval1(Global_NmrProject, "name", shiftListName);
  ApiObject_Free(shiftListName);

  if(!shiftLists || ApiObject_IsNone(shiftLists))
    {
      printf("Error getting shiftLists. Line %d.\n", __LINE__);
      projectValid=0;
      return 0;
    }
  numShiftLists=ApiSet_Len(shiftLists);
  ...
  while((shiftList=ApiSetIterator_Next(shiftListIterator)))
    {
      if(shiftList!=simulatedList)
	{
	  experiments=Nmr_ShiftList_GetExperiments(shiftList);
	  nrExperiments=ApiSet_Len(experiments);
	  ApiObject_Free(experiments);
	  if(nrExperiments>highestNrExperiments)
	    {
	      if(experimentalList)
		{
		  ApiObject_Free(experimentalList);
		}
	      experimentalList=shiftList;
	      highestNrExperiments=nrExperiments;
	    }
	  else
	    {
	      ApiObject_Free(shiftList);
  ...
  ApiObject_Free(shiftListIterator);
  if(!experimentalList)
    {
      printf("Experimental shiftList not found. Line %d.\n", __LINE__);
    }
  ApiObject_Free(shiftLists);
  
  sprintf(keyword, "ShiftError");
  sprintf(details, "Difference between experimental chemical shift and CASPER simulated chemical shift. Error in ppm.");
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "context", "CASPER");
  ApiMap_SetString(parameters, "keyword", keyword);
  ApiMap_SetString(parameters, "details", details);

  resonances=Nmr_NmrProject_GetResonances(*nmrProject);
  resonanceIterator=ApiSet_Iterator(resonances);
  while((resonance=ApiSetIterator_Next(resonanceIterator)))
    {
      simulatedShifts=Nmr_Resonance_FindAllShifts_keyval1(resonance, "parentList", simulatedList);
      experimentalShifts=Nmr_Resonance_FindAllShifts_keyval1(resonance, "parentList", experimentalList);
      if(ApiSet_Len(simulatedShifts)==1 && ApiSet_Len(experimentalShifts)==1)
	{
	  simShift=ApiSet_Get(simulatedShifts, 0);
	  expShift=ApiSet_Get(experimentalShifts, 0);
	  apiFloatSimValue=Nmr_Shift_GetValue(simShift);
	  apiFloatExpValue=Nmr_Shift_GetValue(expShift);
	  ApiMap_SetFloat(parameters, "floatValue", ApiFloat_Get(apiFloatExpValue)-ApiFloat_Get(apiFloatSimValue));
	  ApiMap_SetItem(parameters, "nmrMeasurements", experimentalShifts);
	  shiftValidation=Vald_ValidationStore_NewNmrMeasurementValidation(store, parameters);
	  ApiObject_Free(shiftValidation);

	  totalError+=fabs(ApiFloat_Get(apiFloatExpValue)-ApiFloat_Get(apiFloatSimValue));
	  ApiObject_Free(apiFloatSimValue);
	  ApiObject_Free(apiFloatExpValue);
	  ApiObject_Free(simShift);
	  ApiObject_Free(expShift);
	}
      ApiObject_Free(simulatedShifts);
      ApiObject_Free(experimentalShifts);
      ApiObject_Free(resonance);
    }
  ApiObject_Free(resonanceIterator);
  ApiObject_Free(resonances);
  ...
  if(simulatedList && !ApiObject_IsNone(simulatedList) &&
     (sim->CCorrection>0 || sim->HCorrection>0))
    {
      shiftLists=ApiSet_New();
      ApiSet_Add(shiftLists, simulatedList);
      if(sim->CCorrection>0)
      {
	sprintf(keyword, "ShiftList13CCorrection");
	sprintf(details, "The 13C correction applied to the shiftList.");
	parameters=ApiMap_New();
	ApiMap_SetString(parameters, "context", "CASPER");
	ApiMap_SetString(parameters, "keyword", keyword);
	ApiMap_SetString(parameters, "details", details);
	ApiMap_SetFloat(parameters, "floatValue", sim->CCorrection);
	ApiMap_SetItem(parameters, "nmrMeasurementLists", shiftLists);
	shiftListValidation=Vald_ValidationStore_NewNmrMeasurementListValidation(store, parameters);
	printRaisedException();
	ApiObject_Free(parameters);
	ApiObject_Free(shiftListValidation);
      }
      if(sim->HCorrection>0)
      {
	sprintf(keyword, "ShiftList1HCorrection");
	sprintf(details, "The 1H correction applied to the shiftList.");
	parameters=ApiMap_New();
	ApiMap_SetString(parameters, "context", "CASPER");
	ApiMap_SetString(parameters, "keyword", keyword);
	ApiMap_SetString(parameters, "details", details);
	ApiMap_SetFloat(parameters, "floatValue", sim->HCorrection);
	ApiMap_SetItem(parameters, "nmrMeasurementLists", shiftLists);
	shiftListValidation=Vald_ValidationStore_NewNmrMeasurementListValidation(store, parameters);
	ApiObject_Free(parameters);
	ApiObject_Free(shiftListValidation);
  ...
  if(experimentalList && !ApiObject_IsNone(experimentalList) && 
     (exp->CCorrection>0 || exp->HCorrection>0))
    {
      shiftLists=ApiSet_New();
      ApiSet_Add(shiftLists, experimentalList);
      if(exp->CCorrection>0)
      {
	sprintf(keyword, "ShiftList13CCorrection");
	sprintf(details, "The 13C correction applied to the shiftList.");
	parameters=ApiMap_New();
	...
	ApiMap_SetString(parameters, "keyword", keyword);
	ApiMap_SetString(parameters, "details", details);
	ApiMap_SetFloat(parameters, "floatValue", exp->CCorrection);
	ApiMap_SetItem(parameters, "nmrMeasurementLists", shiftLists);
	shiftListValidation=Vald_ValidationStore_NewNmrMeasurementListValidation(store, parameters);
	ApiObject_Free(parameters);
	ApiObject_Free(shiftListValidation);
      }
      if(exp->HCorrection>0)
      {
	sprintf(keyword, "ShiftList1HCorrection");
	sprintf(details, "The 1H correction applied to the shiftList.");
	parameters=ApiMap_New();
	...
	ApiMap_SetString(parameters, "keyword", keyword);
	ApiMap_SetString(parameters, "details", details);
	ApiMap_SetFloat(parameters, "floatValue", exp->HCorrection);
	ApiMap_SetItem(parameters, "nmrMeasurementLists", shiftLists);
	shiftListValidation=Vald_ValidationStore_NewNmrMeasurementListValidation(store, parameters);
	ApiObject_Free(parameters);
	ApiObject_Free(shiftListValidation);
  ...
  ApiObject_Free(experimentalList);
  ApiObject_Free(simulatedList);
  ApiObject_Free(store);

  return 1;
}

/* FIXME: Currently this function sets the error to rms. If called multiple times (because of multiple chains) during assignment
   the PeakListError will be set to the error of the last chain rather than the whole peaklist. */
int CC_SetPeakListError(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, int mode, float value, float rms)
{
  ApiSet experiments, peakLists;
  ApiSetIterator experimentIterator;
  ApiMap parameters;
  Nmr_Experiment experiment;
  Nmr_DataSource spectrum;
  Nmr_PeakList peakList;
  Vald_ValidationStore store;
  Vald_PeakListValidation validation;
  int numDim, numExperiments;
  char type[32], keyword[64], details[512];
  ...
  experiments=CC_GetExpOfType(nmrProject, type);
  ...
  if(numExperiments>0)
    {
      experimentIterator=ApiSet_Iterator(experiments);
      peakList=0;
      while((experiment=ApiSetIterator_Next(experimentIterator)) && peakList==0)
	{
	  if(!CC_GetProcSpectrumOfDim(&experiment, numDim, &spectrum))
	    {
	      spectrum=0;
	    }
	  if(spectrum)
	...
	{
	  projectValid=0;
	  printf("Cannot find peakList in any of the %d experiments of type %c. Line %d.\n", numExperiments, mode, __LINE__);
	}
    }
  else
    {
      projectValid=0;
      printf("No experiments of type %c. Line %d.\n", mode, __LINE__);
      return 0;
    }
  store=CC_GetCasperValidationStore(ccpnProject, nmrProject);

  peakLists=ApiSet_New();
  ApiSet_Add(peakLists, peakList);

  sprintf(keyword, "%s_PeakListVal", type);
  sprintf(details, "Difference between experimental peak list and CASPER simulated peak list. Error in ppm.");
  if(mode==SP_ID_2D_CH)
    {
      sprintf(details, "%s Carbon is weighted by %f.", details, sqrt(SP_C_SCALING));
    }
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "context", "CASPER");
  ApiMap_SetString(parameters, "keyword", keyword);
  ApiMap_SetString(parameters, "details", details);
  ApiMap_SetFloat(parameters, "floatValue", value);
  ApiMap_SetItem(parameters, "peakLists", peakLists);
  validation=Vald_ValidationStore_NewPeakListValidation(store, parameters);

  if(!validation || ApiObject_IsNone(validation))
    {
      printf("Cannot create PeakListValidation. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
      return 0;
    }
  if(!Vald_PeakListValidation_CheckAllValid(validation, ApiBoolean_False()))
    {
      printf("PeakListValidation invalid. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
    }
  ApiObject_Free(validation);

  if(rms>=0)
    {
      sprintf(keyword, "%s_PeakListRmsVal", type);
      sprintf(details, "Root Mean Square difference between experimental and simulated peaks.");
      if(mode==SP_ID_2D_CH)
	{
	  sprintf(details, "%s Carbon is weighted by %f.", details, sqrt(SP_C_SCALING));	  
	}
      ApiMap_SetString(parameters, "keyword", keyword);
      ApiMap_SetString(parameters, "details", details);
      ApiMap_SetFloat(parameters, "floatValue", rms);
      validation=Vald_ValidationStore_NewPeakListValidation(store, parameters);
...
int CC_SetResidueError(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, int mode, Mols_Residue residue, float value)
{
  Vald_ValidationStore store;
  Vald_MolSysResidueValidation validation;
  ApiSet residues;
  ApiMap parameters;
  char keyword[64], details[512];

  residues = ApiSet_New();
  ApiSet_Add(residues, residue);

  switch(mode)
    {
    case 0:
      sprintf(keyword, "Residue_ExpectedError");
      sprintf(details, "The approximated expected 13C error (in ppm) of the residue.");
      break;
    case SP_ID_CARBON:
      sprintf(keyword, "Residue_13CError");
      sprintf(details, "The total difference in ppm between experimental and simulated 13C shifts in the residue.");
      break;
    case SP_ID_PROTON:
      sprintf(keyword, "Residue_1HError");
      sprintf(details, "The total difference in ppm between experimental and simulated 1H shifts in the residue.");
      break;
    default:
      return 0;
    }
  ...
  ApiMap_SetString(parameters, "details", details);
  ApiMap_SetFloat(parameters, "floatValue", value);
  ApiMap_SetItem(parameters, "residues", residues);
  /*  validation=Vald_ValidationStore_NewMolSysResidueValidation(store, parameters);

  if(!validation || ApiObject_IsNone(validation))
    {
      printf("Cannot create ResidueValidation. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
      return 0;
    }
  if(!Vald_MolSysResidueValidation_CheckAllValid(validation, ApiBoolean_False()))
    {
      printf("ResidueValidation invalid. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
    }
...
int CC_SetStructureError(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject, Mols_MolSystem *molSystem, char *chainCode, int mode, float value)
{
  Vald_ValidationStore store;
  Vald_MolSysChainValidation validation;
  ApiSet chains;
  ApiMap parameters;
  Mols_Chain chain;
  ApiString apiStr;
  ...
  apiStr=ApiString_New(chainCode);
  chain=Mols_MolSystem_FindFirstChain_keyval1(*molSystem, "code", apiStr);
  ApiObject_Free(apiStr);

  chains=ApiSet_New();
  ApiSet_Add(chains, chain);

  switch(mode)
    {
    case 0:
      sprintf(keyword, "Chain_ExpectedError");
      sprintf(details, "The approximated expected 13C error (in ppm) of the chain.");
      break;
    case SP_ID_CARBON:
      sprintf(keyword, "Chain_13CError");
      sprintf(details, "The total difference in ppm between experimental and simulated 13C shifts in the chain.");
      break;
    case SP_ID_PROTON:
      sprintf(keyword, "Chain_1HError");
      sprintf(details, "The total difference in ppm between experimental and simulated 1H shifts in the chain.");
      break;
    default:
      return 0;
    }
  ...
  ApiMap_SetString(parameters, "details", details);
  ApiMap_SetFloat(parameters, "floatValue", value);
  ApiMap_SetItem(parameters, "chains", chains);

  /*  validation=Vald_ValidationStore_NewMolSysChainValidation(store, parameters);

  if(!validation || ApiObject_IsNone(validation))
    {
      printf("Cannot create ChainValidation. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
      return 0;
    }
  if(!Vald_MolSysChainValidation_CheckAllValid(validation, ApiBoolean_False()))
    {
      printf("ChainValidation invalid. Line %d.\n", __LINE__);
      printRaisedException();
      projectValid=0;
    }
...
Vald_ValidationStore CC_GetCasperValidationStore(Impl_MemopsRoot *ccpnProject, Nmr_NmrProject *nmrProject)
{
  Vald_ValidationStore store;
  Meth_Software software;
  ApiMap parameters;

  software=CC_GetCasperSoftwareParameters(ccpnProject);

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "name", "CASPER results");
  ApiMap_SetItem(parameters, "nmrProject", *nmrProject);

  store=Impl_MemopsRoot_FindFirstValidationStore(*ccpnProject, parameters);

  if(!store || ApiObject_IsNone(store))
    {
      ApiMap_SetItem(parameters, "software", software);
      store=Impl_MemopsRoot_NewValidationStore(*ccpnProject, parameters);
      if(!store || ApiObject_IsNone(store))
	{
	  if(!store || ApiObject_IsNone(store))
	    {
	      printf("Cannot create ValidationStore. Line %d.\n", __LINE__);
	      printRaisedException();
	    }
	}
    }

  ApiObject_Free(software);
  ApiObject_Free(parameters);

  return store;
}

Meth_Software CC_GetCasperSoftwareParameters(Impl_MemopsRoot *ccpnProject)
{
  Meth_Software sw;
  Meth_MethodStore store;
  ApiString string;
  ApiMap parameters;

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "name", "CASPER");

  store=Impl_MemopsRoot_FindFirstMethodStore(*ccpnProject, parameters);

  if(!store || ApiObject_IsNone(store))
    {
      store=Impl_MemopsRoot_NewMethodStore(*ccpnProject, parameters);
      if(!store || ApiObject_IsNone(store))
	{
	  printf("Cannot create MethodStore. Line %d.\n", __LINE__);
	  printRaisedException();
	}
    }

  sw=Meth_MethodStore_FindFirstSoftware(store, parameters);
  /*  sprintf(versionString, "%s", FN_VERSION);*/
  if(!sw || ApiObject_IsNone(sw))
    {
      ApiMap_SetString(parameters, "version", FN_VERSION);
      sw=Meth_MethodStore_NewSoftware(store, parameters);
      if(!sw || ApiObject_IsNone(sw))
	{
	  printf("Cannot create Software. Line %d.\n", __LINE__);
	  printRaisedException();
	}
    }
  else
    {
      /* Set the version to the current version number - even if CASPER existed as software before */
      string=ApiString_New(FN_VERSION);
      Meth_Software_SetVersion(sw, string);
      ApiObject_Free(string);
    }
...
int CC_CreateKeyWordDefinitions(Impl_MemopsRoot *ccpnProject)
{
  ApiMap parameters;
  Kwdf_KeywordDefinitionStore store;
  Kwdf_KeywordDefinition definition;

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "context", "CASPER");
  
  store=Impl_MemopsRoot_FindFirstKeywordDefinitionStore(*ccpnProject, parameters);
  if(!store || ApiObject_IsNone(store))
    {
      store=Impl_MemopsRoot_NewKeywordDefinitionStore(*ccpnProject, parameters);
    }
  ApiObject_Free(parameters);

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "C_PeakListVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The difference between simulated and experimental 13C peak lists.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
      printRaisedException();
    }
  ApiObject_Free(definition);
  ApiObject_Free(parameters);

  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "C_PeakListRmsVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The Root Mean Square difference between simulated and experimental 13C peaks.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "H_PeakListVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The difference between simulated and experimental 1H peak lists.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  ApiObject_Free(definition);
  ApiObject_Free(parameters);
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "H_PeakListRmsVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The Root Mean Square difference between simulated and experimental 1H peaks.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "H[C]_PeakListVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The difference between simulated and experimental HSQC or HETCOR peak lists.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "H[C]_PeakListRmsVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The Root Mean Square difference between simulated and experimental (calculated as assigned) HSQC (or HETCOR) peaks.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "H_H.TOCSY_PeakListVal");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The difference between simulated and experimental TOCSY peak lists.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Residue_ExpectedError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The expected error (in ppm). This depends on the quality of the underlying data when simulating a structure.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Residue_13CError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The total difference (in ppm) between the simulated and experimental 13C chemical shifts of this residue.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Residue_1HError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The total difference (in ppm) between the simulated and experimental 1H chemical shifts of this residue.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Chain_ExpectedError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The expected error (in ppm). This depends on the quality of the underlying data when simulating a structure.");
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Chain_13CError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The total difference (in ppm) between the simulated and experimental 13C chemical shifts of this chain.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "Chain_1HError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The total difference (in ppm) between the simulated and experimental 1H chemical shifts of this chain.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "ShiftError");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The shift error (in ppm) compared to the simulated shift.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "ShiftList13CCorrection");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The 13C correction applied to the shiftList.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
  ...
  parameters=ApiMap_New();
  ApiMap_SetString(parameters, "keyword", "ShiftList1HCorrection");
  definition=Kwdf_KeywordDefinitionStore_FindFirstKeywordDefinition(store, parameters);
  if(!definition || ApiObject_IsNone(definition))
    {
      ApiMap_SetString(parameters, "explanation", "The 1H correction applied to the shiftList.");
      definition=Kwdf_KeywordDefinitionStore_NewKeywordDefinition(store, parameters);
    }
  if(!Kwdf_KeywordDefinitionStore_CheckAllValid(definition, ApiBoolean_False()))
    {
