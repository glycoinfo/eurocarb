...
int RE_SetDefault()
{
  ...
  do {
    ...
    if ( defres->Shifts.Type != RE_Residue->Shifts.Type && !TY_FindConversion(defres->Shifts.Type, RE_Residue->Shifts.Type) )
      {
	status=1;
      }
    /* optional accuracy */
...
int RE_CpyDefault()
{
  ...
  if(src_res->Shifts.Type != RE_Residue->Shifts.Type)
    {
