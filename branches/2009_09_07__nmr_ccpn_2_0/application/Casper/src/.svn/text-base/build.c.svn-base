...
void BU_ClearStruct(struct BU_Struct *structure)
{
  ...
  */
  ...
  structure->anomericInvertedStructure=0;
}
...
void CopyStructInfo(struct BU_Struct *a, struct BU_Struct *b)
{
  ...
  /*  b->anomericInvertedStructure=a->anomericInvertedStructure;*/ /* TODO: Consider if this should be copied */
}
...
int BU_PrintStruct3DLink()
{
  ...
  var=(struct VA_Variable *)FindNode(&(VariableList.Head), "UseCarbBank");

  if (var==NULL)
    {
      var=(struct VA_Variable *)ME_CreateNode(&VariableMethod,&VariableList, "UseCarbBank");
      varSetLocally=1;
    }
...
int BU_SetStruct()
{
  ...
  while(BU_Struct)
      ...
      if(!multiBuildFlag && PA_Result.String[0]!='/')
	{
      ...
      SP_CalcChSpectra(BU_Struct);
      SP_CalcHhSpectra(BU_Struct);
      
      SP_Sort2DSpectra(BU_Struct);

      BU_SortDoubleProtons(BU_Struct);
...
struct RE_Residue *BU_GetResidue(char *string)
{
  ...
  int i=0, tmp, carbBank=0;
  struct VA_Variable *useCarbBank;
  struct RE_Residue *res;

  useCarbBank=(struct VA_Variable *)FindNode(&(VariableList.Head), "UseCarbBank");
  if(useCarbBank!=NULL && useCarbBank->Value.Value.Float>0)
    {
      carbBank=1;
    }
  ...
  if(!carbBank)
    {
      while (i<NODE_NAME_LENGTH && isalnum(string[i]) )
	{
	  name[i]=string[i];
	  i++;
	}
    ...
    {
      while(i<NODE_NAME_LENGTH && (isalnum(string[i]) || string[i]=='-'))
	{
	  name[i]=string[i];
	  i++;
	}
      if(i+7<NODE_NAME_LENGTH && string[i]=='(' && isdigit(string[i+1]) &&
	 string[i+2]=='-' && string[i+3]=='1' && string[i+4]==')' &&
	 string[i+5]=='-' && string[i+6]=='O' && string[i+7]=='M' &&
      ...
      if(name[i-1]=='-')
	{
      ...
      name[i]=0;
      for(res=(struct RE_Residue *)ResidueList.Head.Succ; res->Node.Succ!=NULL;
	  res=(struct RE_Residue *)res->Node.Succ)
	{
	  if(strcasecmp(name,res->CarbBank)==0)
	    {
	      return res;
	    }
	}
...
struct BU_Unit *BU_CreateLNUnit()
{
  ...
  if (*BULN_Pos==']')
    {
      while(BULN_Pos>BULN_String && *BULN_Pos!='[')
	{
	  BULN_Pos--;
	}
  ...
  if(!carbBank)
    {
      while(BULN_Pos>=BULN_String && isalnum(*BULN_Pos))
	{
	  BULN_Pos--;
	}
    }
    ...
    {
      while(BULN_Pos>=BULN_String && (isalnum(*BULN_Pos) || *BULN_Pos=='-'))
	{
	  BULN_Pos--;
	}
      if(*(BULN_Pos+1)=='-')
...
struct BU_Unit *BU_Backbone(struct BU_Unit *current)
{
  ...
  if(useCarbBank!=NULL && useCarbBank->Value.Value.Float>0)
    {
      carbBank=1;
      if(*BULN_Pos=='-' && BULN_Pos>BULN_String)
	{
	  BULN_Pos--;  
	}
    }
  ...
  if(carbBank && *BULN_Pos=='-' && BULN_Pos>BULN_String)
    {
      BULN_Pos--;  
    }
...
int BU_LineNotation()
{
  ...
  /* If using CarbBank style input ignore -(x-1)-OH as reducing end type */
  if(carbBank)
    {
      if(*BULN_Pos=='H' && BULN_Pos>BULN_String+1 && *(BULN_Pos-1)=='O')
	{
	  BULN_Pos-=8;
	  *BULN_Pos=0;
	  BULN_Pos--;
	}
      else if((*BULN_Pos=='E' || *BULN_Pos=='e') && BULN_Pos>BULN_String+2 &&
	      *(BULN_Pos-1)=='M' && *(BULN_Pos-2)=='O')
	{
	  BULN_Pos-=10;
	}
    }
...
int BU_CopyStructure(struct BU_Struct *exp, struct BU_Struct *sim)
{
  ...
  for (unit=(struct BU_Unit*)sim->Units.Head.Succ; unit->Node.Succ!=NULL;
       unit=(struct BU_Unit *)unit->Node.Succ)
    {
      ...
      for (pos=0; pos<unit->Shifts.Type->HeavyCnt; pos++)	/* copy every 'substituent' */
	{
...
void BU_RemoveOrphanUnits(struct BU_Struct *str)
{
  ...
  while(numberResiduesInStructure<numberUnits && unit->Node.Succ!=NULL)
	...
	{
	  numberUnits--;
	  unit=FreeNode((struct Node *)unit);
	}
      else
	{
	  unit=(struct BU_Unit *)unit->Node.Succ;
...
/* In CASPER the first proton in a proton pair has lower shift. Sometimes glycosylation shifts and
   corrections can make them switch. Therefore they have to be sorted */
void BU_SortDoubleProtons(struct BU_Struct *str)
{
  struct BU_Unit *unit;
  int i, j;
  float temp;
  ...
  for (unit=(struct BU_Unit *)str->Units.Head.Succ;
       ...
       unit=(struct BU_Unit *)unit->Node.Succ)
    {
      for(i=0; i<unit->Shifts.Type->HeavyCnt; i++)
	{
	  if(unit->Shifts.Type->Atom[i].HCnt==2)
	    {
	      if(unit->Shifts.H[i][0]==BU_VOID_SHIFT ||
		 (unit->Shifts.H[i][0]>unit->Shifts.H[i][1] && 
		  unit->Shifts.H[i][1]!=BU_VOID_SHIFT))
		{
		  temp=unit->Shifts.H[i][0];
		  unit->Shifts.H[i][0]=unit->Shifts.H[i][1];
		  unit->Shifts.H[i][1]=temp;
		}
	    }
	}
    }
  for(i=0; i<str->HCnt; i++)
    {
      if(str->HShiftAtom[i].atom[1]==1)
	{
	  for(j=0; j<str->HCnt; j++)
	    {
	      if(j!=i && str->HShiftAtom[j].unit==str->HShiftAtom[i].unit && 
		 str->HShiftAtom[j].atom[0]==str->HShiftAtom[i].atom[0] &&
		 str->HShiftAtom[j].atom[1]==0)
		{
		  if(j<i)
		    {
		      str->HShiftAtom[i].atom[1]=0;
		      str->HShiftAtom[j].atom[1]=1;
		    }
		}
	    }
	}
  ...
  for(i=0; i<str->ChCnt; i++)
    {
      if(str->ChShiftAtom[i].atom[1]==1)
	{
	  for(j=0; j<str->ChCnt; j++)
	    {
	      if(j!=i && str->ChShiftAtom[j].unit==str->ChShiftAtom[i].unit && 
		 str->ChShiftAtom[j].atom[0]==str->ChShiftAtom[i].atom[0] &&
		 str->ChShiftAtom[j].atom[1]==0)
		{
		  if(j<i)
		    {
		      str->ChShiftAtom[i].atom[1]=0;
		      str->ChShiftAtom[j].atom[1]=1;
		    }
		}
	    }
	}
  ...
  for(i=0; i<str->HhCnt; i++)
    {
      if(str->HhHiShiftAtom[i].atom[1]==1)
	{
	  for(j=0; j<str->HhCnt; j++)
	    {
	      if(j!=i && str->HhHiShiftAtom[j].unit==str->HhHiShiftAtom[i].unit && 
		 str->HhHiShiftAtom[j].atom[0]==str->HhHiShiftAtom[i].atom[0] &&
		 str->HhHiShiftAtom[j].atom[1]==0)
		{
		  if(j<i)
		    {
		      str->HhHiShiftAtom[i].atom[1]=0;
		      str->HhHiShiftAtom[j].atom[1]=1;
		    }
		}
	    }
	}
      if(str->HhLoShiftAtom[i].atom[1]==1)
	{
	  for(j=0; j<str->HhCnt; j++)
	    {
	      if(j!=i && str->HhLoShiftAtom[j].unit==str->HhLoShiftAtom[i].unit && 
		 str->HhLoShiftAtom[j].atom[0]==str->HhLoShiftAtom[i].atom[0] &&
		 str->HhLoShiftAtom[j].atom[1]==0)
		{
		  if(j<i)
		    {
		      str->HhLoShiftAtom[i].atom[1]=0;
		      str->HhLoShiftAtom[j].atom[1]=1;
		    }
		}
	    }
	}
