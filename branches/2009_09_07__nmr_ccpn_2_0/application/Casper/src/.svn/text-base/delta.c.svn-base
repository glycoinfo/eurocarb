			     ...
			     struct DE_Delta *source)
{
  ...
  if(nres->Shifts.Type != source->Shifts[DE_NONRED].Type)
    {
  ...
  if(rres->Shifts.Type != source->Shifts[DE_REDUCING].Type)
    {
			      ...
			      unsigned char rpos, struct RE_Residue *rres)
{
  ...
  if (delta)
    {
      /* If this is a copy of another delta it has already been created in
	 this function and decided to be the best alternative. */
      if(delta->Source)
	{
	  return(delta);	/* found ! */
	}
  ...
  for (alt=(struct RE_Default *)rres->Defaults[rpos].Head.Succ;
       alt->Node.Succ!=NULL; alt=(struct RE_Default *)alt->Node.Succ)
    {
      if(rres->Shifts.Type != alt->Residue->Shifts.Type)
	{
  ...
  for (alt=(struct RE_Default *)nres->Defaults[npos].Head.Succ;
	...
	{
	  if(rres->Shifts.Type != alt2->Residue->Shifts.Type)
	    {
	      conv=(struct TY_Conversion *)
		TY_FindConversion(rres->Shifts.Type, alt2->Residue->Shifts.Type);
	      if (conv==NULL)
		{
		  continue;	/* Can't convert */
		}
	  ...
	  temp=(struct DE_Delta *)
	    FindNode(&(alt2->Residue->Delta[pos].Head),alt->Node.Name);
