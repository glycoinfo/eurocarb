...
int PE_AreSameUnit(struct BU_Unit *first_u, struct BU_Unit *second_u)
{
  ...
  for (pos=0; pos<first_u->Shifts.Type->HeavyCnt; pos++)
    {
...
int PE_AreSameStruct(struct BU_Struct *first, struct BU_Struct *second)
{
  ...
  for (second_u=(struct BU_Unit *)second->Units.Head.Succ;
	    ...
	    first_u=(struct BU_Unit *)first_u->Node.Succ)
	{
      ...
      if (flag==FALSE)
	{
	  printf("DEBUG: Reached second loop in PE_AreSameStruct.\n");
	  return(FALSE);
	}
    }
  return(TRUE);
      ...
      while (second->Node.Succ!=NULL)
	{
	  if (PE_AreSameStruct(first,second))
	      ...
	      /*	      if(fabs(second->ChFit-6.24)<0.01)
		{
	      ...
	      */
	      if(second->anomericInvertedStructure)
		{
		  ME_RemoveNode(&StructMethod,(struct Node *)second->anomericInvertedStructure);
  ...
  while (SE_Simulation.StrActual>SE_Simulation.StrLower)
    {
      ...
      if(last->anomericInvertedStructure)
	{
	  ME_RemoveNode(&StructMethod,(struct Node *)last->anomericInvertedStructure);
	  SE_Simulation.StrActual--;	  
	}
...
int PE_Compare(const struct BU_Struct *str1, const struct BU_Struct *str2, int mode)
{
  ...
  switch (mode)
    {
    case SE_PURGE_C:
      if(str1->anomericInvertedStructure && str1->anomericInvertedStructure!=str2)
	{
	  fit1=(str1->CFit+str1->anomericInvertedStructure->CFit)/2;
	}
      else
	{
	  fit1=str1->CFit;
	}
      if(str2->anomericInvertedStructure && str2->anomericInvertedStructure!=str1)
	{
	  fit2=(str2->CFit+str2->anomericInvertedStructure->CFit)/2;
	}
      else
	{
	  fit2=str2->CFit;
    ...
    case SE_PURGE_H:
      if(str1->anomericInvertedStructure && str1->anomericInvertedStructure!=str2)
	{
	  fit1=(str1->HFit+str1->anomericInvertedStructure->HFit)/2;
	}
      else
	{
	  fit1=str1->HFit;
      ...
      if(str2->anomericInvertedStructure && str2->anomericInvertedStructure!=str1)
	{
	  fit2=(str2->HFit+str2->anomericInvertedStructure->HFit)/2;
	}
      else
	{
	  fit2=str2->HFit;
    ...
    case SE_PURGE_CH:
      if(str1->anomericInvertedStructure && str1->anomericInvertedStructure!=str2)
	{
	  fit1=(str1->ChFit+str1->anomericInvertedStructure->ChFit)/2;
	}
      else
	{
	  fit1=str1->ChFit;
      ...
      if(str2->anomericInvertedStructure && str2->anomericInvertedStructure!=str1)
	{
	  fit2=(str2->ChFit+str2->anomericInvertedStructure->ChFit)/2;
	}
      else
	{
	  fit2=str2->ChFit;
    ...
    case SE_PURGE_HH:
      if(str1->anomericInvertedStructure && str1->anomericInvertedStructure!=str2)
	{
	  fit1=(str1->HhFit+str1->anomericInvertedStructure->HhFit)/2;
	}
      else
	{
	  fit1=str1->HhFit;
      ...
      if(str2->anomericInvertedStructure && str2->anomericInvertedStructure!=str1)
	{
	  fit2=(str2->HhFit+str2->anomericInvertedStructure->HhFit)/2;
	}
      else
	{
	  fit2=str2->HhFit;
    ...
    case SE_PURGE_TOT:
      if(str1->anomericInvertedStructure && str1->anomericInvertedStructure!=str2)
	{
	  fit1=(str1->TotFit+str1->anomericInvertedStructure->TotFit)/2;
	}
      else
	{
	  fit1=str1->TotFit;
      ...
      if(str2->anomericInvertedStructure && str2->anomericInvertedStructure!=str1)
	{
	  fit2=(str2->TotFit+str2->anomericInvertedStructure->TotFit)/2;
	}
      else
	{
	  fit2=str2->TotFit;
  ...
  if (fit1==fit2)
    {
      return(strcmp(str1->Node.Name, str2->Node.Name));
    }
  return(1);
}
...
int PE_MkStruct()
{
  ...
  struct BU_Unit *unit, *unit1, *unit2;
  struct BU_Struct *structure;
  struct BU_Struct *anomericInvertedStructure=0;
  struct RE_Residue *residue;
  struct SE_Unit *setupUnit;
  struct SE_Residue *setupResidue;
  int i_SP;
  char unit_name[BU_INFO_SIZE];
  char residue_name[RE_INFO_LEN];
  int pos1,pos2,anomerPos;
  int b_SP;
  int state, remove, freeRedEnd;
  int i, item, bond;
  float fit;
  ...
  /* TODO: Check if this structure already exists? */
  ...
  /* If the anomeric end is free generate the other anomeric configuration as well. */
  if(SE_Simulation.FreeAnomericEnd)
    {
      sprintf(str_name,"%.16s%d",SE_Simulation.Name,SE_Simulation.StrCnt);
      anomericInvertedStructure=(struct BU_Struct *)
	ME_CreateNode(&StructMethod, &StructList, str_name);
      if (anomericInvertedStructure==NULL) Error(PA_ERR_FATAL,"Out of memory");
      SE_Simulation.StrCnt++;
      if(BU_CopyStructure(anomericInvertedStructure, structure)!=PA_ERR_OK)
	{
	  remove=TRUE;
	  ME_RemoveNode(&StructMethod, (struct Node *)structure);
	  ME_RemoveNode(&StructMethod, (struct Node *)anomericInvertedStructure);
	  SE_Simulation.StrCnt-=2;
	  Error(PA_ERR_FAIL, "Could not copy structure");
	}
      freeRedEnd=0;
      for(unit=(struct BU_Unit *)anomericInvertedStructure->Units.Head.Succ;
	  unit->Node.Succ!=NULL; unit=(struct BU_Unit *)unit->Node.Succ)
	{
	  anomerPos=BU_ANOMER_POS(unit);
	  if(anomerPos!=-1)
	    {
	      if(!unit->Subst[anomerPos])
		{
		  freeRedEnd=1;
		  strcpy(residue_name, unit->Residue->Node.Name);
		  /* To avoid making duplicate entries (which would later be removed) make sure only
		     configuration 'a' is used to generate 'b' and not the other way around. */
		  if(residue_name[0]=='b')
		    {
		      remove=TRUE;
		      ME_RemoveNode(&StructMethod, (struct Node *)structure);
		  ...
		  else if(residue_name[0]=='a')
		    {
		      residue_name[0]='b';
		      setupUnit=(struct SE_Unit *)FindNode(&(SE_Simulation.Units.Head), unit->Node.Name);
		      if(!setupUnit)
			{
			  Error(PA_ERR_FAIL, "Cannot find unit");
			}
		      setupResidue=(struct SE_Residue *)FindNode(&(setupUnit->Residues.Head), residue_name);
		      if(!setupResidue)
			{
			  break;
			}
		      switch(unit->Residue->JCH)
			{
			case RE_SMALL:	anomericInvertedStructure->JCH[RE_SMALL]--;
			  break;
			case RE_MEDIUM:	anomericInvertedStructure->JCH[RE_MEDIUM]--;
			  break;
			case RE_LARGE:	anomericInvertedStructure->JCH[RE_LARGE]--;
			  break;
			}
		      switch (unit->Residue->JHH)
			{
			case RE_SMALL:	anomericInvertedStructure->JHH[RE_SMALL]--;
			  break;
			case RE_MEDIUM:	anomericInvertedStructure->JHH[RE_MEDIUM]--;
			  break;
			case RE_LARGE:	anomericInvertedStructure->JHH[RE_LARGE]--;
			  break;
			}
		      residue=setupResidue->Base;
		      unit->Residue=residue;
		      switch (residue->JCH)
			{
			case RE_SMALL:	anomericInvertedStructure->JCH[RE_SMALL]++;
			...
			case RE_LARGE:	anomericInvertedStructure->JHH[RE_LARGE]++;
		      ...
		      if(ListLen(&SE_Simulation.Rules)>0 && !RU_MatchRules(&SE_Simulation,anomericInvertedStructure))
			{
			  ME_RemoveNode(&StructMethod, (struct Node *)anomericInvertedStructure);
			  anomericInvertedStructure=0;
			  SE_Simulation.StrCnt--;	  
			  break;
			}
		      
		      state=CA_CalcAllGlycosylations(anomericInvertedStructure);
		      ...
		      if(state!=PA_ERR_OK)
			{
		      ...
		      anomericInvertedStructure->anomericInvertedStructure=structure;
		      structure->anomericInvertedStructure=anomericInvertedStructure;
		    }
		  break;
		}
	    }
      ...
      /* If there is no free reducing end remove the structure with inverted anomeric configuration. */
      if(!freeRedEnd)
	{
	  ME_RemoveNode(&StructMethod, (struct Node *)anomericInvertedStructure);
	  anomericInvertedStructure=0;
  ...
  if((SE_Simulation.Experimental->CCnt!=0||
      ...
      SE_Simulation.Criteria==SE_PURGE_TOT))
    {
	 ...
	 structure cannot be correct */
      if(!anomericInvertedStructure &&
	 (SE_Simulation.Experimental->CCnt > structure->CCnt ||
	  SE_Simulation.Experimental->HCnt > structure->HCnt))
	{
	  ME_RemoveNode(&StructMethod, (struct Node *)structure);
	  return(PA_ERR_OK);
	}
      if(structure->CCnt==0 && structure->HCnt==0)
	{
      ...
      else
	{
      ...
      if(anomericInvertedStructure)
	{
	  SP_Calc1DSpectra(anomericInvertedStructure);
	  if(anomericInvertedStructure->CCnt==0 && anomericInvertedStructure->HCnt==0)
	    {
	      if(SE_Simulation.Criteria==SE_PURGE_C)
		{
		  anomericInvertedStructure->CFit=9999;
		}
	      else if(SE_Simulation.Criteria==SE_PURGE_H)
		{
		  anomericInvertedStructure->HFit=9999;
		}
	      else
		{
		  anomericInvertedStructure->CFit=9999;
	    ...
	    {
	      SP_RawCompare(SE_Simulation.Experimental, anomericInvertedStructure, SE_Simulation.Criteria);
	    }
	}
    }
  ...
  if(SE_Simulation.Experimental->ChCnt!=0&&
     (SE_Simulation.Criteria==SE_PURGE_CH||
      SE_Simulation.Criteria==SE_PURGE_TOT))
    {
      SP_CalcChSpectra(structure); /* Calculate only CH here */
      SP_Sort2DSpectra(structure);

      /* If there are more experimental shifts than simulated shifts the
	 structure cannot be correct. If the reducing end is free (i.e.
	 if there is an anomericInvertedStructure) allow more experimental
	 than simulated shifts. */
      if(SE_Simulation.Experimental->ChCnt > structure->ChCnt && !anomericInvertedStructure)
	{
	  ME_RemoveNode(&StructMethod, (struct Node *)structure);
	  return(PA_ERR_OK);
	}
      ...
      else
	{
      ...
      if(anomericInvertedStructure)
	{
	  SP_CalcChSpectra(anomericInvertedStructure);
	  SP_Sort2DSpectra(anomericInvertedStructure);
	  if(anomericInvertedStructure->ChCnt==0)
	    {
	      anomericInvertedStructure->ChFit=9999;
	    }
	  else
	    {
	      SP_Raw2DCompare(SE_Simulation.Experimental, anomericInvertedStructure, SE_Simulation.Criteria);
	    }
	}
    }
  ...
  /* If there are already more structures than wanted and this is worse than the worst don't add it.
     This means the lists won't have to be resorted. */
  if(SE_Simulation.StrActual>SE_Simulation.StrLower)
      ...
      if(SE_Simulation.Criteria == SE_PURGE_C)
	{
	  if(anomericInvertedStructure)
	    {
	      fit=(structure->CFit+anomericInvertedStructure->CFit)/2;
	    }
	  else
	    {
	      fit=structure->CFit;
      ...
      else if(SE_Simulation.Criteria == SE_PURGE_H)
	{
	  if(anomericInvertedStructure)
	    {
	      fit=(structure->HFit+anomericInvertedStructure->HFit)/2;
	    }
	  else
	    {
	      fit=structure->HFit;
      ...
      else if(SE_Simulation.Criteria == SE_PURGE_CH)
	{
	  if(anomericInvertedStructure)
	    {
	      fit=(structure->ChFit+anomericInvertedStructure->ChFit)/2;
	    }
	  else
	    {
	      fit=structure->ChFit;
  ...
  if(anomericInvertedStructure)
    {
      SE_Simulation.StrActual+=2;
      if(SE_Simulation.Criteria == SE_PURGE_C && 
	 (SE_Simulation.StrActual==2 || structure->CFit > SE_Simulation.WorstFit ||
	  anomericInvertedStructure->CFit > SE_Simulation.WorstFit))
	{
	  SE_Simulation.WorstFit=Max(structure->CFit, anomericInvertedStructure->CFit);
	}
      else if(SE_Simulation.Criteria == SE_PURGE_H && 
	      (SE_Simulation.StrActual==2 || structure->HFit > SE_Simulation.WorstFit ||
	       anomericInvertedStructure->HFit > SE_Simulation.WorstFit))
	{
	  SE_Simulation.WorstFit=Max(structure->HFit, anomericInvertedStructure->HFit);
	}
      else if(SE_Simulation.Criteria == SE_PURGE_CH && 
	      (SE_Simulation.StrActual==2 || structure->ChFit > SE_Simulation.WorstFit ||
	       anomericInvertedStructure->ChFit > SE_Simulation.WorstFit))
	{
	  SE_Simulation.WorstFit=Max(structure->ChFit, anomericInvertedStructure->ChFit);
	}
    }
  else
    {
      SE_Simulation.StrActual+=1;
      if(SE_Simulation.Criteria == SE_PURGE_C && 
...
int PE_Connected()
{
  ...
  if(SE_ResidueSP>0)
	...
	} while (flag && count!=SE_ResidueSP);
    }
  for (item=0; item<SE_ResidueSP; ++item)
    {
      SE_ResidueStack[item].Connected=FALSE;
    }
  if (count!=SE_ResidueSP)
    {
      return (FALSE);
    }
  return(TRUE);
...
{
  int next_item, next_pos, comparison, flag=0, i, nEquivalentFrom=0, idEquivalentFrom, nEquivalentTo, idEquivalentTo, pos;
  ...
  *pos_pointer=*pos_pointer+1;
  ...
  for(i=0;i<SE_BondSP;++i)
    {
      if(SE_BondStack[i].ToRes==from_item)
	{
	  flag=1;
	  i=SE_BondSP;
	}
  ...
  if(!flag)
    {
      for(comparison=0; comparison<SE_ResidueSP; ++comparison)
	{
	  if(from_item==comparison)
	    {
	      idEquivalentFrom=nEquivalentFrom;
	      ++nEquivalentFrom;
	    }
	  else if(equivalentUnits[from_item][comparison])
	    {
	      flag=0;
	      for(i=0;i<SE_BondSP;++i)
		{
		  if(SE_BondStack[i].ToRes==comparison || SE_BondStack[i].FromRes==comparison)
		    {
		      flag=1;
		      i=SE_BondSP;
		    }
  ...
  for (next_item=*item_pointer; next_item<SE_ResidueSP; ++next_item, *pos_pointer=0)
    {
      nEquivalentTo=0;
      if(from_item!=next_item)
	{
	  flag=0;
	  for(i=0;i<SE_BondSP;++i)
	    {
	      if(SE_BondStack[i].ToRes==next_item)
		{
		  flag=1;
		  i=SE_BondSP;
		}
	    }
	  if(!flag && !equivalentUnits[next_item][from_item])
	    {
	      if(nEquivalentFrom>1)
		{
		  pos=next_item;
		  for(i=0;i<next_item;++i)
		    {
		      if(i==from_item || equivalentUnits[from_item][i])
			{
			  --pos;
			}
		    }
		  if(pos%nEquivalentFrom!=idEquivalentFrom)
		    {
		      /*		      printf("Skipping %s linking to %s\n", SE_ResidueStack[from_item].UnitSpecification->Node.Name, SE_ResidueStack[next_item].UnitSpecification->Node.Name);*/
		      continue;
		    }
		}
	      for (comparison=0; comparison<SE_ResidueSP; ++comparison)
		{
		    if(comparison!=from_item && equivalentUnits[next_item][comparison])
		    {
		      flag=0;
		      for(i=0;i<SE_BondSP;++i)
			{
		      ...
		      if(flag)
			{
		      ...
		      if(next_item==comparison)
			{
			  idEquivalentTo=nEquivalentTo;
			}
		      ++nEquivalentTo;
		    }
		}
		       /*if(nEquivalentTo>1)
		{
		  if(from_item%nEquivalentTo!=idEquivalentTo)
		    {
		      continue;
		    }
		    }*/
		      ...
		      if(ListLen(&rule->Residues)>0)
			{
			  if(!FindNode((struct Node *)&rule->Residues,
				       residue->Base->Node.Name))
			    {
			      ...
			      residue=(struct SE_Residue *)RemNode((struct Node *)residue);
			      residue=(struct SE_Residue *)residue->Node.Pred;
			      flag=0;
			      break;
			    }
			}
...
{
  struct BU_Struct *structure;
  struct SE_Unit *firstUnit, *secondUnit;
  struct VA_Variable *freeAnomeric;
  int new_number, i, j;
  char tempAssMode;
  float bestFit=9999;
  ...
  freeAnomeric=(struct VA_Variable *)FindNode(&(VariableList.Head), "freeanomeric");
  if(freeAnomeric && freeAnomeric->Value.Value.Float>0)
    {
      SE_Simulation.FreeAnomericEnd=1;
    }
  else
    {
      SE_Simulation.FreeAnomericEnd=0;
    }

  /* Allocate memory for an array showing which units are equivalent. This can reduce the calculation times. */
  equivalentUnits=malloc(sizeof(int *)*ListLen(&SE_Simulation.Units));
  for(firstUnit=(struct SE_Unit *)First(SE_Simulation.Units), i=0;firstUnit->Node.Succ!=NULL;
      firstUnit=(struct SE_Unit *)firstUnit->Node.Succ, ++i)
    {
      /* Allocate one extra element to store the number of equivalent units for each unit. */
      equivalentUnits[i]=malloc(sizeof(int)*ListLen(&SE_Simulation.Units));
      secondUnit=(struct SE_Unit *)SE_Simulation.Units.Head.Succ;
      for(secondUnit=(struct SE_Unit *)First(SE_Simulation.Units), j=0;secondUnit->Node.Succ!=NULL;
	  secondUnit=(struct SE_Unit *)secondUnit->Node.Succ, ++j)
	{
	  if(i==j)
	    {
	      equivalentUnits[i][j]=1;
	      continue;
	    }
	  if(SE_EquivalentUnits(firstUnit, secondUnit, &SE_Simulation))
	    {
	      equivalentUnits[i][j]=1;
	    }
	  else
	    {
	      equivalentUnits[i][j]=0;	      
	    }
	}
    }
  ...
  for(i=0;i<ListLen(&SE_Simulation.Units);++i)
    {
      free(equivalentUnits[i]);
    }
  free(equivalentUnits);
  ...
  for (structure=(struct BU_Struct *)StructList.Head.Succ;
      ...
      if(structure!=SE_Simulation.Experimental)
	{
	  ...
	  if(structure->ChCnt<=0)
	    {
	      SP_CalcChSpectra(structure);
	    }
	  SP_CalcHhSpectra(structure);
	  ...
	  BU_SortDoubleProtons(structure);
  ...
  for (structure=(struct BU_Struct *)StructList.Head.Succ;
	...
	sprintf(structure->Node.Name,"%.16s%d", SE_Simulation.Name, 
		new_number++);
      structure->TotFit=structure->TotFit/Max(bestFit, 0.000001);
    }
