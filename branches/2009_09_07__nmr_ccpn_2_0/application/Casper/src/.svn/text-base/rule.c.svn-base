...
{
  int i,j=0,patternlen,stlen, inBranch=0, branchMatch;
  ...
  /* If an end anchor is used make a reverse search. No gaps can be allowed
     except for wildcards ('.') and branches in the structure. */
  if(pattern[patternlen-1]=='$')
    {
      for(i=patternlen-2, j=stlen-1;i>=start && j>=0;--i, --j)
	{
	  if(pattern[i]!='.')
	    {
	      if(str[j]==']')
  ...
  for(i=start;i<patternlen;i++)
      ...
      for(;j<stlen;j++)
	{
	  ...
	  if(str[j]==pattern[i] || 
	    ...
	    {
	      /* If the start anchoring tag is used there can be no gaps (except for branches) until
