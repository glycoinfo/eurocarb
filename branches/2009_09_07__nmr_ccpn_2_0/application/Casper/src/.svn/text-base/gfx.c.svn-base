...
int GFX_CntUnits(struct BU_Struct *structure)
{
  ...
  for (unit=(struct BU_Unit *)structure->Units.Head.Succ;
      ...
      if(strcasecmp(unit->Residue->Shifts.Type->CTsuperclass, "subst")!=0)
	{
	  ...
	  if(nLinks==0)
	    {
	      if(cnt==0 && unit->Node.Succ->Succ==0)
		{
		  return 1;
		}
	    }
...
int GFX_RankUnits(struct BU_Unit *l_unit, struct BU_Unit *r_unit)
{
  ...
  struct BU_Unit *marker, *next_l_unit, *next_r_unit;
  ...
  do {
    ...
    /* Check if next unit is branched. This is common for the non-reducing end of a repeating unit. */
    ...
    next_l_unit=l_unit->Subst[anomerPos_l];
    next_r_unit=r_unit->Subst[anomerPos_r];
    if (next_l_unit==NULL)
      {
	return (1);
      }
    ...
    temp=GFX_CntSubst(next_r_unit)-GFX_CntSubst(next_l_unit);
    if(temp!=0)
      {
	return (temp);
      }
...
int GFX_PrintUnit(struct BU_Unit *unit, char *str, int last, int inShiftList)
{
  ...
  /* TODO: fix output of substituents, e.g. GlcNAcP */
  for (i=0; i<unit->Residue->Shifts.Type->HeavyCnt; i++)
    {
      if(i==anomerPos1)
	{
      ...
      if(unit->Subst[i]!=NULL)
	{
	  /* Don't print substituents if generating a structure string for comparison with biological rules,
	     i.e. if inShiftList==0 and str!=NULL */
	  if((inShiftList || str==NULL) && strcasecmp(unit->Subst[i]->Residue->Shifts.Type->CTsuperclass, "subst")==0)
	    {
  ...
  if((inShiftList || str==NULL) && anomerPos1!=-1 && unit->Subst[anomerPos1]!=0 && strcasecmp(unit->Subst[anomerPos1]->Residue->Shifts.Type->CTsuperclass, "subst")==0)
    {
  ...
  if(OMeFlag)
      ...
      if(str==NULL)
	{
	  if(!carbBank)
	    ...
	    {
	      printf("-(%s-1)-OME", unit->Shifts.Type->Atom[anomerPos1].Label);
	    }
	}
      else
	{
	  if(!carbBank)
	    ...
	    {
	      sprintf(str, "%s-(%s-1)-OME", str, unit->Shifts.Type->Atom[anomerPos1].Label);		  
	    }
	}
    }
	  ...
	  if(unit->Subst[anomerPos1]!=NULL && (strcasecmp(unit->Node.Name, "a")!=0 || 
	      ...
	      if(str==NULL)
		{
		  ...
		  /* If this is the first unit do not print the linkage if in carbBank mode instead print a
		    ...
		    {
		      printf("-(%s-1)-OH", unit->Shifts.Type->Atom[anomerPos1].Label);
		    }
		}
	      else
		{
		  ...
		  /* If this is the first unit do not print the linkage if in carbBank mode instead print a
		    ...
		    {
		      sprintf(str, "%s-(%s-1)-OH", str, unit->Shifts.Type->Atom[anomerPos1].Label);
		    }
		}
	      if(!last)
		{
		  if(str==NULL)
			...
			{
			  printf("%s)",unit->Subst[anomerPos1]->Subst[BU_ANOMER_POS(unit->Subst[anomerPos1])]->Shifts.Type->Atom[unit->Subst[anomerPos1]->Position].Label);
			}
		      else
			{
			  printf("%s)",
		    ...
		    {
		      if(inShiftList && strcmp(unit->Subst[anomerPos1]->Residue->Shifts.Type->CTsuperclass, "subst")==0)
			{
