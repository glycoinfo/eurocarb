...
int SE_EquivalentUnits(struct SE_Unit *a, struct SE_Unit *b, struct SE_Simulation *setup)
{
  ...
  /* If either of the units are specified as part of a known linkage they are
     not equivalent. */
  for(link=(struct SE_Linkage *)setup->Linkages.Head.Succ;
      link->Node.Succ!=NULL;
      link=(struct SE_Linkage *)link->Node.Succ)
    {
      if(link->FromUnit==a || link->ToUnit==a ||
	 link->FromUnit==b || link->ToUnit==b)
	{
	  return 0;
	}
    }
      ...
      for(i=0;i<=absconfloop;i++)
	{
	  ...
	  if(npos<=0)
	      ...
	      while (*string && (type==PA_STRING))
		{
	      ...
	      /* If only '*' is given as position mark all of them as free. */
	      if(npos==0 && !certaintyflag)
		{
		  for(pos=0;pos<rres->Shifts.Type->HeavyCnt;pos++)
		    {
		      if(rres->Shifts.Type->Atom[pos].Type&TY_FREE_POS)
			{
			  strcpy(allpos[npos++], rres->Shifts.Type->Atom[pos].Label);
			}
		    }
		}
	    }
	  ...
	  sres=(struct SE_Residue *)
	    ME_CreateNode(&SE_ResidueMethod, &(SE_CurrUnit->Residues), rres->Node.Name);
	  if (sres==NULL)
	    {
