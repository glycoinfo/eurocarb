...
public class PeakList extends BaseDocument implements SAXUtils.SAXWriter {
    ...
    /**
       Create a list containing a single peak with a specified
       mass/charge value and 0 intensity
     */
    ...
    return FileUtils.getIcon("peaksdoc");
    }
    ...
    filters.add(new ExtensionFileFilter("mgf", "Mascot generic peaklist format files"));
    filters.add(new ExtensionFileFilter("xml", "Bruker peak list files"));
    filters.add(new ExtensionFileFilter("msa", "Cartoonist peak list files"));
    filters.add(new ExtensionFileFilter("ctd", "Cartoonist centroids file"));
    filters.add(new ExtensionFileFilter(new String[] {"txt", "csv"}, "Comma separated peak list files"));
    filters.add(new ExtensionFileFilter(new String[] {"txt", "csv", "ctd", "mgf", "msa", "xml"}, "All peak list files"));
    ...
    /**
       Add a new peak to the list, if a previous peak with the same
       mass/charge value was present, the intensity is updated
       @return the index of the new peak or -1 if the peak was
       <code>null</code>
     */
    ...
    /**
       ...
       @param ind the index of the peak in the list
     */
    ...
    /**
       ...
       @param inds the indexes of the peak in the list
     */
    ...
    /**
       Remove the specified peak from the list
       @param mz the mass/charge value of the peak to remove
     */
    ...
    /**
       ...
       @param mz the mass/charge value of the peak
     */
    ...
    /**
       ...
       @param ind the index of the peak
     */
    ...
    /**
       ...
       @param ind the index of the peak
       @param _mz the new mass/charge value
     */
    public void setMZ(int ind, double _mz) {
    double _int = getIntensity(ind);
    removePVT(ind);
    putPVT(_mz,_int);
    updateMaxIntensity();
    fireDocumentChanged();
    ...
    /**
       ...
       @param ind the index of the peak
       @param _int the new intensity value
     */
    public void setIntensity(int ind, double _int) {
    double _mz = getMZ(ind);
    removePVT(ind);
    putPVT(_mz,_int);
    updateMaxIntensity();
    fireDocumentChanged();
    ...
    /**
       Return the intensity value of a peak normalized by the maximum
    ...
    /**
       ...
       @param inds the indexes of the peaks
     */
    ...
    /**
       ...
       mass/charge range
       @param from_mz minimum mass/charge value
       @param to_mz maximum mass/charge value
     */
    ...
    /**
       Return the peaks as a 2xN table contain mass/charge and
       intensity values
     */
    ...
    /**
       Return the peaks as a 2xN table contain mass/charge and
       intensity values
       @param rel_int <code>true</code> if the intensities should be
       normalized by the maximum intensity
     */
    ...
    /**
       ...
       @param p specifies the mass/charge value and the new intensity
     */
    ...
    /**
       ...
       @param peaks specifies the mass/charge values and the new
       intensities
     */
    ...
    try {
        org.proteomecommons.io.mgf.MascotGenericFormatPeakListReader mgf_reader = new org.proteomecommons.io.mgf.MascotGenericFormatPeakListReader(file.getPath());
        org.proteomecommons.io.Peak[] mgf_peaks = mgf_reader.getPeakList().getPeaks();
        
        if( !merge )
        clear();
        for( int i=0; i<mgf_peaks.length; i++ )
        putPVT(mgf_peaks[i].getMassOverCharge(),mgf_peaks[i].getIntensity());
        updateMaxIntensity();        
    ...
    try{
        ...
        Collection<Peak> read = parseString(transform(buffer,"/transforms/bruker_peak_list.xsl"));
        if( read.size()>0 ) {    
        if( merge )
            mergeData(read,false); 
        else
    ...
    /**
       Create a string representation of the peak list where the peaks
    ...
    /**
       Create a string representation of a collection of peaks where
       the peaks are separated by newlines
     */
    ...
    /**
       Load a list of peaks to the current list from a string
       representaion of a collection of peaks separated by newlines
       @param merge if <code>false</code> the list is emptied before
       adding the new peaks
     */
    ...
    /**
       Create a list of peaks from a string representaion of a
    ...
    */
    ...
    protected Object finalizeContent(String namespaceURI, String localName, String qName) throws SAXException{
        ...
        theDocument.putPVT(((Peak)o).getMZ(),((Peak)o).getIntensity());    
