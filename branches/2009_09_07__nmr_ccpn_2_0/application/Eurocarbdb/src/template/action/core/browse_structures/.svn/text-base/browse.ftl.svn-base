...
<#list results as seq>
    <tr>
        <td>
            <@ecdb.linked_sugar_image id=seq.glycanSequenceId seq=seq.sequenceGWS?url scale="0.5" />
            <br/>
            <span style="font-size: x-small">
            ID ${seq.glycanSequenceId?c}<#if ! seq.isDefinite()> (<abbr title="structure contains one or more unknown structural elements"><em>indefinite</em></abbr>)</#if>, entered ${seq.dateEntered?date} by <@ecdb.contributor c=seq.contributor />
            </span>
        </td>
        <td>
        <#if seq.hasEvidence()>
