...
*   for more details.
*
*   Last commit: $Rev: 1988 $ by $Author: magnusl@organ.su.se $ on $Date:: 2010-09-29 #$  
*/
/*------------------------------------------------------------------
*  Author: ml
*  EUROCarbDB Project
...
public class Casper extends EurocarbAction
{
    ...
    if(!this.getProjectFileName().equals(""))
        ...
        if(this.getProject().exists())
	       ...
	       this.getProjectFileName().endsWith(".zip"))
               {
                   try
                   {
                       this.unzipProject();
	...
	if(!this.getMode().equals("curate"))
	    {
		saveDir = new File(filenames + File.separator + "save");
	    }
	else
	    {
		saveDir = new File(getProjectFileName() + "_CASPER");
	    }
	       
        if((!file.exists() || file.length()==0) && (this.getDisableCcpn() || saveDir.exists()))
            {
	    System.out.println("Project save dir exists: " + saveDir.exists());
	    System.out.println("Removing CASPER files. Everything worked as expected.");
            if(file.exists())
                {
                file.delete();
                }
	...
	InputStream stm;
	Process proc;
	String outFilename, projectFile, targetFilename, dirName;
	BufferedReader input;
	ZipFile zip;
	ZipEntry entry;
	Enumeration en;
	int lastSlash, bytecount;
	byte[] buffer = new byte[128];

	outDir=this.createTempDirectory();
			
	if(this.getProjectFileName().endsWith(".zip"))
	    {
		zip = new ZipFile(this.getProject());
		...
		while(en.hasMoreElements())
			...
			if(entry.isDirectory())
				...
				projectFile=(entry.getName());
				continue;
			    }
			try {stm = zip.getInputStream(entry);}
			catch (Exception e)
			    {
				System.out.println("Cannot get input stream for '" + entry.toString() + "'");
				throw(e);
			    }
			targetFilename = outDir.getAbsolutePath() + File.separator + entry.toString();
			
			lastSlash = targetFilename.lastIndexOf(File.separator);
			if (lastSlash > 0)
			    {
				dirName = targetFilename.substring(0, lastSlash);
			...
			try
			    ...
			    {
				System.out.println("Error opening output file '" + targetFilename + "': " + e);
				throw(e);
			    }
			try
			    {
			    ...
			    {
				System.out.println("Error writing output file '" + targetFilename + "': " + e);
				throw(e);
			    }
