...
class ScrolledDensityMatrix(ScrolledCanvas):
  ...
  def __init__(self, parent,  matrix=None, boxSize=8, maxVal=None,
               ...
               borderColor='grey', zoom=1.0, *args, **kw):
  ...
  def mouseEnter(self, event):
    ...
    if self.canvasDict.get(item) and (self.canvasDict[item][0] == 'box'):
      ...
      if self.xText:
        ...
        self.xText = c.create_text(x2,y2,font=self.font,text=textX, anchor='nw', fill='darkBlue')
        self.yText = c.create_text(x1,y1,font=self.font,text=textY, anchor='sw', fill='darkBlue')
  ...
  def draw(self):
    ...
    if self.title:
      ...
      createText(midWay,self.origin[1]+self.indent-h2,
    ...
    if labelAxes:
      for j in range(M):
        yText = createText(x-w2, y+h2, font=self.font,
    ...
    for i in range(N):
      ...
      if labelAxes:
        xText = createText(x+w2, y+h2, font=self.font,
    ...
    for i in range(B):
      ...
      if barPlot:
        ...
        if len(legend) < B:
          ...
          itemB = createText(x+sz+sz/2, y, font=self.font,
        ...
        cConfig(itemB, text=text, font=self.font)
