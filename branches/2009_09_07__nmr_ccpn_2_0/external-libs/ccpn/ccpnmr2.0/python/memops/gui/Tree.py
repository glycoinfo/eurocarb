...
class Tree(Frame):
  ...
  def __init__(self, parent, iconSize='medium', multiSelect=False,
    ...
    self.canvas = Canvas(self, relief='flat', borderwidth=0,
                         background=BG_COLOR, xscrollcommand=self._setHScrollbar, *args, **kw)
...
class Node:
  ...
  def collapse(self):
    ...
    self.isOpen = False 
    if self.callback:
      self.tree.openDict[self.object] = False
...
if __name__ == '__main__':
  ...
  tree = Tree(root, multiSelect=True, doubleCallback=doubleClick)
