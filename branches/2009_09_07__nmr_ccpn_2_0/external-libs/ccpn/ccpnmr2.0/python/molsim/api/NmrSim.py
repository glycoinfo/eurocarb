...
Autogenerated by PyFileApiGen revision 1.51.2.3 on Mon Apr  6 17:04:47 2009
...
a restraint derived from relaxation measurements.
  """
  #   from data model element molsim.NmrSim.EnergyTermParameter revision 1.3.2.9
  _metaclass = metaPackage.getElement('EnergyTermParameter')
  _packageName = 'molsim.NmrSim'
  _packageShortName = 'NSIM'
  _fieldNames = ('applicationData', 'className', 'code', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'value', 'access', 'nmrSimEnergyTerm', 'parent', 'root', 'topObject',)
...
class NmrSimEnergyTerm(memops.api.Implementation.DataObject):
  ...
  """
  #   from data model element molsim.NmrSim.NmrSimEnergyTerm revision 1.5.2.9
  _metaclass = metaPackage.getElement('NmrSimEnergyTerm')
  _packageName = 'molsim.NmrSim'
  _packageShortName = 'NSIM'
  _fieldNames = ('applicationData', 'className', 'code', 'details', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'termId', 'access', 'constraintLists', 'energyTerm', 'energyTermParameters', 'parent', 'root', 'run', 'topObject',)
...
generations etc. taht use NMR data.
  """
  #   from data model element molsim.NmrSim.NmrSimStore revision 1.23.2.9
  _metaclass = metaPackage.getElement('NmrSimStore')
  _packageName = 'molsim.NmrSim'
  _packageShortName = 'NSIM'
  _fieldNames = ('application', 'applicationData', 'className', 'createdBy', 'fieldNames', 'guid', 'inConstructor', 'isDeleted', 'isLoaded', 'isModifiable', 'isModified', 'isReading', 'lastUnlockedBy', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'access', 'activeRepositories', 'memopsRoot', 'packageLocator', 'parent', 'root', 'runs', 'topObject',)
...
class Run(memops.api.Implementation.DataObject):
  ...
  """
  #   from data model element molsim.NmrSim.Run revision 1.29.2.9
  _metaclass = metaPackage.getElement('Run')
  _packageName = 'molsim.NmrSim'
  _packageShortName = 'NSIM'
  _fieldNames = ('applicationData', 'className', 'details', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'serial', 'access', 'annealProtocol', 'inputConstraintStore', 'inputMeasurementLists', 'inputPeakLists', 'inputStructures', 'molSystem', 'nmrSimEnergyTerms', 'nmrSimStore', 'outputConstraintStore', 'outputEnsemble', 'outputMeasurementLists', 'outputPeakLists', 'parent', 'root', 'runParameters', 'topObject',)
  ...
  def _checkDelete(self, objsToBeDeleted, objsToBeChecked, linkCounter, topObjectsToCheck):
    ...
    outputEnsemble = dataDict.get('outputEnsemble')
    if (outputEnsemble is not None):
  ...
  def _singleDelete(self, objsToBeDeleted):
    ...
    *Implementation 
    ...
    outputEnsemble = dataDict.get('outputEnsemble')
    if ((outputEnsemble is not None and not (outputEnsemble in objsToBeDeleted))):
  ...
  def checkValid(self, complete=False):
    ...
    if (complete):
      ...
      value = dataDict.get('outputEnsemble')
      if (value is not None):
        from ccp.api.molecule.MolStructure import StructureEnsemble as importedType
        if (not isinstance(value, importedType)):
          raise ApiError("""%s.checkValid: outputEnsemble:
        ...
        if (not (xx1 is yy1)):
          raise ApiError("""%s.checkValid: outputEnsemble:
           Link outputEnsemble between objects from separate partitions
           - memops.Implementation.MemopsRoot does not match""" % self.qualifiedName
           + ": %s:%s" % (self, value)
          )
    ...
    if (value is not None):
      if (not (value.molSystem is self.molSystem)):
        raise ApiError("""%s.checkValid: outputEnsemble:
         outputEnsemble constraint value_molSystem_is_self_molSystem violated by value""" % self.qualifiedName
         + ": %s" % (value,)
        )
  ...
  def findAllRunParameters(self, **conditions):
    ...
    if (nConditions == 0):
      ...
      if ((nConditions == 1)):
        (key, condition) = items[0]
        directAttrs = frozenset(('booleanValue', 'code', 'floatValue', 'id', 'intValue', 'textValue', 'access', 'run',))
        if (key in directAttrs):
          for v in currentValues:
            if (condition == v.__dict__.get(key)):
              result.add(v)
  ...
  def getOutputEnsemble(self):
    """
    Get for molsim.NmrSim.Run.outputEnsemble
    """
    dataDict = self.__dict__
    result = dataDict.get('outputEnsemble')
    return result
  ...
  def setOutputEnsemble(self, value):
    """
    ...
    if (value is not None):
      from ccp.api.molecule.MolStructure import StructureEnsemble as importedType
      if (not isinstance(value, importedType)):
        raise ApiError("""%s.setOutputEnsemble:
         value is not of class ccp.molecule.MolStructure.StructureEnsemble""" % self.qualifiedName
         + ": %s" % (value,)
        )

    topObject = dataDict.get('topObject')
    currentValue = dataDict.get('outputEnsemble')
    notInConstructor = not (dataDict.get('inConstructor'))
    ...
    if (value is not None):
      oldSelf = value.__dict__.get('outputFromNmrRun')
      if (oldSelf is not None):
        oldSelf.__dict__['outputEnsemble'] = None

      value.__dict__['outputFromNmrRun'] = self

    dataDict['outputEnsemble'] = value
    if (notIsReading):
      for topObjectModify in topObjectsToCheck:
        if (not (topObjectModify.__dict__.get('isLoaded'))):
          topObjectModify.load()
    ...
    if ((notInConstructor and notOverride)):
      ...
      ll = _notifies.get('setOutputEnsemble')
      if ll:
        for notify in ll:
          if notify not in ll1:
            notify(self)
  ...
  outputEnsemble = property(getOutputEnsemble, setOutputEnsemble, None,
  r"""MolStructure Ensemble generated from Run
  """)
...
class RunParameter(ccp.api.general.Template.MultiTypeValue):
  ...
  """
  #   from data model element molsim.NmrSim.RunParameter revision 1.1.2.10
  _metaclass = metaPackage.getElement('RunParameter')
  _packageName = 'molsim.NmrSim'
  _packageShortName = 'NSIM'
  _fieldNames = ('applicationData', 'booleanValue', 'className', 'code', 'fieldNames', 'floatValue', 'id', 'inConstructor', 'intValue', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'textValue', 'access', 'parent', 'root', 'run', 'topObject',)
  ...
  def __init__(self, parent, **attrlinks):
    ...
    try:
      ...
      if (notIsReading):
        ...
        if (objKey is None):
          raise ApiError("""%s.__init__:
           Key attribute or link not set (from ['code', 'id'])""" % self.qualifiedName
           + ": %s" % (self,)
          )
        else:
          if (childrenDict.get(objKey) is None):
  ...
  def checkValid(self, complete=False):
    ...
    if (complete):
      ...
      value = dataDict.get('id')
      if (isinstance(value, memops.api.Implementation.Int.PythonType)):
        pass
      elif ([x for x in memops.api.Implementation.Int.compatibleTypes if isinstance(value, x)]):
        value = memops.api.Implementation.Int.create(value)
      else:
        raise ApiError("""%s.checkValid: id:
         memops.Implementation.PositiveInt input is not of a valid type""" % self.qualifiedName
         + ": %s" % (value,)
        )
  ...
  def setId(self, value):
    ...
    if (notOverride):
      if (notInConstructor):
        raise ApiError("""%s.setId:
         cannot set id, frozen""" % self.qualifiedName
         + ": %s" % (self,)
        )
