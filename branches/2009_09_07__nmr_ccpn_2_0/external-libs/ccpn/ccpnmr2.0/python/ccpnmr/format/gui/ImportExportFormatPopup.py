...
import string, Tkinter, threading, time
        ...
        if addInfo:
          ...
          for addItem in addInfo:
            ...
            if inputKeyWord not in self.IOkeywords[component]:
              print "  Error: invalid %s keyword '%s'!" % (self.importExportFlag,inputKeyWord)
              continue
        ...
        if returnValue:
          if hasattr(self,'fileNames'):
            if self.fileNames == False:
              showInfo("Success","Successfully %sed file:\n%s." % (self.importExportFlag,self.fileName),self)
            else:
              showInfo("Success","Successfully %sed multiple files." % (self.importExportFlag),self)
          ...
          if vars().has_key('fileName'):
            showError("Error","Error while %sing file:\n%s." % (self.importExportFlag,self.fileName),self)
          else:
            showError("Error","Error while %sing multiple files." % (self.importExportFlag),self)
