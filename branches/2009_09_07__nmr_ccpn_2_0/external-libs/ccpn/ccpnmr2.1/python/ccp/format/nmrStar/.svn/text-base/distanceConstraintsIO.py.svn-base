...
class NmrStarDistanceConstraintFile(NmrStarConstraintFile):
  ...
  def parseSaveFrame(self):
    ...
    if self.saveFrame.tables.has_key(tableName):
      ...
      constraintLogicTableTags = self.saveFrame.tables[tableName].tags

      for i in range(0,len(constraintLogicTableTags[tableTagNames['constraintID']])):

        Id = constraintLogicTableTags[tableTagNames['constraintID']][i]

        if not findConstraint.has_key(Id):
          self.constraints.append(NmrStarDistanceConstraint())
          self.constraints[-1].setData(constraintLogicTableTags,i,tableTagNames)
          findConstraint[Id] = self.constraints[-1]

        else:

          self.constraints[-1].addNode(constraintLogicTableTags,i,tableTagNames)
      ...
      for i in range(0,len(nodeMembersTableTags[tableTagNames['constraintID']])):
      ...
      for i in range(0,len(distanceTableTags[tableTagNames['constraintID']])):
...
class NmrStarDistanceConstraint(GenericConstraint):
  ...
  def setData(self,constraintLogicTableTags,i,tableTagNames):
    ...
    self.addNode(constraintLogicTableTags,i,tableTagNames)
  
  def addNode(self,constraintLogicTableTags,i,tableTagNames):
  
    self.nodes.append(NmrStarDistanceConstraintItem())
    self.nodes[-1].setData(constraintLogicTableTags,i,tableTagNames)
