...
class ScrolledWindow(Frame):
  ...
  def __init__(self, parent, ncols = 1, nrows = 1,
    ...
    # want to cache widgets rather than continually create and delete them
    self.nrowsMax = nrows
    self.ncolsMax = ncols
    self.canvasStack = []
  ...
  def setBackground(self, color):
    ...
    for i in range(self.ncolsMax-1):
      separator = self.xseparators[i]
      separator.setColor(color)
 
    for j in range(self.nrowsMax-1):
...
'''
  ...
  def gridAll(self):
    ...
    ncolsMax = self.ncolsMax
    canvases = self.canvases
    
    rowspan = 2*nrows - 1
    rowspanMax = 2*nrowsMax - 1
    ...
    columnspanMax = 2*ncolsMax - 1
    for name in self.ypanel_names:
      (p, shown) = self.ypanel_dict[name]
      if (shown):
        columnspan = columnspan + 1
        columnspanMax = columnspanMax + 1

    rows = range(nrows)
    cols = range(ncols)
    extraRows = range(nrows, nrowsMax)
    extraCols = range(ncols, ncolsMax)

    # do not use minsize=1, it remembers it even if you later use grid_forget()
    for j in range(rowspanMax):
      #grid_rowconfigure(j, weight=0, minsize=1)
      grid_rowconfigure(j, weight=0)
    #for j in range(2*nrows, rowspan):
    #  self.grid_rowconfigure(j, weight=0, minsize=1)
    #for j in range(nrows-1):
    #  self.grid_rowconfigure(2*j+1, weight=0)

    for i in range(columnspanMax):
      #grid_columnconfigure(i, weight=0, minsize=1)
      grid_columnconfigure(i, weight=0)
    ...
    for j in rows:
      ...
      grid_rowconfigure(2*j, weight=1)
    ...
    for i in cols:
      ...
      grid_columnconfigure(2*i, weight=1)
  ...
  def markLocation(self, event):
    ...
    if event.num == 1 and hasattr(self.parent, 'unpostMenu'):
      self.parent.unpostMenu(event)
  ...
  def addRow(self, doGrid = True):
    ...
    for i in range(self.ncols):
      ...
      w = c.winfo_width()
      if self.canvasStack:
        c = self.canvasStack.pop()
        c.xview_region = xview_region
        c.yview_region = yview_region
        c.width = w
        c.height = h
