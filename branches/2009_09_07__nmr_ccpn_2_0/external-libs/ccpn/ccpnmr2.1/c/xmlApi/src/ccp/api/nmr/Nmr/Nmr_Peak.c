...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Nmr_Peak_GetActiveAccess(Nmr_Peak self)
{
  ...
  if (!method)
    return raiseApiException("ccp.nmr.Nmr.Peak: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.nmr.Nmr.Peak: method 'getActiveAccess' is not callable");
