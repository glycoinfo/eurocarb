...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Anal_AxisRegion_GetActiveAccess(Anal_AxisRegion self)
{
  ...
  if (!method)
    return raiseApiException("ccpnmr.Analysis.AxisRegion: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccpnmr.Analysis.AxisRegion: method 'getActiveAccess' is not callable");
