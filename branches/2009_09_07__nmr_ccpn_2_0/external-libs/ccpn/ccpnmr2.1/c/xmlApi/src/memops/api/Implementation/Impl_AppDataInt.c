...
**/
Impl_AppDataInt Impl_AppDataInt_Clone(Impl_AppDataInt self, ApiMap attrDict)
{
  ...
  if (!method)
    return raiseApiException("memops.Implementation.AppDataInt: cannot find method 'clone'");
  if (!PyCallable_Check(method))
    return raiseApiException("memops.Implementation.AppDataInt: method 'clone' is not callable");
