...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Nmr_PeakCluster_GetActiveAccess(Nmr_PeakCluster self)
{
  ...
  if (!method)
    return raiseApiException("ccp.nmr.Nmr.PeakCluster: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.nmr.Nmr.PeakCluster: method 'getActiveAccess' is not callable");
