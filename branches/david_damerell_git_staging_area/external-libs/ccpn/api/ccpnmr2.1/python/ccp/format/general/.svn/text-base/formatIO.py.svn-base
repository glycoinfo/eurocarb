...
class Citation:
  ...
  def __init__(self,parent,type,title,status,pubShortName,volume,issue,firstPage,year,publisherPlace,publisher = None,country = None,isPrimary = False,lastPage = None, pubLongName = None, pubMedId = None, details = None, bookTitle = None, bookSeries = None):
...
class Sequence:
  ...
  def __init__(self,molName = None, chainCode = defaultMolCode, *args, **keywds):
    ...
    if molName:
      self.molName = string.strip(molName)
    else:
...
class SequenceElement:
  ...
  def setResidueCode(self,*args):
    ...
    if len(args) >= 1:
      self.code3Letter = string.upper(args[0])
