...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Nmr_AtomSet_GetActiveAccess(Nmr_AtomSet self)
{
  ...
  if (!method)
    return raiseApiException("ccp.nmr.Nmr.AtomSet: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.nmr.Nmr.AtomSet: method 'getActiveAccess' is not callable");
