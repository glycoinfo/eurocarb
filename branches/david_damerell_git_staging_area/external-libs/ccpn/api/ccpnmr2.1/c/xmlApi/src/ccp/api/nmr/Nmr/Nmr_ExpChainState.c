...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Nmr_ExpChainState_GetActiveAccess(Nmr_ExpChainState self)
{
  ...
  if (!method)
    return raiseApiException("ccp.nmr.Nmr.ExpChainState: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.nmr.Nmr.ExpChainState: method 'getActiveAccess' is not callable");
