...
Autogenerated by PyFileApiGen revision 1.51.2.3 on Mon Apr  6 17:03:56 2009
...
a given set of coordinates.
  """
  #   from data model element ccp.molecule.MolStructure.Atom revision 1.46.2.9
  _metaclass = metaPackage.getElement('Atom')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('applicationData', 'className', 'elementSymbol', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'access', 'atom', 'atomValidations', 'chemAtom', 'coords', 'parent', 'residue', 'root', 'topObject',)
...
class Chain(memops.api.Implementation.DataObject):
  ...
  """
  #   from data model element ccp.molecule.MolStructure.Chain revision 1.42.2.9
  _metaclass = metaPackage.getElement('Chain')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('applicationData', 'className', 'code', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'access', 'chain', 'chainValidations', 'parent', 'residues', 'root', 'structureEnsemble', 'topObject',)
...
class Coord(memops.api.Implementation.DataObject):
  ...
  """
  #   from data model element ccp.molecule.MolStructure.Coord revision 1.37.2.9
  _metaclass = metaPackage.getElement('Coord')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('altLocationCode', 'applicationData', 'bFactor', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'occupancy', 'packageName', 'packageShortName', 'qualifiedName', 'x', 'y', 'z', 'access', 'atom', 'model', 'parent', 'root', 'topObject',)
...
Corresponds to a PDB MODEL.
  """
  #   from data model element ccp.molecule.MolStructure.Model revision 1.8.2.9
  _metaclass = metaPackage.getElement('Model')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('applicationData', 'className', 'details', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'name', 'packageName', 'packageShortName', 'qualifiedName', 'serial', 'access', 'coords', 'inputToNmrRuns', 'parent', 'root', 'structureEnsemble', 'structureGroups', 'structureValidations', 'topObject', 'violationLists',)
...
class Residue(memops.api.Implementation.DataObject):
  ...
  """
  #   from data model element ccp.molecule.MolStructure.Residue revision 1.42.2.9
  _metaclass = metaPackage.getElement('Residue')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('applicationData', 'className', 'fieldNames', 'inConstructor', 'isDeleted', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'seqCode', 'seqId', 'seqInsertCode', 'access', 'atoms', 'chain', 'parent', 'residue', 'residueValidations', 'root', 'topObject',)
...
MolSystem or part of it.
  """
  #   from data model element ccp.molecule.MolStructure.StructureEnsemble revision 1.9.2.9
  _metaclass = metaPackage.getElement('StructureEnsemble')
  _packageName = 'ccp.molecule.MolStructure'
  _packageShortName = 'COOR'
  _fieldNames = ('applicationData', 'atomNamingSystem', 'className', 'createdBy', 'ensembleId', 'fieldNames', 'guid', 'inConstructor', 'isDeleted', 'isLoaded', 'isModifiable', 'isModified', 'isReading', 'lastUnlockedBy', 'metaclass', 'packageName', 'packageShortName', 'qualifiedName', 'resNamingSystem', 'access', 'activeRepositories', 'coordChains', 'ensembleValidations', 'memopsRoot', 'models', 'molSystem', 'outputFromNmrRun', 'packageLocator', 'parent', 'root', 'structureAnalyses', 'structureGeneration', 'topObject', 'validationStores',)
  ...
  def _singleDelete(self, objsToBeDeleted):
    ...
    *Implementation function* - will CORRUPT DATA if called outside 
    ...
    if ((outputFromNmrRun is not None and not (outputFromNmrRun in objsToBeDeleted))):
      outputFromNmrRun.__dict__['outputEnsemble'] = None
  ...
  def checkValid(self, complete=False):
    ...
    if (complete):
      ...
      if (value is not None):
        ...
        oldSelf = value.__dict__.get('outputEnsemble')
        if (not (oldSelf is self)):
          raise ApiError("""%s.checkValid: outputFromNmrRun:
           non-reciprocal link StructureEnsemble from object""" % self.qualifiedName
           + ": %s" % (self,)
  ...
  def setOutputFromNmrRun(self, value):
    ...
    if (currentValue is not None):
      currentValue.__dict__['outputEnsemble'] = None
    ...
    if (value is not None):
      ...
      value.__dict__['outputEnsemble'] = self
