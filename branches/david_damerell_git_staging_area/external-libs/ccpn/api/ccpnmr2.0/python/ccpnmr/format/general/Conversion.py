...
class FormatConversion(object):
  ...
  def linkResonances(self,fractionCutoff = 0.05, **keywds):
    ...
    for nmrConstraintStore in self.ccpnProject.sortedNmrConstraintStores():
      ...
      if self.getFractionUnlinkedResonances(nmrConstraintStore.fixedResonances) > fractionCutoff:    
        linkResonances(self.ccpnProject, nmrConstraintStore = nmrConstraintStore, **keywds)
    
    if self.getFractionUnlinkedResonances(self.nmrProject.resonances) > fractionCutoff:    
      linkResonances(self.ccpnProject, nmrProject = self.nmrProject, **keywds)
    
    self.resonancesLinked = True
          
    #
    # TODO: here have to get some stats on how well process went?
    #
