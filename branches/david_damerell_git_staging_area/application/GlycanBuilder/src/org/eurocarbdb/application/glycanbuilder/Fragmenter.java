...
/**
   This utility class is used to generate all possible fragments of a
   glycan molecule.
...
public class Fragmenter {
    ...
    /**
       Return the maximum number of cleavages that this fragmenter will
    ...
    /**
       Set the maximum number of cleavages that this fragmenter will
       compute for each structure
     */
    ...
    /**
       Return the maximum number of cross-ring fragments that this
       fragmenter will compute for each structure
     */
    ...
    /**
       set the maximum number of cross-ring fragments that this
       fragmenter will compute for each structure
     */
    ...
    /**
       Return <code>true</code> if this fragmenter will create
       fragments containing no intact saccharides.
     */
    ...
    /**
       Set to <code>true</code> if this fragmenter should create
       fragments containing no intact saccharides.
     */
    ...
    /**
       Generate all possible configuration of labile residues given a
       set of structures.
       @see Glycan#detachLabileResidues
       @see Glycan#getAllLabilesConfigurations
     */
    static public FragmentDocument generateLabilesConfigurations(Collection<Glycan> structures) {
    ...
    /**
       Compute all fragments of a given structure resulting from
       cleaveages at the specified residue. Fuzzy structures and
       structure with repeating units cannot be fragmented.       
     */
    ...
    /**
       Compute all fragments of a given structure resulting from
       cleavages at the specified residue. Fuzzy structures and
       structure with repeating units cannot be fragmented. Return the
       results in <code>fragments</code>
    */
    ...
    /**
       Compute all fragments of a given structure. Return the results
       in <code>fragments</code>.
     */
    ...
    if( structure!=null && !structure.isFuzzy(true) && !structure.hasRepetition() ) {
        ...
        computeAllFragmentsWithLabiles(fragments,parent,max_no_cleavages,Math.min(max_no_cleavages,max_no_crossrings),mass_opt); 
        for( Glycan conf : parent.getAllLabilesConfigurations() )
            fragments.addFragment(conf,getFragmentType(conf));                
        }
        else {
        computeAllFragments(fragments,parent.getRoot(),max_no_cleavages,Math.min(max_no_cleavages,max_no_crossrings),mass_opt); 
    ...
    if( canDoCleavage(current) ) {
        ...
        if( fragments.addFragment(b_frag,getFragmentType(b_frag)) )
            computeAllFragments(fragments,b_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        ...
        if( fragments.addFragment(c_frag,getFragmentType(c_frag)) )
            computeAllFragments(fragments,c_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        }
        
        if( yfragments ) {
        Glycan y_frag = getYFragment(current,mass_opt);
        if( fragments.addFragment(y_frag,getFragmentType(y_frag)) )
            computeAllFragments(fragments,y_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);        
        }
        
        if( zfragments ) {
        Glycan z_frag = getZFragment(current,mass_opt);
        if( fragments.addFragment(z_frag,getFragmentType(z_frag)) )
            computeAllFragments(fragments,z_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);        
        }               
    }
    ...
    if( cur_max_no_crossrings>0 && canDoRingFragment(current) ) {
        ...
        for(CrossRingFragmentType crt : CrossRingFragmentDictionary.getCrossRingFragmentTypesA(current) ) {
            ...
            computeAllFragments(fragments,a_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings-1,mass_opt);             
        }
        }
        ...
        for(CrossRingFragmentType crt : CrossRingFragmentDictionary.getCrossRingFragmentTypesX(current) ) {
            ...
            computeAllFragments(fragments,x_frag.getRoot(),cur_max_no_cleavages-1,cur_max_no_crossrings-1,mass_opt);            
        }
        }
    }
    ...
    for( Linkage l : current.getChildrenLinkages() ) 
        computeAllFragments(fragments,l.getChildResidue(),cur_max_no_cleavages,cur_max_no_crossrings,mass_opt);
    }
    ...
    computeAllFragmentsWithLabiles(fragments,structure.getRoot(),avail_labiles,cur_max_no_cleavages,cur_max_no_crossrings,mass_opt); 
    }
    ...
    if( canDoCleavage(current) ) {
        if( bfragments ) {
        for( Glycan b_frag : Glycan.getAllLabilesConfigurations(getBFragment(current,mass_opt),avail_labiles) ) {
            if( fragments.addFragment(b_frag,getFragmentType(b_frag)) ) 
            computeAllFragmentsWithLabiles(fragments,b_frag,cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        }
        }

        if( cfragments ) { 
        for( Glycan c_frag : Glycan.getAllLabilesConfigurations(getCFragment(current,mass_opt),avail_labiles) ) {
            if( fragments.addFragment(c_frag,getFragmentType(c_frag)) ) 
            computeAllFragmentsWithLabiles(fragments,c_frag,cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        }
        }

        if( yfragments ) {
        for( Glycan y_frag : Glycan.getAllLabilesConfigurations(getYFragment(current,mass_opt),avail_labiles) ) {
            if( fragments.addFragment(y_frag,getFragmentType(y_frag)) ) 
            computeAllFragmentsWithLabiles(fragments,y_frag,cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        }
        }
        
        if( zfragments ) {
        for( Glycan z_frag : Glycan.getAllLabilesConfigurations(getZFragment(current,mass_opt),avail_labiles) ) {
            if( fragments.addFragment(z_frag,getFragmentType(z_frag)) )
            computeAllFragmentsWithLabiles(fragments,z_frag,cur_max_no_cleavages-1,cur_max_no_crossrings,mass_opt);
        }
        }               
    }
    ...
    if( cur_max_no_crossrings>0 && canDoRingFragment(current) ) {
        if( afragments ) {
        for(CrossRingFragmentType crt : CrossRingFragmentDictionary.getCrossRingFragmentTypesA(current) ) {
            for( Glycan a_frag : Glycan.getAllLabilesConfigurations(getAFragment(current,crt,true,mass_opt),avail_labiles) ) {
            if( (!a_frag.isSmallRingFragment() || a_frag.countCharges()>0 ) && fragments.addFragment(a_frag,getFragmentType(a_frag)) )
                computeAllFragmentsWithLabiles(fragments,a_frag,cur_max_no_cleavages-1,cur_max_no_crossrings-1,mass_opt); 
            }
        }
        }
        ...
        if( xfragments ) {
        for(CrossRingFragmentType crt : CrossRingFragmentDictionary.getCrossRingFragmentTypesX(current) ) {
            for( Glycan x_frag : Glycan.getAllLabilesConfigurations(getXFragment(current,crt,true,mass_opt),avail_labiles) ) {
            if( (!x_frag.isSmallRingFragment() || x_frag.countCharges()>0 ) && fragments.addFragment(x_frag,getFragmentType(x_frag)) )
                computeAllFragmentsWithLabiles(fragments,x_frag,cur_max_no_cleavages-1,cur_max_no_crossrings-1,mass_opt);
            }
        }
        }
    }
    ...
    // recursion
    for( Linkage l : current.getChildrenLinkages() ) 
        computeAllFragmentsWithLabiles(fragments,l.getChildResidue(),avail_labiles,cur_max_no_cleavages,cur_max_no_crossrings,mass_opt);    
    }           

    /**
       Return <code>true</code> if the structure can be cleaved at the
       specified residue with a glycosidic cleavage. Fuzzy structures
       and structure with repeating unit cannot be fragmented. If
       <code>tolerate_labiles</code> is <code>true</code> the
       fragments could be computed even if some labile residues have
       unspeficied connectivity.
       @see Glycan#isFuzzy(boolean)
       @see #canDoCleavage(Residue)
     */
    static public boolean canDoCleavage(Glycan structure, Residue current, boolean tolerate_labiles ) {
    return (structure!=null && !structure.isFuzzy(tolerate_labiles) && !structure.hasRepetition() && canDoCleavage(current));
    }

    /**
       Return <code>true</code> if the structure can be cleaved at the
       specified residue with a glycosidic cleavage. Fuzzy structures
       and structure with repeating unit cannot be fragmented.
       @see Glycan#isFuzzy
       @see #canDoCleavage(Residue)
     */
    static public boolean canDoCleavage(Glycan structure, Residue current ) {
    return (structure!=null && !structure.isFuzzy() && !structure.hasRepetition() && canDoCleavage(current));
    }

    /**
       Return <code>true</code> if a glycosidic-cleavage can be
       computed at the specified residue.
       @see Residue#isCleavable
     */
    static public boolean canDoCleavage(Residue current ) {
    return (current!=null && current.isCleavable() && current.getParent()!=null && current.getParent().isCleavable());
    }


    /**
       Return <code>true</code> if the structure can be cleaved at the
       specified residue with a ring cleavage. Fuzzy structures and
       structure with repeating units cannot be fragmented. If
       <code>tolerate_labiles</code> is <code>true</code> the
       fragments could be computed even if some labile residues have
       unspeficied connectivity.
       @see Glycan#isFuzzy(boolean)
       @see #canDoCleavage(Residue)
     */
    static public boolean canDoRingFragment(Glycan structure, Residue current, boolean tolerate_labiles ) {
    return (structure!=null && !structure.isFuzzy(tolerate_labiles) && !structure.hasRepetition() && canDoRingFragment(current));    
    }
    ...
    return (structure!=null && !structure.isFuzzy() && !structure.hasRepetition() && canDoRingFragment(current));    
    }

    /**
       Return <code>true</code> if a ring-cleavage can be computed at
       the specified residue. All linkages with the residue must be
    ...
    /**
       Return all possible configurations of labile residues for the
       A-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @param type the type of A fragment to be computed
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getAFragment(current,type,false,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    ...
    /**
       Return all possible configurations of labile residues for the
       B-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @see Glycan getAllLabilesConfigurations
     */
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getBFragment(current,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    return Glycan.getAllLabilesConfigurations(fragment.detachLabileResidues(),avail_labiles);
    }
    ...
    /**
       Return all possible configurations of labile residues for the
       C-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @see Glycan getAllLabilesConfigurations
     */
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getCFragment(current,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    return Glycan.getAllLabilesConfigurations(fragment.detachLabileResidues(),avail_labiles);
    }
    ...
    /**
       Return all possible configurations of labile residues for the
       X-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @param type the type of X fragment to be computed
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getXFragment(current,type,false,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    return Glycan.getAllLabilesConfigurations(fragment.detachLabileResidues(),avail_labiles);
    }
    ...
    /**
       Return all possible configurations of labile residues for the
       Y-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @see Glycan getAllLabilesConfigurations
     */
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getYFragment(current,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    return Glycan.getAllLabilesConfigurations(fragment.detachLabileResidues(),avail_labiles);
    }
    ...
    /**
       Return all possible configurations of labile residues for the
       Z-type fragment resulting from the cleavage of the given
       structure at the specified residue.
       @see Glycan getAllLabilesConfigurations
     */
    ...
    structure = structure.detachLabileResidues();
    TypePattern avail_labiles = structure.getDetachedLabilesPattern();
    Glycan fragment = getZFragment(current,structure.getMassOptions().removeExchanges());
    if( fragment==null )
        return new Vector<Glycan>();   
    return Glycan.getAllLabilesConfigurations(fragment.detachLabileResidues(),avail_labiles);
    }
    ...
    /**
       Return the A-type fragment resulting from the cleavage of the
       given structure at the specified residue.
       @param type the type of A fragment to be computed
       @see CrossRingFragmentDictionary#getCrossRingFragmentTypesA(Residue)
     */
    ...
    if( !canDoRingFragment(structure,current) )
        return null;
    return getAFragment(current,type,false,structure.getMassOptions().removeExchanges());    
    }
    ...
    if( !internal_fragments && type.anyValidPosition(current.getParentLinkage().getChildPositions()) )
        return null;    
    ...
    // a glycan must contain at least one saccharide or a charge (in negative mode)
    Glycan a_frag = new Glycan(a_root,false,mass_opt);
    if( (!a_frag.isSmallRingFragment() && !a_root.hasRingFragments()) || (small_ring_fragments==true && a_frag.countCharges(allow_virtual_charges)>0 ) )
        return a_frag;
    return null;
    }    

    /**
       Return the B-type fragment resulting from the cleavage of the
       given structure at the specified residue.
    */
    public Glycan getBFragment(Glycan structure, Residue current) {
    if( !canDoCleavage(structure,current) )
        return null;
    return getBFragment(current,structure.getMassOptions().removeExchanges());    
    }
    ...
    /**
       Return the C-type fragment resulting from the cleavage of the
       given structure at the specified residue.
    */
    public Glycan getCFragment(Glycan structure, Residue current) {
    if( !canDoCleavage(structure,current) )
        return null;
    return getCFragment(current,structure.getMassOptions().removeExchanges());    
    }
    ...
    /**
       Return the X-type fragment resulting from the cleavage of the
       given structure at the specified residue.
       @param type the type of X fragment to be computed
       @see CrossRingFragmentDictionary#getCrossRingFragmentTypesX(Residue)
    */
    ...
    if( !canDoRingFragment(structure,current) )
        return null;
    return getXFragment(current,type,false,structure.getMassOptions().removeExchanges());    
    }
    ...
    // a glycan must contain at least one saccharide or a charge (in negative mode)    
    if( (!x_frag.isSmallRingFragment() && !x_leaf.hasRingFragments()) || (small_ring_fragments==true && x_frag.countCharges(allow_virtual_charges)>0 ) )
        return x_frag;
    return null;
    }
    
    /**
       Return the Y-type fragment resulting from the cleavage of the
       given structure at the specified residue.
    */
    public Glycan getYFragment(Glycan structure, Residue current) {
    if( !canDoCleavage(structure,current) )
        return null;
    return getYFragment(current,structure.getMassOptions().removeExchanges());    
    }
    ...
    /**
       Return the Z-type fragment resulting from the cleavage of the
       given structure at the specified residue.
    */
    public Glycan getZFragment(Glycan structure, Residue current) {
    if( !canDoCleavage(structure,current) )
        return null;
    return getZFragment(current,structure.getMassOptions().removeExchanges());    
    }
    ...
    /**
       Return the fragment resulting from the cleavage of the labile
       residue <code>current</code> from the given structure.
    */
    public Glycan getLFragment(Glycan structure, Residue current) {
    if( !canDoCleavage(structure,current) )
        return null;
    return getLFragment(current,structure.getMassOptions().removeExchanges());    
    }
    ...
    /**
       Return a representation of the type of fragment represented by
       this structure as a string formed by the cleavage types. Return
       the empty string if no cleavages are present
