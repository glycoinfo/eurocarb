  ...
  def read(self, coordinateFile = None, verbose = 0):
    ...
    for coordinate in coordinateFile.modelCoordinates[modelNums[0]]:
      ...
      if segId != coordinate.segId:
        ...
        self.sequences.append(CnsSequence(chainCode = coordinate.segId))
...
if __name__ == "__main__":  
  ...
  files = ['../reference/xplor/neuhaus.pdb',
           '../reference/aria/protDNACath.pdb',
           '/ebi/msd/nmrqual/reference/AartProject/test_200406/db/monomers/1b2t/coord_reprotonated/1b2t_1.pdb']

  for file in files:

    file = os.path.join(getTopDirectory(),file)

    cnsFile = CnsSequenceFile(file)
  
    cnsFile.read(verbose = 1)

    for seq in cnsFile.sequences:
