...
class NmrStarFormat(DataFormat):
  ...
  def setStructureGenerationEnsemble(self, strucGen):
    ...
    if strucGen and strucGen not in self.entry.structureGenerations:
  ...
  def setMeasurementListSoftware(self):
    ...
    #self.measurementListKeywds['software'] = []
        ...
        if self.measurementFile.measureSofts:
          for soft in self.measurementFile.measureSofts:
            if soft.softId == softId:
              #if software not in self.measurementListKeywds['software']:
              #  self.measurementListKeywds['software'].append(software)
  ...
  def setReadCoordinatesChainMapping(self,chains,localKeywds):
    ...
    if forceChainMappings:
      ...
      localKeywds['forceChainMappings'].extend(forceChainMappings)
...
#
  ...
  def readConfStats(self):
    ...
    for confStat in self.confStatFile.confStat:
      ...
      strucGen = strucEns.structureGeneration

      if strucGen and confStat.details:
        strucGen.details = confStat.details
  ...
  def readSamples(self):
    ...
    for sample in self.sampleFile.samples:
      ...
      for sampleComponent in self.sampleFile.sampleComponents:
        ...
        if not newRefComp:
          newRefComp = self.project.currentRefSampleComponentStore.newMolComponent(name = sampleComponent.name + ':' + sample.name, details =  sampleComponent.name) # Need to set right molecule
