...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Anal_AbstractMark_GetActiveAccess(Anal_AbstractMark self)
{
  ...
  if (!method)
    return raiseApiException("ccpnmr.Analysis.AbstractMark: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccpnmr.Analysis.AbstractMark: method 'getActiveAccess' is not callable");
