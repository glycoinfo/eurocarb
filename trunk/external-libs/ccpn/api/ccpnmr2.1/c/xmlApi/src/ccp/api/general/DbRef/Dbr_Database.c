...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Dbr_Database_GetActiveAccess(Dbr_Database self)
{
  ...
  if (!method)
    return raiseApiException("ccp.general.DbRef.Database: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.general.DbRef.Database: method 'getActiveAccess' is not callable");
