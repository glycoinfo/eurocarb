...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Anal_Ruler_GetActiveAccess(Anal_Ruler self)
{
  ...
  if (!method)
    return raiseApiException("ccpnmr.Analysis.Ruler: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccpnmr.Analysis.Ruler: method 'getActiveAccess' is not callable");
