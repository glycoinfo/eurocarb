...
the access permissions on the (recursive) parents.
  ...
  @returns   the result
**/
Acco_AccessObject Entr_Study_GetActiveAccess(Entr_Study self)
{
  ...
  if (!method)
    return raiseApiException("ccp.nmr.NmrEntry.Study: cannot find method 'getActiveAccess'");
  if (!PyCallable_Check(method))
    return raiseApiException("ccp.nmr.NmrEntry.Study: method 'getActiveAccess' is not callable");
