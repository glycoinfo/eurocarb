...
class DataFormat(TopShared):
  ...
  def readMeasurements(self,fileName = None, verbose = True, **keywds):
    ...
    if not self.setIOkeywords('readMeasurements', keywds, verbose = verbose):
      return
  ...
  def writeMeasurements(self, fileName = None, verbose = True, **keywds):
    ...
    if not self.setIOkeywords('writeMeasurements', keywds, verbose = verbose):
      return False
  ...
  def setConstraintCodes(self,readWrite, keywds, verbose = True):
    ...
    if not self.setIOkeywords(readWrite + self.constraintApiCode + 'Constraints', keywds, verbose = verbose):
      return False
  ...
  def readConstraints(self, fileName = None, verbose = True, **keywds):
    ...
    if not self.setIOkeywords('readConstraints', keywds, verbose = verbose):
      return
    ...
    if not self.constraintList: 
      ...
      self.constraintList = self.ApiConstraintList(self.nmrConstraintStore,name = self.constraintListName, details = 'Autogenerated by readConstraints.')
  ...
  def writeConstraints(self, fileName = None, verbose = True, **keywds):
    ...
    if not self.setIOkeywords('writeConstraints', keywds, verbose = verbose):
      return
  ...
  def setIOkeywords(self,functionName, keywds, verbose = True):
  ...
  def setConstraintFileClass(self):
    ...
    try:
      ...
      self.ConstraintFileClass = getattr(self.constraintsIO,"%s%sConstraintFile" % (self.formatLabel,self.constraintApiCode))
    except:
      if not self.minimalPrompts:
    ...
    for self.rawMeasurement in self.measurementFileValues:    
      ...
      if self.resonanceNames.has_key(resName):
        ...
        for resonance in self.resonanceNames[resName]:
          ...
          if self.measurement:
            self.setMeasurementSpecific(resName)
    ...
    for self.rawMeasurement in self.measurementFileValues:
      ...
      if len(resNames) == self.measurementResNum:
        ...
        for resonanceComb in resonanceCombs:
          ...
          if self.measurement:
            self.setMeasurementSpecific(resNames)
  ...
  def checkMolecules(self):
    ...
    for self.sequence in self.sequenceFile.sequences:
      ...
      if numOrigSequences == 1 and self.molName:
        baseMolName = self.molName[:76].strip()

      elif self.sequence.molName:
        baseMolName = self.sequence.molName[:76].strip()
            ...
            for (sequenceElementIndex,bioPolymerTypes) in unknownSequenceElements:
              if molType and molType in bioPolymerTypes:
                ...
                if hasattr(sequenceElement,'code3Letter') and sequenceElement.code3Letter:
                  ...
                  for molType in bioPolymerCodes.keys() + ['carbohydrate']:
                    file = joinPath('molecule', 'ChemComp' , "%s.%s.xml" % (molType,sequenceElement.code3Letter.capitalize()))
                  ...
                  if not chemCompFound:
                    ...
                    for molType in bioPolymerCodes.keys() + ['carbohydrate']:
                      for tempCcpCode in chemCompStdDict[molType].keys():
                        if sequenceElement.code3Letter == tempCcpCode or sequenceElement.code3Letter == chemCompStdDict[molType][tempCcpCode][1]:
                          origSequenceDict[origMolName][1][sequenceElementIndex][-1] = molType
                          chemCompFound = True
                          #print "Found in %s in dictionary, molType %s" % (sequenceElement.code3Letter,molType)
                          break
                    ...
                    for carbCcpCode in chemCompStdDict[molType].keys():
                      (code1letter,cifCode,name) = chemCompStdDict[molType][carbCcpCode]
                  ...
                  if not chemCompFound:
                    ...
                    if not chemCompFound:
                      ...
                      for tempCcpCode in chemCompOtherDict[molType].keys():
                        if sequenceElement.code3Letter == tempCcpCode or sequenceElement.code3Letter == chemCompOtherDict[molType][tempCcpCode][1]:
                          origSequenceDict[origMolName][1][sequenceElementIndex][-1] = molType
                          chemCompFound = True
    ...
    for origMolName in origMolNames:
      ...
      for seqElIndex in range(0,seqLength):
        ...
        if not chemComp:
          print "  Warning: no %s chemComp %s code %s selected. Will be ignored." % (residueType,codeText,code)
          ...
          while (newChain == 1):
            ...
            if formatChainCode and not chainMapping:
              ...
              for firstSeqId in seqIds:
                ...
                chainMapping = NameMapping.ChainMapping(
  ...
  def setSingleResonanceMeasurementSpecific(self,resName):
    ...
    if abs(self.measurement.value - self.rawMeasurement.value) > self.measurementAllowedDiff:
      ...
      measurement.value = interaction.selection

  def setHExchProtectionSpecific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)

  def setHExchRateSpecific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)

  def setShiftAnisotropySpecific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)
  
  def setShiftSpecific(self,resName):
  ...
  def setT1Specific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)

  def setT1RhoSpecific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)
    
  def setT2Specific(self,resName):
    
    self.setSingleResonanceMeasurementSpecific(resName)
    
  def setMultipleResonanceMeasurementSpecific(self,resNames):
    
    if abs(self.measurement.value - self.rawMeasurement.value) > self.measurementAllowedDiff:
      
      print "  Warning: different values for same %s measurement. Keeping first value." % self.measurementApiCode
  
  def setNoeSpecific(self,resNames):
     
    self.setMultipleResonanceMeasurementSpecific(resNames)
  
  def setJCouplingSpecific(self,resNames):
     
    self.setMultipleResonanceMeasurementSpecific(resNames)

  def setRdcSpecific(self,resNames):
...
#
  ...
  def getCode3Letter(self,sequenceElement):
    ...
    return sequenceElement.code3Letter.strip().upper()
  ...
  def createPeakList(self,peakListName):
    ...
    peakList = Nmr.PeakList(self.dataSource, isSimulated = False)
  ...
  def setPeak(self):
    ...
    if newPeak:
      ...
      self.peak = Nmr.Peak(self.peakList, details = "Original number %d" % self.rawPeak.num)
    ...
    for resNameIndex in range(len(self.resNames)):
      ...
      for resonance in resonances:
        ...
        if not peakDimContrib:
          Nmr.PeakDimContrib(self.peakDim, resonance = resonance, **keywds)
        elif keywds:
  ...
  #
  # Constraint specific (EXCEPT dihedrals!)
  # 
    ...
    for resonance in self.resonancesToLink:
      ...
      if not self.useOriginalResNames and not self.resonanceToAtoms.has_key(resonance):
        ...
        if self.resonanceSet:
          ...
          if not namingSystem:
            print "  No naming system %s available for resonance %s - not written." % (self.namingSystemName,getResNameText(resonance))
            continue
          ...
          #
            ...
            for atomSet in atomSets:
              ...
              if not sysNamesLink:
                ...
                if not chemAtomSetName and not self.individualAtoms and chemAtom.chemAtomSet and hasattr(self,'compressResonances') and self.compressResonances:
  ...
  def setPeakDimContribs(self,peakAssignDim, peakDims = None, resonances = None):
    ...
    if len(self.assignCombination) == peakAssignDim:
      ...
      peakContrib = Nmr.PeakContrib(self.peak, weight = 1)
      ...
      for i in range(0,len(peakDims)):
        ...
        if not self.resonancesPeakContrib[i].has_key(resonance):
          ...
          peakDimContrib = Nmr.PeakDimContrib(peakDim,resonance = resonance, peakContribs = [peakContrib])
  ...
  def autoSelectCoordNamingSystem(self):
    ...
    if self.namingSystemName not in bestHits:
  ...
  def setPeople(self):
    ...
    for rawPerson in self.peopleAndCitationsFile.persons:
      ...
      if not firstName and len(rawPerson.initials) >= 1:
        firstName = rawPerson.initials[0]
        if len(rawPerson.initials) > 1:
          midInitials = rawPerson.initials[1:]

      # TODO - need to deal with actual middle names - as they get turned into initials

      if midInitials and len(midInitials) > 1:
        if midInitials[1].islower():
          midInitials = midInitials[0]
      ...
      if self.useExistingPersons and self.project.currentAffiliationStore:
        ...
        if person and not person.middleInitials and midInitials and type(midInitials) == type(str() ):
          person.middleInitials = self.makeInitialsForCcpn(midInitials)
      ...
      if not person:
        ...
        if midInitials:
          if type(midInitials) == type(str() ):
            midInitials = self.makeInitialsForCcpn(midInitials)

          keywds['middleInitials'] = midInitials

          #print 'INITIALS1: [%s]' % midInitials
    ...
    for tperson in self.project.currentAffiliationStore.sortedPersons():
      if tperson.familyName and familyName and tperson.familyName.upper() == familyName.upper():
        if (tperson.givenName and firstName and tperson.givenName.upper() == firstName.upper()) or \
           (initials and ((tperson.firstInitial and tperson.firstInitial.upper() == initials[0].upper()) or \
           (tperson.givenName and tperson.givenName[0].upper() == initials[0].upper()))):
      ...
      if not citation:
        ...
        if rawCitation.type == 'journal':
          ...
          if rawCitation.type == 'thesis':
            ...
            keywds['bookTitle'] = rawCitation.title

          if rawCitation.publisher:
        ...
        # TODO TODO remove returnMemopsLine() when data type changesd!!
        keywds['title'] = title.replace('\n', ' ').strip()
