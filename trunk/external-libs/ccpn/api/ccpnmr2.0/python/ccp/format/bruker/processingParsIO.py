...
class BrukerProcessingParData(BrukerGenericFile,GenericProcessingParsFile):
  ...
  def process(self):
    ...
    mainPars.update(self.parFiles['procs'].parameters)
    ...
    for dimIndex in range(0,self.numDim):
      ...
      if dimIndex > 0:
        mainPars = self.parFiles['acqu%ds' % (dimIndex + 1)].parameters
        mainPars.update(self.parFiles['proc%ds' % (dimIndex + 1)].parameters)
      ...
      for parInfo in dimPars:
        ...
        if parKey:
          ...
          if parKey in ('SI','XDIM'):
            ...
            elif mainPars.get('NUC1'):
              value = mainPars['NUC1'].values[0]
            else:
              value = '1H'
