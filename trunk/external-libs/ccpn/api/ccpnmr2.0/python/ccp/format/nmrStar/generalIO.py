...
class NmrStarFile(NmrStarGenericFile):
  ...
  def setSfDict(self,component):
      ...
      if component == sfName or component == 'all':
        ...
        self.sfDict[sfName].tables = [
            ...
            ['_Database_name', [        
                ...
                ['_Sequence_identity',                      lambda x = value: returnStarInt(x)],
                ['_Sequence_positive',                      lambda x = value: returnStarInt(x)],
    ...
    #############################################
      ...
      try:
        from msd.nmrStar.IO.nmrStarDict import sfDict as refSfDict
      except:
    ...
    for origSaveFrame in origNmrStarFile.datanodes:
      ...
      for tagtable in origSaveFrame.tagtables:
        ...
        try:
          ...
          tagtable.tagnames.index('_Saveframe_category')
          versionHits['2.1.1'] += 1
          ...
          for i in range(0,len(tagtable.tagnames)):
            ...
            components = string.split(tagtable.tagnames[i],self.tagSep)
        ...
        try:
          ...
          if self.version == '2.1.1':
            ...
            for i in range(0,len(tagtable.tagnames)):
              ...
              (sfName,tagName) = string.split(tagtable.tagnames[i],self.tagSep)
  ...
  def __init__(self,title,name,prefix = None):
    ...
    if prefix:
      ...
      if name[0] not in string.ascii_uppercase:
        self.prefix = '_' + string.capitalize(name)
      else:
        self.prefix = '_' + name
...
class ConversionError:
  ...
  def write(self,error):
    ...
    error = string.replace(error,os.linesep,'')
    string.strip(error)
...
class GenericConstraint:
  ...
  def setErrors(self,errorTags,tableTagNames,i):
    ...
    for tableTagName in tableTagNames.keys():
      ...
      if string.find(tableTagNames[tableTagName],'%s') == -1:
