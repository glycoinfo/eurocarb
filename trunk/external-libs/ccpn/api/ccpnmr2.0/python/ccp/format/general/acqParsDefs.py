...
class GenericAcqParData(FormatFile):
  ...
  def getSpecRefs(self,pars,nullValue,specTypeRef = None):
    ...
    for refs in self.refList:
      ...
      if noValue == 0:
        ...
        nucleus = pars[nucleusCode].values[0]

        searchObj = self.patt['nucleusLetterDigit'].search(nucleus)

        if searchObj:
          # Switch letter to back for 'H1' style nuclei
          nucleus = searchObj.group(2) + searchObj.group(1)
  ...
  def getFreqDimInfo(self,valppoint,npoints):
    ...
    if npoints > 1:
      ...
      print "  ERROR: %s has only %d number of points for FID" % (self.fileDir,npoints)
      npointsFreq = 2
