...
class ScrolledMatrix(Frame):
  ...
  def refreshSize(self, event=None):
    ...
    for j in columns:
      ...
      if ( j2 < len(self.textHeading) ) and (j2 >= 0):
        if self.textHeading[j2]:
          ...
          headHeight = max(headHeight, coords[3] - coords[1] + (3*pad))
      ...
      if cellWidth is None:
        ...
        if self.toggledCols[j]:
          ...
          ii = 0
          for i in range(self.vOffset,min(self.visibleRows+self.vOffset,len(self.textMatrix)) ):
            if ii >= len(extraRowList):
              extraRowList.append(0)
            
            try:
              tryText = self.textMatrix[i][j]
              numLines = tryText.count('\n')
              if tryText is not None:
                tryText = self.formatText(tryText)
                if len(tryText) > len(text):
                  text = tryText
            except:
              numLines = 0
              
            if numLines > extraRowList[ii]:
              extraRowList[ii] = numLines
            ii +=1
    ...
    if self.maxRows:
      self.visibleRows = max(min(self.maxRows,int((height-headHeight)/cellHeight)-extraRows), 1)
    else:
      self.visibleRows = max(int((height-headHeight)/cellHeight)-extraRows, 1)
  ...
  def sortLine(self, l, noUpdate=None):
    ...
    newRange   = []
    for i in range(len(matrix)):
      newMatrix.append( matrix[indices[i]] )
      if indices[i] < len(colors):
        newColors.append( colors[indices[i]] )
      object = self.objectList[indices[i]]
    ...
    if not noUpdate:
      if obj is not None and not self.multiSelect:
        self.hilightObject(obj)
      self.drawCanvas()
  ...
  def selectObjects(self, objects):
    ...
    for object in objects:
      ...
      if index is not None:
        ...
        self.currentRange.append(index)
  ...
  def moveHScrollbar(self, prop, prop2, refresh=True):
    ...
    if gaps == 0 or N == 0:
      ...
      visibleCells = float(self.visibleCols)
      totalCells   = float(N)
      width = min(1.0, visibleCells/totalCells )
      overlap = int(totalCells - visibleCells)
      self.horizScrollbar.set(min(gaps, max(0, prop)), max(width, min(1.0, prop + width)))
      self.hOffset = min(max(0, int('%1.1d' % (prop/gaps * (overlap+1) ) ) ), overlap)
  ...
  def moveVScrollbar(self, prop, prop2, refresh=True):
    ...
    gaps = prop+1-prop2
    visibleCells = float(self.visibleRows)
    ...
    if gaps == 0 or N == 0:
      ...
      width = min(1.0, visibleCells/totalCells )
      overlap = int(totalCells - visibleCells)
      self.vertScrollbar.set(min(gaps, max(0, prop)), max(width, min(1.0, prop + width)))
      self.vOffset = min(max(0, int('%1.1d' % (prop/gaps * (overlap+1)) ) ), overlap)
  ...
  def refreshScrollbars(self, reset=False):
    ...
    if N <= self.visibleCols:
      ...
      if reset:
        ...
        prop = self.horizScrollbar.get()[0]
...
class GraphMatrixPopup(BasePopup):
  ...
  def body(self, guiFrame):
    ...
    self.scrolledGraph = ScrolledGraph(guiFrame, showCoords=False)
    self.scrolledGraph.grid(row=0, column=0, sticky=Tkinter.NSEW)

    texts = ['Close']
    commands = [self.close]
    bottomButtons = ButtonList(guiFrame, texts=texts, commands=commands, expands=False)
...
class FilterMatrixPopup(BasePopup):
  ...
  def body(self, guiFrame):
    ...
    modeEntries = [ 'Literal','Case Sensitive Literal','Regular Expression' ]
    ...
    modeButtons  = RadioButtons(guiFrame, modeEntries, self.selectMode, borderwidth=0, relief='flat')
  ...
  def filterMatrix(self, includeMatches=True):
    ...
    if self.mode != 'Regular Expression':
...
class FilterMatrix:
  ...
  def __init__(self, parent, scrolledMatrix, searchString, column=None, mode='literal', selectedObjectsOnly=False):
