...
def recursiveMkdir(directory):
  """make directory if need to, creating parent directories if need be
  NBNB should not be used, use os.makedirs instead
  """
  ...
  if (os.path.exists(directory)):
    ...
    if (os.path.isdir(directory)):
      ...
      os.remove(directory) # not really a directory
 
  (parent_dir, tail_dir) = splitPath(directory)
 
  if (parent_dir != directory): # at top of absolute path keep getting '/'
 
    recursiveMkdir(parent_dir)
 
  os.mkdir(directory)
 
def recursiveDirOp(directory, file_func = None, dir_func = None):
  """recurse through directory, operating on files and directories
  ...
  for path in listdir:
    ...
    fullpath = normalisePath(directory + os.sep + path)
 
    if (os.path.isdir(fullpath)):
 
      recursiveDirOp(fullpath, file_func, dir_func)
...
def recursiveDirRemove(directory):
  """remove all contents of directory
  """
 
  recursiveDirOp(directory, os.remove, os.rmdir)
 
def backupCreateDir(directory, backup_directory):
  """backup and (re-)create directory
  """
 
  if (os.path.exists(backup_directory)):
 
    recursiveDirRemove(backup_directory)
 
  if (os.path.exists(directory)):
 
    recursiveMkdir(parent_directory(backup_directory))
